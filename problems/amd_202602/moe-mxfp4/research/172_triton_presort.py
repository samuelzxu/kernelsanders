"""
v172: Triton fused_moe_mxfp4_silu with FP4 activations PRE-SORTED into
a padded buffer. The kernel reads A at sorted_token_ids[i]//topk, so we
create a pre-expanded buffer where position sorted_token_ids[i]//topk already
has the right data. This avoids OOB on masked loads.

Alternative approach: just zero-init sorted_ids buffer to prevent garbage OOB.
"""
import os, sys, stat
...
# Use zero-initialized sorting buffers to prevent garbage OOB
_sorting_bufs = {}
...
def _fast_sorting(topk_ids, topk_weights, E, model_dim, dtype, block_size_M):
    ...
    if key not in _sorting_bufs:
        ...
        # ZERO-init sorted_ids to prevent garbage values for padding entries
        _sorting_bufs[key] = (
            torch.zeros(max_num_tokens_padded, dtype=dtypes.i32, device=device),
            torch.zeros(max_num_tokens_padded, dtype=dtypes.fp32, device=device),
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Diagnostic only: test Triton stage 1, return CK result
    if not _triton_tested:
        ...
        try:
            ...
            # Pad A and A_mx_scale: kernel reads at sorted_token_ids[i]//topk
            # Maximum possible: (max_num_tokens_padded-1)//topk
            max_token_idx = (max_num_tokens_padded - 1) // topk + 1
            pad_rows = max(0, max_token_idx - M)
            if pad_rows > 0:
            ...
            # Debug: check sorted_ids for max values
            sid_cpu = sorted_ids.cpu()
            max_sid = sid_cpu.max().item()
            min_sid = sid_cpu.min().item()
            num_valid = num_valid_ids.cpu()[0].item()
            max_a_idx = max_sid // topk
            print(f"[v172] sorted_ids: min={min_sid} max={max_sid} num_valid={num_valid} "
                  f"max_a_idx={max_a_idx} A_rows={x_fp4_padded.shape[0]}", file=sys.stderr)
            ...
            print(f"[v172] launching: M={M} E={E} N={N1} K_packed={x_fp4_padded.shape[1]} "
                  f"A_padded={x_fp4_padded.shape} scale_padded={x_scales_padded.shape}", file=sys.stderr)
            ...
            print(f"[v172] OK! a2 max={a2.abs().max().item():.4f} "
                  f"nonzero={a2.count_nonzero().item()}", file=sys.stderr)

        except Exception as e:
            import traceback
            print(f"[v172] FAILED: {e}", file=sys.stderr)
