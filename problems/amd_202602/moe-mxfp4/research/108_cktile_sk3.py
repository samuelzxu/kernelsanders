"""
v108: Try sk=3 for very sparse E=257 shapes (tok/exp < 5).
Current sk=2 for tok/exp < 5: 257*2=514 blocks, 2 waves.
With sk=3: 257*3=771 blocks, 3 waves.
Hypothesis: for E=257 bs=16 (0.56 tok/exp), sk=3 gives more parallelism
per expert → each split handles K/3 → shorter per-block compute time.
Risk: extra reduction step for sk=3 might negate any benefit.
Also tests block_m=32 vs 16 for very sparse shapes.
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if inter_dim > 1024:
        ...
        # Very sparse: try sk=3 for maximum K-parallelism
        use_cktile = True
        sk = 3
