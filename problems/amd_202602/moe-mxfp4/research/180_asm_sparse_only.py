"""
v180: Only use ASM 1-stage for truly sparse E=33 shapes (tokens_per_expert < 40).
For dense E=33 shapes (tokens_per_expert >= 40), let CK 2-stage handle.
ASM might be slower for dense shapes due to less efficient tiling.
"""
import os, sys, stat
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    # ASM 1-stage ONLY for sparse E<=33 shapes with moderate density
    # For very dense (tokens_per_expert >= 40), CK 2-stage is likely better
    if (inter_dim <= 1024 and expert <= 33 and tokens_per_expert < 40
            and q_type == QuantType.per_1x32 and is_shuffled):
        return MOEMetadata(functools.partial(fused_moe_1stage, kernelName="",
            activation=activation, quant_type=q_type), None, 32, 0, True)

    # CK 2-stage: tune block_m for large inter_dim
