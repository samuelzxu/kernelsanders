"""
v164: Use cktile split_k=2 for ALL cktile shapes (not just very sparse).
Tests whether sk=2 is better than sk=1 for moderate-density E=33 shapes.
"""
import os, sys, stat
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    elif tokens_per_expert < 5: use_cktile = True
    elif tokens_per_expert < 40 and expert <= 33: use_cktile = True

    if use_cktile and is_shuffled:
        md.ksplit = 2
        md.block_m = 16 if token < 2048 else 32 if token < 16384 else 64
        # CHANGE: Always use split_k=2 (not sk=1 for moderate shapes)
    ...
    # ASM 1-stage for E=33 d<=512 dense
