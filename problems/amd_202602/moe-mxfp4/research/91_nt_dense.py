"""
v91: Force non-temporal loads for ALL shapes via AITER_USE_NT=1.
The MI355X architecture guide recommends non-temporal loads for data
that is accessed once and does not need caching.

MoE weights are large and accessed once per expert per call.
NT loads bypass L2, freeing it for activation data reuse.

Current default: use_nt=True for tok/exp < 64, False otherwise.
By forcing use_nt=True for all shapes, we test if NT loads help
dense shapes (E=33 bs=512) where weights are especially large.
"""
import os
os.environ['HIP_FORCE_DEV_KERNARG'] = '1'
os.environ['GPU_MAX_HW_QUEUES'] = '2'
os.environ['AITER_USE_NT'] = '1'  # Force non-temporal loads for all shapes
