"""
v173: Extend ASM 1-stage to d_expert=2048 shapes.
Currently inter_dim>1024 falls to CK 2-stage (345us for shape 7).
ASM 1-stage is much faster for smaller shapes. Test if it works for larger inter_dim.
"""
import os, sys, stat
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    # ASM 1-stage: EXTENDED to inter_dim <= 4096 (was 1024)
    # The ASM kernel tiles along N, so it should work for larger inter_dim
    if inter_dim <= 4096 and q_type == QuantType.per_1x32 and is_shuffled:
        return MOEMetadata(
