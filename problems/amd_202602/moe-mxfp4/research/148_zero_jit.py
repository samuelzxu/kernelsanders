"""
v148: ALL 7 pre-compiled AITER modules + v103 logic.
Zero JIT compilation needed — saves ~275s on cold runners.
All computation time goes to actual benchmarking.
"""
import os, sys, stat
    ...
    for name in _MODULES:
        ...
        if not os.path.exists(path):
            ...
            try:
                ...
                print(f"[v148] Downloaded {name}", file=sys.stderr)
            except Exception as e:
                print(f"[v148] Failed {name}: {e}", file=sys.stderr)

try:
    _install_precompiled()
except Exception as e:
    print(f"[v148] Install failed: {e}", file=sys.stderr)
