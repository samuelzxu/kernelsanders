"""
v101: Extend cktile sk=1 to E=33 bs=512 d=512 (tok/exp=124, inter_dim=512).
Based on v103 (v98+opus), adds E=33 bs=512 d=512 to cktile coverage.
Key insight: default CK uses block_m=64 for E=33 bs=512 (only 26% CU utilization).
cktile with block_m=16 gives ~1 full wave (103% utilization) - much better occupancy.
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if inter_dim > 1024:
        ...
        # E=33 all bs with d<=512: covers bs=16/128/512 (up to tok/exp=139.6)
        use_cktile = True
        sk = 1
