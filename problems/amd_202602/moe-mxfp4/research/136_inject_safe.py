"""
v136: Safe CSV injection — only for shapes where 64x32 kernel works.
v135 showed E=33 bs=512 d=512 dropped from 216us to 183us (15% faster!)
when CSV entry was injected. But E=33 bs=512 d=2048 FAILED correctness.
Fix: don't inject for inter_dim > 1024 (let heuristic handle d=2048).
Also don't inject kernel names for E=33 — let C++ runtime pick the best.
Only inject block_m override via CSV for shapes that benefit.
"""
import os
...
# Kernel names from the CSV — only use for shapes with matching geometry
_K64_S1 = 'moe_ck2stages_gemm1_64x32x32x128_1x1_MulABScaleShuffled_v3_Nswizzle0_Quant3_MulRoutedWeight0_silu_FP4X2_FP4X2_B16'
_K64_S2 = 'moe_ck2stages_gemm2_64x32x32x128_1x1_MulABScaleExpertWeightShuffled_v1_Nswizzle0_Quant3_MulRoutedWeight1_FP4X2_FP4X2_B16'
...
if _fm.cfg_2stages is not None:
    ...
    # E=257 shapes (inter_dim=256): inject CSV entries with 64x32 kernel
    for token, bm in [(16, 32), (32, 32), (64, 32), (128, 32), (256, 32), (512, 32)]:
        cfg = {'kernelName1': _K64_S1, 'kernelName2': _K64_S2,
               'run_1stage': False, 'ksplit': 0, 'block_m': bm}
        for g1u1 in [True, 1]:
            for dw in [False, 0]:
                key = (256, token, 7168, 256, 257, 9, *_common, g1u1, dw)
                _fm.cfg_2stages[key] = cfg

    # E=33, inter_dim=512: inject with 64x32 kernel and heuristic block_m
    # v135 showed 183us vs 216us — huge win!
    for token, bm in [(16, 32), (128, 64), (512, 64)]:
        cfg = {'kernelName1': _K64_S1, 'kernelName2': _K64_S2,
               'run_1stage': False, 'ksplit': 0, 'block_m': bm}
        for g1u1 in [True, 1]:
            for dw in [False, 0]:
                key = (256, token, 7168, 512, 33, 9, *_common, g1u1, dw)
                _fm.cfg_2stages[key] = cfg

    # E=33, inter_dim=2048: DO NOT inject — let heuristic handle (64x32 fails here)
