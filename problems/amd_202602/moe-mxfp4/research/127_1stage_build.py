"""
v127: Build ASM module at import time, then use 1-stage for applicable shapes.
The 1-stage fmoe_g1u1 kernel fuses both GEMMs, eliminating requant overhead.
Build takes ~30s but fits within 12-min total budget on warm runners.
Only for d_expert <= 1536 (d=2048 has precision issues).
Uses block_m=32 for 1-stage shapes (kernel name has 32x in tile).
Falls back to 2-stage (cktile or CK) for other shapes.
"""
import os
...
# Eagerly trigger ASM module build at import time
_1stage_ok = False
try:
    # This triggers JIT build of module_moe_asm if not cached (~30s build, <1s if cached)
    _ = aiter.fmoe_g1u1
    _1stage_ok = True
except Exception:
    _1stage_ok = False
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Decide: 1-stage (fused both GEMMs) vs 2-stage (cktile or CK)
    # 1-stage: for dense non-cktile shapes with d_expert <= 1536 and ASM available
    tokens_per_expert = (M * topk) / E
    can_cktile = (inter_dim <= 1024 and
                  (tokens_per_expert < 5 or (tokens_per_expert < 40 and E <= 33)))
    use_1stage = (_1stage_ok and isG1U1 and inter_dim <= 1536 and not can_cktile)

    if use_1stage:
        # 1-stage path: block_m=32 required for ASM kernel
        block_size_M = 32
        ...
        # 2-stage path: cktile for sparse, CK for dense/d>1536
        metadata = _patched(
