"""
v158: Force 1-stage ASM MoE (fmoe_g1u1) for FP4/per_1x32.
The 1-stage path is registered in AITER but auto-disabled for per_1x32.
Force it by setting run_1stage=True. This fuses gate+up GEMM + SiLU + down GEMM
into a SINGLE kernel launch — no intermediate materialization.
"""
import os, sys, stat
...
from aiter.fused_moe import (fused_moe, fused_moe_1stage, get_inter_dim, get_padded_M,
    get_2stage_cfgs, MOEMetadata, get_block_size_M,
    cktile_moe_stage1, cktile_moe_stage2, fused_moe_2stages)

_orig = get_2stage_cfgs.__wrapped__
BLOCK_SIZE_M = 32  # 1-stage default
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    # Try 1-stage for all shapes
    block_m = get_block_size_M(token, topk, expert, inter_dim)

    # Force 1-stage ASM path
    return MOEMetadata(
        functools.partial(
            fused_moe_1stage,
            kernelName="",  # let AITER pick the best kernel
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Call fused_moe_1stage DIRECTLY
    fused_moe_1stage(
        hidden_states, w1, w2,
