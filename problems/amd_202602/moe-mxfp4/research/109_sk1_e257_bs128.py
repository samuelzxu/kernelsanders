"""
v109: Test sk=1 for E=257 bs=128 (tok/exp=4.48 near sk=2 threshold).
Current: tok/exp<5 → sk=2 for E=257 bs=128.
With sk=1 + block_m=16: 257 M-blocks → 1 wave of 257 CUs (excellent utilization!).
With sk=2 + block_m=16: 514 M-blocks → 2 waves (extra reduction overhead).
Hypothesis: 1 wave with full K=7168 may be faster than 2 waves with K=3584 each.
For E=257 bs=16 (tok/exp=0.56): keep sk=2 (sparse enough to need K-split parallelism).
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if inter_dim > 1024:
        ...
        # Very sparse (E=257 bs=16: tok/exp=0.56): keep sk=2 for K-parallelism
        use_cktile = True
        sk = 2
    elif tokens_per_expert < 5:
        # Moderate sparse (E=257 bs=128: tok/exp=4.48): try sk=1 (1 wave)
        # Previously sk=2 gave 2 waves with split overhead
        # sk=1 with 257 M-blocks → exactly 1 wave → better occupancy?
        use_cktile = True
        sk = 1
