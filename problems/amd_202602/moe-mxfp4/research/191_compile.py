"""
v191: torch.compile on the hot path. This compiles the Python dispatch
logic into optimized native code, eliminating interpreter overhead.
Uses fullgraph=False to handle dynamic control flow.
"""
import os, sys, stat
...
def _get_compiled():
    ...
    if _compiled_fn is None:
        try:
            _compiled_fn = torch.compile(fused_moe, mode="reduce-overhead", fullgraph=False)
            print("[v191] torch.compile succeeded", file=sys.stderr)
        except Exception as e:
            print(f"[v191] torch.compile failed: {e}", file=sys.stderr)
            _compiled_fn = fused_moe
    return _compiled_fn
...
def custom_kernel(data: input_t) -> output_t:
    ...
    fn = _get_compiled()
    return fn(hidden_states, w1, w2, topk_weights, topk_ids,
