"""
v196: Test different ASM kernel variants for 1-stage shapes.
The runner has multiple MXfp4 Silu ASM kernels:
- vs (vector store) vs novs (no vector store)
- 1tg (1 tile group) vs 2tg (2 tile groups)
Test which variant is fastest for our E=33 d=512 shapes.
"""
import os, sys, stat
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if use_cktile and is_shuffled:
        ...
        return md
    # ASM 1-stage: try "novs" (no vector store) variant
    if inter_dim <= 1024 and expert <= 33 and q_type == QuantType.per_1x32 and is_shuffled:
        # Test: use novs variant instead of vs
        kernel = "fmoe_bf16_pertokenMXfp4_g1u1_novs_silu_1tg_ps_32x512"
        return MOEMetadata(functools.partial(fused_moe_1stage,
            kernelName=kernel,
