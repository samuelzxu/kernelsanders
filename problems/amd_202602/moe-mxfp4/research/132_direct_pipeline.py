"""
v132: Replace fused_moe_2stages with a stripped-down direct pipeline.
Eliminates ~20 conditional branches and redundant operations per call.
Only handles our exact code path: per_1x32 + Silu + fp4x2 + token<=1024.
Saves ~3-5us of Python overhead per call.
"""
import os
...
# Patch get_2stage_cfgs for internal AITER calls
...
def _direct_2stages(hidden_states, w1, w2, topk,
                    sorted_ids, sorted_weights, sorted_expert_ids, num_valid_ids,
                    moe_buf, block_size_M, w1_scale, w2_scale, metadata):
    """Stripped-down fused_moe_2stages for per_1x32 + Silu + fp4x2 only."""
    token_num = hidden_states.shape[0]
    E, _, inter_dim = get_inter_dim(w1.shape, w2.shape)
    dtype = moe_buf.dtype
    device = hidden_states.device
    is_shuffled = getattr(w1, "is_shuffled", False)

    # Step 1: Quant hidden_states (always fused path since token_num <= 1024)
    ...
    # Step 4: Requant or skip (cktile skips requant via ksplit > 1)
