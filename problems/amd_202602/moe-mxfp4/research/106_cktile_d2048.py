"""
v106: Attempt cktile for E=33 bs=512 d=2048 (inter_dim=2048).
Previously excluded because BF16 MFMA < FP4 MFMA for large K.
BUT: CU utilization jumps 26% → 116% (4.5x more M-blocks with block_m=16).
Hypothesis: 4.5x better occupancy may offset ~4x lower BF16 throughput.
Net expected effect: ~10% improvement if theory holds (risky, could be worse).
If worse, reverts to v105 combined approach.
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if tokens_per_expert < 5:
        ...
        # Cover all E=33 shapes: d=512 AND d=2048
        # For d=512: avoids FP4 quant + better CU util (39% → 116%)
        # For d=2048: risky but 26% → 116% CU util may offset lower BF16 throughput
        use_cktile = True
        sk = 1
