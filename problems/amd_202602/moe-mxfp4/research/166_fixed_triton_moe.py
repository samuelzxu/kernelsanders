"""
v166: Re-test fused_moe_mxfp4 with CORRECT scale layout [E, N, K//32].
v165 proved tl.dot_scaled works with this format. Now verify the full MOE kernel.
"""
import os, sys, stat
...
from aiter.fused_moe import fused_moe, get_2stage_cfgs, cktile_moe_stage1, cktile_moe_stage2, fused_moe_1stage, MOEMetadata, get_inter_dim, get_padded_M
import aiter.fused_moe as _fm
from aiter.ops.triton.moe.moe_op_mxfp4 import fused_moe_mxfp4

# Standard cktile patching
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Test Triton stage 1 with CORRECT scale layout
    if not _triton_tested:
        _triton_tested = True
        try:
            block_size_M = 32
            sorted_ids, sorted_weights, sorted_expert_ids, num_valid_ids, _ = _fast_sorting(
                topk_ids, topk_weights, E, d_hidden_pad, hidden_states.dtype, block_size_M)

            N1 = gate_up_weight.shape[1]  # 2*d_expert_pad
            # Scale: [E*N, K//32] → [E, N, K//32] — this IS [E, N, K//32] format!
            ...
            stage1_out = torch.zeros((max_sorted, topk, 2 * d_expert_pad),
            ...
            fused_moe_mxfp4(
                ...
                stage1_out,                        # C: [max_sorted, topk, 2*d_expert_pad]
                a_scale, b_scale,
                None,                              # A_mx_scale: None (BF16)
                b_scale_3d,                        # B_mx_scale: [E, N, K//32] ← CORRECT!
            ...
            # Check a few values against stage1_out
            print(f"[v166] Triton stage1 OK! shape={stage1_out.shape} "
                  f"max={stage1_out.abs().max().item():.4f}", file=sys.stderr)
            _triton_ok = True
        except Exception as e:
            import traceback
            print(f"[v166] Triton stage1 failed: {e}", file=sys.stderr)
            traceback.print_exc(file=sys.stderr)

    # Use standard CK/ASM path for actual output
