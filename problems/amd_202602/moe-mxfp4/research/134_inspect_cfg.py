"""
v134: Diagnostic - inspect cfg_2stages dict to understand key format
and what entries exist vs what's missing.
"""
import os
...
# Force CSV loading by calling get_2stage_cfgs for a known shape
...
# Inspect the global config dict
import sys
if _fm.cfg_2stages is not None:
    keys = list(_fm.cfg_2stages.keys())
    # Print all keys to stderr so they show in runner logs
    print(f"[DIAG] cfg_2stages has {len(keys)} entries", file=sys.stderr)
    for k in sorted(keys, key=str):
        v = _fm.cfg_2stages[k]
        kn1 = v.get('kernelName1', '?')[:60] if isinstance(v, dict) else '?'
        bm = v.get('block_m', '?') if isinstance(v, dict) else '?'
        print(f"[DIAG] key={k} block_m={bm} kn1={kn1}", file=sys.stderr)
else:
    print("[DIAG] cfg_2stages is None!", file=sys.stderr)

# Also check: what does the heuristic return for E=33 shapes?
for token in [16, 128, 512]:
    for inter_dim in [512, 2048]:
        md = get_2stage_cfgs(
            get_padded_M(token), 7168, inter_dim, 33, 9,
            dtypes.bf16, dtypes.fp4x2, dtypes.fp4x2,
            QuantType.per_1x32, True,
            ActivationType.Silu, False, 0, 0, True,
        )
        print(f"[DIAG] E=33 token={token} inter={inter_dim}: block_m={md.block_m} ksplit={md.ksplit} run_1stage={md.run_1stage}", file=sys.stderr)

# Check E=257 bs=512
md257 = get_2stage_cfgs(
    get_padded_M(512), 7168, 256, 257, 9,
    dtypes.bf16, dtypes.fp4x2, dtypes.fp4x2,
    QuantType.per_1x32, True,
    ActivationType.Silu, False, 0, 0, True,
)
print(f"[DIAG] E=257 token=512 inter=256: block_m={md257.block_m} ksplit={md257.ksplit}", file=sys.stderr)
# Try to extract kernel name from stage1 partial
if hasattr(md257.stage1, 'keywords'):
    print(f"[DIAG] E=257 bs=512 stage1 keywords: {md257.stage1.keywords}", file=sys.stderr)
elif hasattr(md257.stage1, 'func'):
    print(f"[DIAG] E=257 bs=512 stage1 func: {md257.stage1.func.__name__}", file=sys.stderr)

# Now use v103 logic as the actual kernel
