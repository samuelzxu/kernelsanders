"""
Version 17 + try FlyDSL stage2 if available.
FlyDSL stage2 kernels are reportedly faster for the reduction step.
The tuned CSV uses them for token >= 1024, but they might help at smaller sizes too.
Falls back to CK stage2 if FlyDSL unavailable.
"""
import torch
...
# Check FlyDSL availability at import time
try:
    from aiter.ops.flydsl.utils import is_flydsl_available
    from aiter.ops.flydsl.moe_kernels import flydsl_moe_stage2
    _HAS_FLYDSL = is_flydsl_available()
except ImportError:
    _HAS_FLYDSL = False
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Try FlyDSL stage2 if available, else fall back to CK
    if _HAS_FLYDSL:
        flydsl_moe_stage2(
            inter_states=a2_q,
            w2=w2,
