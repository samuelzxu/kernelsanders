"""
v149: Pre-compiled CK modules + Triton fused_moe_mxfp4 for dense shapes.
Tests whether tl.dot_scaled Triton kernels are faster than CK for dense MOE.
CK fallback for sparse shapes (cktile) and if Triton fails.

Strategy:
- Dense shapes (tok/expert >= 40 or d_expert > 1024): Try Triton first, CK fallback
- Sparse shapes: CK cktile (proven faster)
- Test shapes: CK (safe)
"""
...
try:
    ...
    _triton_ok = True
    print("[v149] Triton MXFP4 available", file=sys.stderr)
except ImportError as e:
    print(f"[v149] No Triton MXFP4: {e}", file=sys.stderr)
...
def _triton_kernel(data):
    """Triton MXFP4 path for dense shapes — Stage 1 only test."""
    (
        hidden_states, gate_up_weight, down_weight,
        gate_up_weight_scale, down_weight_scale,
        gate_up_weight_shuffled, down_weight_shuffled,
    ...
    # For now, just use CK — Triton path needs more work on stage 2 indexing
    # This is a placeholder to test Triton JIT compilation time
    return _ck_kernel(data)


_triton_compile_tested = False
_triton_failed = False

@torch.inference_mode()
def custom_kernel(data: input_t) -> output_t:
    global _triton_compile_tested, _triton_failed

    # Always use CK for correctness — Triton stage 2 indexing not solved yet
    return _ck_kernel(data)
