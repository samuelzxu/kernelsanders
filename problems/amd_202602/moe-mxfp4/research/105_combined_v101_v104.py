"""
v105: Combine v101 (cktile for E=33 d=512 dense) + v104 (block_m=64 for E=33 d=2048).
Both optimizations target E=33 bs=512 but for different inter_dim ranges.
- inter_dim <= 512, tok/exp < 150, expert <= 33 → cktile sk=1 (v101)
- inter_dim > 1024, expert <= 33, tok/exp >= 100 → block_m=64 override (v104)
Expected improvement: both E=33 bs=512 d=512 AND d=2048 should improve.
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if inter_dim > 1024:
        use_cktile = False
        # v104: block_m=64 for E=33 bs=512 d=2048
        # Default block_m=128 → 66/256=26% CU util; block_m=64 → 99/256=39% CU util
        if expert <= 33 and tokens_per_expert >= 100:
            md.block_m = 64
    elif tokens_per_expert < 5:
        use_cktile = True
        sk = 2
    elif inter_dim <= 512 and tokens_per_expert < 150 and expert <= 33:
        # v101: E=33 all bs with d<=512 (tok/exp up to 139.6)
        use_cktile = True
        sk = 1
    elif tokens_per_expert < 40 and expert <= 33:
        # v103: E=33 bs=16/128 with d=512 (already covered above, but keep for safety)
        use_cktile = True
        sk = 1
