"""
v116: Force 256x kernel variant for E=257 bs=512 via CSV config injection.
Currently CK auto-selects 64x32x32x128 for E=257 bs=512 (d_expert=256).
The 256x32x128x128 variant is used for E=257 bs=128 and might be better
for bs=512 too (wider N-tile = 128 vs 32, better memory access pattern).
Uses AITER_CONFIG_FMOE env to inject custom tuned config.
"""
import os
import tempfile

# Create custom CSV config that forces 256x kernel for E=257 bs=512
_csv_content = """cu_num,token,model_dim,inter_dim,expert,topk,act_type,dtype,q_dtype_a,q_dtype_w,q_type,use_g1u1,doweight_stage1,block_m,kernelName1,kernelName2,run_1stage,ksplit
256,512,7168,256,257,9,ActivationType.Silu,torch.bfloat16,torch.float4_e2m1fn_x2,torch.float4_e2m1fn_x2,QuantType.per_1x32,True,False,64,moe_ck2stages_gemm1_256x32x128x128_1x4_MulABScaleShuffled_v3_Nswizzle0_Quant3_MulRoutedWeight0_silu_FP4X2_FP4X2_B16,moe_ck2stages_gemm2_64x32x32x128_1x1_MulABScaleExpertWeightShuffled_v1_Nswizzle0_Quant3_MulRoutedWeight1_FP4X2_FP4X2_B16,False,0
"""

_tmpf = tempfile.NamedTemporaryFile(mode='w', suffix='.csv', delete=False, prefix='moe_tune_')
_tmpf.write(_csv_content)
_tmpf.flush()
os.environ['AITER_CONFIG_FMOE'] = _tmpf.name
