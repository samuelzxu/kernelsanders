"""
v176: Triton fused_moe_mxfp4_silu with CLAMPED sorted_ids.
Root cause of v168-v172 crash: AITER sorting kernel sets padding entries
to ~150M (not M*topk). AMD masked loads still compute OOB addresses.
Fix: clamp sorted_ids to [0, M*topk-1] and pad A by 1 extra row.
Still returns CK for comparison (diagnostic mode).
"""
import os, sys, stat
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Only test Triton on non-cktile dense shapes (diagnostic)
    if not use_cktile and not _triton_tested:
        ...
        try:
            ...
            # CLAMP sorted_ids to prevent OOB (padding entries can be ~150M!)
            num_valid = M * topk
            ...
            # Pad A by 1 row for clamped padding entries (they map to index M after //topk)
            ...
            print(f"[v176] Triton silu FP4 OK! a2 max={a2.abs().max().item():.4f} "
                  f"nonzero={a2.count_nonzero().item()}/{a2.numel()}", file=sys.stderr)

        except Exception as e:
            import traceback
            print(f"[v176] FAILED: {e}", file=sys.stderr)
