"""
v115: Test dispatch_policy=1 ONLY for E=33 shapes.
v09 showed dispatch_policy=1 was 24-74% SLOWER for E=257,
but E=33 was never tested separately.
With E=33 (few experts), dispatch_policy=1 might help because
the per-expert routing is more balanced (fewer, denser experts).
E=257 shapes keep dispatch_policy=0 (default).
"""
import os
...
def _fast_sorting(topk_ids, topk_weights, E, model_dim, dtype, block_size_M):
    ...
    # Use dispatch_policy=1 for E=33 (dense experts), 0 for E=257 (sparse)
    dp = 1 if E <= 33 else 0
    _sort_fwd(
