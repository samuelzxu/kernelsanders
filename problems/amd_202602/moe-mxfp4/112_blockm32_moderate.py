"""
v112: Test block_m=32 for moderate-density cktile shapes.
Currently all cktile shapes use block_m=16 (token < 2048).
E=33 bs=128 (tok/exp=34.9) has ~35 tokens per expert.
With block_m=32: ~2 blocks per expert (efficient).
With block_m=16: ~3 blocks per expert (more overhead).
Both fit in 1 wave (66 vs 99 blocks for 256 CUs).
Hypothesis: block_m=32 reduces per-block overhead for moderate shapes.
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if use_cktile and is_shuffled:
        md.ksplit = 2
        # Use block_m=32 for moderate-density shapes (tok/exp >= 10)
        # Keep block_m=16 for sparse shapes where fine granularity helps
        if tokens_per_expert >= 10:
            md.block_m = 32 if token < 16384 else 64
        else:
