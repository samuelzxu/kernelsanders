"""
v104: Override block_m=64 for E=33 bs=512 d=2048 in standard CK path.
Default block_m=128 gives only 26% CU utilization (66/256 M-blocks).
block_m=64 gives 39% CU utilization (99/256 M-blocks) - 50% improvement.
v54 showed block_m=32 was 21% slower; block_m=64 may find better tradeoff.
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if inter_dim > 1024:
        use_cktile = False
        # For large inter_dim with low CU utilization: try block_m=64
        # Default is block_m=128 giving ~66/256 = 26% CU util
        # block_m=64 gives ~99/256 = 39% CU util
