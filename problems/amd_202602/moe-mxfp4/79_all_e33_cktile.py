"""
v79: Extend cktile to ALL E=33 shapes (including dense bs=512).
Use split_k=1 for all E=33 (eliminates quant without split overhead).
Keep split_k=2 for E=257 sparse shapes.
Keep CK for E=257 bs=512 (CSV tuned).
Keep CK for E=33 d=2048 (large inter_dim, cktile overhead too high).
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if inter_dim > 1024:
        use_cktile = False  # Large inter_dim: cktile overhead too high
    elif tokens_per_expert < 5:
        use_cktile = True
        sk = 2  # Very sparse: split_k=2 for parallelism
    elif expert <= 33:
        use_cktile = True
        sk = 1  # ALL E=33 with small inter_dim: cktile sk=1

    if use_cktile and is_shuffled:
        md.ksplit = 2  # Flag for bf16 activation path
