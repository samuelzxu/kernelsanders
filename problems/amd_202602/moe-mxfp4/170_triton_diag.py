"""
v170: Diagnostic - run Triton fused_moe_mxfp4_silu stage 1 only (FP4 act),
check output, but return CK result. Isolates whether crash is in Triton or cktile_stage2.
"""
import os, sys, stat
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if not use_cktile and not _triton_tested:
        ...
        try:
            ...
            print(f"[v170] quant OK: x_fp4={x_fp4.shape} x_scales={x_scales.shape}", file=sys.stderr)
            ...
            print(f"[v170] pad OK: padded={x_fp4_padded.shape} scales={x_scales_padded.shape} pad_rows={pad_rows}", file=sys.stderr)
            ...
            print(f"[v170] launching Triton silu: M={M} E={E} N={N1} K={d_hidden_pad} "
                  f"max_sorted={max_num_tokens_padded} out={stage1_out.shape}", file=sys.stderr)
            ...
            print(f"[v170] Triton silu stage1 OK! a2={a2.shape} "
                  f"max={a2.abs().max().item():.4f} "
                  f"nonzero={a2.count_nonzero().item()}", file=sys.stderr)

        except Exception as e:
            import traceback
            print(f"[v170] Triton silu FAILED: {e}", file=sys.stderr)
            traceback.print_exc(file=sys.stderr)

    # Always return CK result
