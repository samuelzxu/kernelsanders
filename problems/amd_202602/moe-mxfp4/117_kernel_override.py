"""
v117: Override CK kernel variant for E=257 bs=512 via metadata monkey-patch.
Force the 256x32x128x128_1x4 stage1 kernel (wider N-tile) for E=257 bs=512.
Currently auto-selects 64x32x32x128_1x1 which may not be optimal.
The 256x variant is already used for E=257 bs=128, suggesting it handles
sparse expert routing well. For bs=512, it might also be better.
"""
import os
...
# Get references to CK stage functions for kernel name override
from aiter.ops.moe_op import ck_moe_stage1 as _ck_stage1, ck_moe_stage2 as _ck_stage2

_orig = get_2stage_cfgs.__wrapped__

# Kernel names observed from runner logs
_K256x_STAGE1 = 'moe_ck2stages_gemm1_256x32x128x128_1x4_MulABScaleShuffled_v3_Nswizzle0_Quant3_MulRoutedWeight0_silu_FP4X2_FP4X2_B16'
_K64x_STAGE1 = 'moe_ck2stages_gemm1_64x32x32x128_1x1_MulABScaleShuffled_v3_Nswizzle0_Quant3_MulRoutedWeight0_silu_FP4X2_FP4X2_B16'
_K64x_STAGE2 = 'moe_ck2stages_gemm2_64x32x32x128_1x1_MulABScaleExpertWeightShuffled_v1_Nswizzle0_Quant3_MulRoutedWeight1_FP4X2_FP4X2_B16'
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    elif expert == 257 and tokens_per_expert > 10:
        # For E=257 bs=512 (tok/exp=18): try forcing 256x stage1 kernel
        # Auto-selects 64x32x32x128 but 256x32x128x128 may be better
        md.stage1 = functools.partial(
            _ck_stage1,
            kernelName=_K256x_STAGE1,
            activation=ActivationType.Silu,
