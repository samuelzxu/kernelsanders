"""
v135: Inject CSV entries with correct key types.
The cfg_2stages dict only has token=1 for per_1x32 — CSV entries for
token=32/64/128/256/512 aren't loading (key type mismatch between
pandas int64 and Python bool for use_g1u1/doweight_stage1).
Fix: inject entries with BOTH int and bool key variants.
Also change block_m from CSV's 32 to heuristic-optimal values for
our benchmark shapes.
"""
import os
...
# Inject entries for E=257 per_1x32 shapes that are MISSING from the dict
# Use heuristic-optimal block_m values instead of CSV's block_m=32
if _fm.cfg_2stages is not None:
    ...
    # Inject for E=257 shapes with both True/1 and False/0 key variants
    for token, bm in [(16, 32), (32, 32), (64, 32), (128, 32), (256, 32), (512, 32)]:
        cfg = {**_base, 'block_m': bm}
        for g1u1 in [True, 1]:
            for dw in [False, 0]:
                key = (256, token, 7168, 256, 257, 9, *_common, g1u1, dw)
                _fm.cfg_2stages[key] = cfg

    # Also inject for E=33 shapes (currently have NO CSV entries at all)
    # Use heuristic block_m values
    for token, inter, bm in [
        (16, 512, 32), (128, 512, 64), (512, 512, 64),
        (512, 2048, 128),
    ]:
        cfg = {**_base, 'block_m': bm}
        for g1u1 in [True, 1]:
            for dw in [False, 0]:
                key = (256, token, 7168, inter, 33, 9, *_common, g1u1, dw)
                _fm.cfg_2stages[key] = cfg
