"""
v138: Override stage functions for E=33 d=512 dense shapes with kernel name hint.
v135 showed 15% speedup (216us -> 183us) when CSV entry provided kernel name.
The CSV injection path times out on cold runners because it affects the code path.
This version injects the kernel name directly into the stage partial functions
WITHOUT modifying cfg_2stages, avoiding any CSV code path changes.
"""
import os
...
# Import CK stage functions for direct override
try:
...
# Kernel name that proved faster for E=33 d=512 in v135
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    elif _has_ck_funcs and expert <= 33 and inter_dim <= 512 and tokens_per_expert >= 40:
        # E=33 d=512 dense shapes: override with explicit kernel name hint
        # v135 showed 15% speedup with this kernel name vs heuristic dispatch
        md.stage1 = functools.partial(
            ck_moe_stage1,
            kernelName=_K64_S1,
            activation=ActivationType.Silu,
