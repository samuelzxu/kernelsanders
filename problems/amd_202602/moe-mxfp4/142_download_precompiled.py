"""
v142: Download pre-compiled AITER modules at runtime.
Saves ~52s of sorting/quant JIT compilation on cold runners.
Falls back gracefully if download fails.
"""
import os, sys, stat

# Download pre-compiled modules to AITER JIT cache BEFORE any AITER imports
_JIT_DIR = "/home/runner/aiter/aiter/jit"
_BASE_URL = "https://github.com/samuelzxu/aiter-precompiled/releases/download/v0.1-rocm72"
_MODULES = [
    ...
    for name in _MODULES:
        ...
        if not os.path.exists(path):
            ...
            try:
                ...
                print(f"[precompiled] Downloaded {name}", file=sys.stderr)
            except Exception as e:
                print(f"[precompiled] Failed to download {name}: {e}", file=sys.stderr)

try:
    _install_precompiled()
except Exception as e:
    print(f"[precompiled] Install failed: {e}", file=sys.stderr)
