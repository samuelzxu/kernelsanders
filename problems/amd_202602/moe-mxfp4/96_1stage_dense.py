"""
v96: Use 1-stage fmoe_g1u1 for dense shapes, cktile for sparse.
The 1-stage kernel fuses GEMM1 + SwiGLU + GEMM2 into a single kernel,
eliminating the intermediate requantization step entirely.
Uses RAW (un-shuffled) weights for 1-stage path.
Keeps cktile BF16 for sparse shapes where quant overhead dominates.
Excludes E=65 from 1-stage path (known precision issue).
"""
import os
...
from aiter.fused_moe import (
    ...
    get_padded_M, get_2stage_cfgs, BLOCK_SIZE_M,
    cktile_moe_stage1, cktile_moe_stage2, MOEMetadata,
)
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    elif not use_cktile and expert != 65:
        # Dense shapes: try 1-stage fused kernel
        use_1stage = True
        md.run_1stage = True
        md.block_m = BLOCK_SIZE_M
        md.stage1 = functools.partial(
            fused_moe_1stage,
            kernelName="",
            activation=ActivationType.Silu,
...
def custom_kernel(data: input_t) -> output_t:
    ...
    M, topk = topk_ids.shape
    w1_shuffled, w2_shuffled = gate_up_weight_shuffled, down_weight_shuffled
    E, model_dim, inter_dim = get_inter_dim(w1_shuffled.shape, w2_shuffled.shape)
    isG1U1 = inter_dim != w1_shuffled.shape[1]
    dtype = hidden_states.dtype
    is_shuffled = getattr(w1_shuffled, "is_shuffled", False)
    ...
    if metadata.run_1stage:
        # 1-stage: use RAW weights (not shuffled) with fmoe_g1u1
        metadata.stage1(
            hidden_states, gate_up_weight, down_weight,
        ...
        # 2-stage: use shuffled weights (cktile or CK path)
        fused_moe_2stages(
