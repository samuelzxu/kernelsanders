"""
v95: Pre-allocate sorting buffers + block_m tuning for E=33 dense shapes.
Eliminates 5 torch.empty() calls per invocation by caching sorting buffers.
Also overrides block_m for E=33 dense shapes (heuristic gives 64/128,
try 32 which is what DSv3 tuned config uses for E=257).
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    elif not use_cktile and expert <= 33 and inter_dim <= 512:
        # E=33 dense d=512: heuristic gives block_m=64, try 32
        md.block_m = 32
...
# Pre-allocated buffer cache keyed by (M, E, model_dim, block_size_M)
_sorting_bufs = {}
