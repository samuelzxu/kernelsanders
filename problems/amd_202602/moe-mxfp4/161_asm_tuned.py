"""
v161: Per-shape ASM kernel selection for 1-stage MXFP4.
Tests specific kernel variants to find optimal config per shape.
Uses CK 2-stage for shapes where ASM is known to be slower.
"""
import os, sys, stat
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    # For dense shapes: try ASM 1-stage with specific kernel
    # Only use ASM for shapes where d_expert <= 512 (32x512 tile covers the N dim efficiently)
    if inter_dim <= 1024 and q_type == QuantType.per_1x32 and is_shuffled:
        # Force 1-stage ASM for medium-density shapes
        return MOEMetadata(
    ...
    # Default CK 2-stage for d_expert > 1024
    return md
