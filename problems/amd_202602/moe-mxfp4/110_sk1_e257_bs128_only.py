"""
v110: Correctly test sk=1 ONLY for E=257 bs=128 (tok/exp=4.48).
Fixes v109's bug where E=33 bs=16 (tok/exp=3.9) was also changed to sk=1.
Key: use expert count to distinguish E=257 shapes from E=33 shapes.
- E=257 bs=128 (tok/exp=4.48, expert=257): try sk=1 (from sk=2)
- E=33 bs=16 (tok/exp=3.9, expert=33): keep sk=2 (don't change)
- E=257 bs=16 (tok/exp=0.56, expert=257): keep sk=2
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if inter_dim > 1024:
        ...
        use_cktile = True
        # For large expert count (E=257): try sk=1 for tok/exp in [2, 5)
        # For small expert count (E=33): always sk=2 when tok/exp < 5
        sk = 2 if (expert <= 33 or tokens_per_expert < 2) else 1
