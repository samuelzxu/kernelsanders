"""
v94: Use flydsl stage2 with afp4_wfp4 for dense E=33 shapes.
The tuned CSV uses flydsl for token>=1024 but CK for token<1024.
Maybe flydsl stage2 is faster than CK stage2 even at token=512.
This keeps input quant + requant (standard FP4 pipeline) but
swaps only the stage2 kernel to flydsl.
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    elif (not use_cktile and is_shuffled and is_flydsl_available()
          and expert <= 33 and inter_dim <= 512 and tokens_per_expert >= 40):
        # Dense E=33 d=512: try flydsl stage2 with fp4 activations
        # Keep CK stage1 + requant, swap only stage2
        flydsl_name = "flydsl_moe2_afp4_wfp4_bf16_t64x256x256_reduce"
        md.stage2 = functools.partial(
            _flydsl_stage2_wrapper,
