"""
v80: Tune the split_k threshold. Try split_k=1 for E=257 bs=128 (4.5 tok/exp).
v78 used split_k=2 for tok/exp < 5. E=257 bs=128 is right at the boundary.
What if split_k=1 is better for this shape too?

Threshold: tok/exp < 2 → sk=2, tok/exp < 40 → sk=1 (for small inter_dim).
"""
import os
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    if inter_dim > 1024:
        use_cktile = False
    elif tokens_per_expert < 1:
        use_cktile = True
        sk = 2  # Ultra sparse (E=257 bs=16): need split_k for parallelism
    elif tokens_per_expert < 5:
        # E=257 bs=128 (4.5), E=33 bs=16 (4.4): try sk=1 (no split overhead)
        use_cktile = True
        sk = 1
    elif tokens_per_expert < 40 and expert <= 33:
        # E=33 bs=128 (35): sk=1 (proven in v78)
        use_cktile = True
        sk = 1
