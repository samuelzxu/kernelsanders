"""
v145: Download pre-compiled AITER modules + v103 logic.
Saves ~86s of JIT compilation on cold runners.
Modules: aiter_enum (12s), moe_sorting_opus (27s), quant (25s), activation (22s).
"""
import os, sys, stat
    ...
    for name in _MODULES:
        ...
        if not os.path.exists(path):
            ...
            try:
                ...
                print(f"[v145] Downloaded {name}", file=sys.stderr)
            except Exception as e:
                print(f"[v145] Failed {name}: {e}", file=sys.stderr)

try:
    _install_precompiled()
except Exception as e:
    print(f"[v145] Install failed: {e}", file=sys.stderr)
