"""
v113: Test GPU_MAX_HW_QUEUES=1 instead of 2.
MoE pipeline is sequential (sort→quant→stage1→requant→stage2),
so multiple HW queues add dispatch overhead without enabling overlap.
With 1 queue: minimal dispatch overhead, fully serialized execution.
Hypothesis: single queue reduces contention for sequential workload.
