"""
Version 12 but using HIP C++ quantization (dynamic_per_group_scaled_quant_fp4)
instead of fused Triton quant. The C++ kernel might be faster than Triton
for the quantization step. Separate quant + sort = 2 kernels vs 1 fused kernel,
but individual kernels might be faster.
"""
import torch
...
_quant_func = get_quant(QuantType.per_1x32)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Use HIP C++ quant + separate Triton sort for input
    a1, a1_scale = _quant_func(
        hidden_states, scale=None, quant_dtype=dtypes.fp4x2,
    )
    ...
    # Use HIP C++ quant + separate Triton sort for intermediate
    a2_flat = a2.view(-1, inter_dim)
    a2_q, a2_scale = _quant_func(
        a2_flat, scale=None, quant_dtype=dtypes.fp4x2,
    )
    a2_scale = fp4_utils.moe_mxfp4_sort(
        a2_scale[:M * topk, :].view(M, topk, -1),
        sorted_ids, num_valid_ids, M, block_m,
    )
