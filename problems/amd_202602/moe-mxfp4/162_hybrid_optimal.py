"""
v162: Optimal hybrid — pre-allocated sorting + ASM 1-stage for E=33 d<=512 + CK 2-stage for rest.
Downloads all 8 pre-compiled modules (including module_moe_asm).
"""
import os, sys, stat
...
from aiter.fused_moe import (
    fused_moe, fused_moe_2stages, fused_moe_1stage, get_inter_dim, get_padded_M,
    get_2stage_cfgs, MOEMetadata, get_block_size_M,
    cktile_moe_stage1, cktile_moe_stage2,
)

_orig = get_2stage_cfgs.__wrapped__
BLOCK_SIZE_M_1STAGE = 32
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    # Cktile for sparse shapes (same as v103)
    ...
    # ASM 1-stage for E=33 d<=512 shapes (fused gate+up+silu+down in single kernel)
    if inter_dim <= 1024 and q_type == QuantType.per_1x32 and is_shuffled:
        return MOEMetadata(
            functools.partial(fused_moe_1stage, kernelName="",
                activation=activation, quant_type=q_type),
            None, BLOCK_SIZE_M_1STAGE, 0, True)

    # CK FP4 2-stage for dense shapes (d=2048)
    return md
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if metadata.run_1stage:
        # ASM 1-stage: use fused_moe (high-level) which handles dispatch correctly
        ...
        # CK 2-stage: use pre-allocated sorting buffers for lower overhead
