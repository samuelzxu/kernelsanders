"""
v147: Download ALL 6 pre-compiled AITER modules + v103 logic.
Saves ~209s of JIT compilation on cold runners.
Only module_moe_ck2stages (CK FP4 2-stage) still requires JIT (~105s).
"""
import os, sys, stat
    ...
    for name in _MODULES:
        ...
        if not os.path.exists(path):
            ...
            try:
                ...
                print(f"[v147] Downloaded {name}", file=sys.stderr)
            except Exception as e:
                print(f"[v147] Failed {name}: {e}", file=sys.stderr)

try:
    _install_precompiled()
except Exception as e:
    print(f"[v147] Install failed: {e}", file=sys.stderr)
