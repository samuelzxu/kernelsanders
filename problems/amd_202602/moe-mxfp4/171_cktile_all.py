"""
v171: Force cktile 2-stage for ALL shapes (not just sparse).
Key insight: cktile path skips FP4 requant between stages (a2_scale=None).
CK default path requants intermediate to FP4 which adds overhead.
Even if cktile GEMM is slightly slower, skipping requant may compensate.
"""
import os, sys, stat
...
def _patched(token, model_dim, inter_dim, expert, topk,
    ...
    # Sparse shapes: cktile with split_k=2 (very sparse) or split_k=1
    ...
    # NEW: Also force cktile for dense shapes (previously fell through to CK default)
    # This skips the requant between stages
    elif is_shuffled and inter_dim <= 4096:
        use_cktile = True; sk = 1
    ...
    # ASM 1-stage for small E shapes with d_expert <= 512
