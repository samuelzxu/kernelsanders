"""
v124: Test if a minimal load_inline compiles within time budget.
If this works, we can use custom HIP C++ for sorting/quant acceleration.
The load_inline compiles a trivial function to test the build system.
Falls back to v103 approach if compilation fails.
"""
import os
...
# Test minimal load_inline compilation
_compile_ok = False
try:
    t0 = time.time()
    from torch.utils.cpp_extension import load_inline
    _test_mod = load_inline(
        name='test_noop',
        cpp_sources='',
        cuda_sources='''
