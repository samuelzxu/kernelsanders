"""
MXFP4-MM: #161 - KSPLIT=14 for K=7168 (matching aiter's tuned config).

Research finding: aiter's own gfx950 tuned configs for K=7168, N=2112 use
NUM_KSPLIT=14 with BLOCK_K=256 for small M (M<=16, M<=32). This gives:
  - Total K-blocks = 7168/256 = 28
  - 28/14 = 2 iterations per split (perfectly balanced)
  - CTAs = 14 * ceil(16/16) * ceil(2112/64) = 14 * 1 * 33 = 462 (1.8 waves)
  - Much higher GPU utilization than KSPLIT=8 (264 CTAs, 1.03 waves)

num_stages=2 (only 2 K-iterations per split, 3rd stage buffer wasted).
Fused quant+GEMM threshold stays at M<=16 (extending to M<=64 was tested
in #159 and regressed badly: K=2048,m=64 went 19.5µs→54.2µs).
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_ksplit14_o1'
...
_CONFIGS = {
    ...
    # K=7168: KSPLIT=14 matches aiter's tuned gfx950 config
    # 28 K-blocks / 14 = 2 iterations per split (perfectly balanced)
    # 14 * 33 = 462 CTAs = 1.8 waves (vs 264 CTAs with KSPLIT=8)
    "N=2112-K=7168": {
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Fused quant+GEMM for M<=16 with KSPLIT==1 only (K=512 shapes)
