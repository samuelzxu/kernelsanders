"""
MXFP4-MM: #94 - Eliminate reduction kernel for K=1536.
BSM=32 BSN=64 BSK=1024 KSPLIT=1 → 384 tiles with NO reduction.
Same tile count as BSM=64 BSN=64 BSK=512 KSPLIT=2 but saves reduction overhead.
BSK=1024 with K=1536: 2 iterations (1024+512, EVEN_K=False with masking).
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # K=2048: BSN=64 (confirmed improvement)
    "N=7168-K=2048": {
    ...
    # KEY CHANGE: K=1536 with BSK=1024 KSPLIT=1 → no reduction kernel
    # 384 tiles (same as BSM=64 KSPLIT=2) but no 768-block reduction
    "N=3072-K=1536": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "M_LEQ_256": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 1024, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 16, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
}
