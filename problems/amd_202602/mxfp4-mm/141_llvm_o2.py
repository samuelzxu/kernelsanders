"""
MXFP4-MM: #141 - LLVM O2 optimization level instead of O3.
#136 proved copy+patch+redirect works. But injecting flags into the
flags[] parameter didn't affect pass selection.
Try: change OPTIMIZE_O3 → OPTIMIZE_O2. O2 skips aggressive optimizations
like LSR and Machine LICM that increase register pressure.
"""
...
def _patch_to_o2():
    try:
        ...
        # Replace O3 with O2 in the optimize_module call
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_patched_o2'
            os.makedirs(dst_dir, exist_ok=True)
            dst = os.path.join(dst_dir, 'compiler.py')
            patched = content.replace('llvm.OPTIMIZE_O3', 'llvm.OPTIMIZE_O2')
            ...
            print("[PATCH] Changed LLVM opt O3→O2", file=sys.stderr)
