"""
MXFP4-MM: #158 - instruction_sched_variant="default" for AMD scheduling hints.

AMD Triton backend has an underdocumented parameter: instruction_sched_variant.
When set to "default" (vs "none"), Triton inserts AMD-specific scheduling hints
via the amd.passes.ttgpuir.insert_instruction_sched_hints pass. These hints
tell the hardware scheduler how to interleave memory loads and MFMA operations,
potentially improving execution unit utilization.

HIPOptions.instruction_sched_variant values:
  "none"    - no hints inserted (current default)
  "default" - AMD-recommended hints for MFMA kernels
  "llvm"    - use LLVM's own instruction scheduler
  "pre-ra"  - pre-register-allocation scheduling

This is patched into the Triton AMD compiler alongside the existing O1 patch.
The patch changes the default from "none" to "default" for all kernel
compilations in this session.

Based on: triton/third_party/amd/backend/compiler.py HIPOptions dataclass.
"""
...
def _patch_compiler():
    try:
        ...
        with open(fpath, 'r') as f:
            content = f.read()
        dst_dir = '/tmp/triton_amd_sched_o1'
        os.makedirs(dst_dir, exist_ok=True)
        dst = os.path.join(dst_dir, 'compiler.py')
        patched = content
        # O1 optimization
        if 'llvm.OPTIMIZE_O3' in patched:
            patched = patched.replace('llvm.OPTIMIZE_O3', 'llvm.OPTIMIZE_O1')
            print("[PATCH] O3->O1", file=sys.stderr)
        # instruction_sched_variant: change default from "none" to "default"
        if 'instruction_sched_variant: str = "none"' in patched:
            patched = patched.replace(
                'instruction_sched_variant: str = "none"',
                'instruction_sched_variant: str = "default"'
            )
            print("[PATCH] instruction_sched_variant: none->default", file=sys.stderr)
...
# Same configs as #148 (best known) — isolating the instruction_sched_variant effect
_CONFIGS = {
    "N=2880-K=512": {
