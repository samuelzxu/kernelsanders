"""
MXFP4-MM: Use gemm_afp4wfp4_preshuffle with ATOM's exact scale formatting.
ATOM reshapes scales via simple .view() — no permute/contiguous needed.
This eliminates both double-quant AND unshuffle overhead.
"""
import json
import os
import sys
...
# Also inject PRESHUFFLED configs
_PRESHUFFLE_CONFIGS = {
    "N=2880-K=512": {
        "M_LEQ_4": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 1, "num_warps": 2, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": None},
        "M_LEQ_32": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 1, "num_warps": 2, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": None},
        "any": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 1, "num_warps": 2, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": None}
    },
    "N=4096-K=512": {
        "M_LEQ_32": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 1, "num_warps": 2, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": None},
        "any": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 1, "num_warps": 2, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": None}
...
def _inject_configs():
    ...
    for shape_key, config in _CONFIGS.items():
        ...
        with open(fpath, "w") as f:
            json.dump(config, f)
    for shape_key, config in _PRESHUFFLE_CONFIGS.items():
...
# Import preshuffle kernel (same import path as ATOM)
try:
    from aiter.ops.triton.gemm.basic.gemm_afp4wfp4 import gemm_afp4wfp4_preshuffle
    _PRESHUFFLE_AVAILABLE = True
    print("[INFO] gemm_afp4wfp4_preshuffle available", file=sys.stderr)
except ImportError:
    _PRESHUFFLE_AVAILABLE = False
    print("[INFO] gemm_afp4wfp4_preshuffle NOT available", file=sys.stderr)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Try preshuffle path (ATOM's approach — no double quant, no unshuffle)
    if _PRESHUFFLE_AVAILABLE:
        # Format A_scale: shuffle for M>=32, keep unshuffled for M<32
        if m >= MXFP4_QUANT_BLOCK_SIZE:
            A_scale_sh = e8m0_shuffle(A_scale)
            # Reshape shuffled scale to (M//32, K) format
        ...
        # Format B_scale: reshape shuffled scale to (N//32, K) format
        B_scale_fmt = B_scale_sh.view(torch.uint8).view(
            B_scale_sh.shape[0] // MXFP4_QUANT_BLOCK_SIZE, -1
        )

        # Format B_shuffle: already in (N//16, K*16) from shuffle_weight
        B_fmt = B_shuffle.view(torch.uint8)
        if B_fmt.dim() == 2 and B_fmt.shape[0] != n // 16:
            # Reshape if needed
            B_fmt = B_fmt.view(n // 16, -1)
