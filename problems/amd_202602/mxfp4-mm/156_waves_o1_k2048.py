"""
MXFP4-MM: #156 - waves_per_eu=2 for K=2048 shapes with O1 compiler.

With #148's LLVM O1 patch, register pressure is lower than O3. This opens
headroom to try waves_per_eu=2 for the K=2048 shapes (currently 1).

waves_per_eu controls how many wavefronts the scheduler targets per EU.
Higher values allow the hardware to switch between wavefronts to hide
memory latency. With O1's smaller VGPR footprint, two waves can coexist
without conflicting for register file space.

K=2048 is the most memory-bandwidth-intensive shape in the benchmark:
  N=7168, K=2048, M=large: 7168*1024B B_q + 7168*64B B_scale per K-pass
  Total B traffic: ~7.4MB (K=2048 B matrix)

For medium M (M=64-512), these shapes run ~2-4 waves per CU. With
waves_per_eu=2, each EU can hide HBM latency by running a second wavefront
while the first waits for data.

Also tests GROUP_SIZE_M=4 as a middle ground between 2 (current) and 8 (#153).
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_waves_o1'
...
_CONFIGS = {
    ...
    # K=2048: waves_per_eu 1→2 for both configs (more latency hiding with O1 VGPR savings)
    # GROUP_SIZE_M 2→4 (between #148's 2 and #153's 8)
    "N=7168-K=2048": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 4, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 1, "num_warps": 8, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
    "N=3072-K=1536": {
