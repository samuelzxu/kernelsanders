"""
MXFP4-MM: #135 - Runtime patch of HIPBackend.make_llir to disable LSR+LICM.
Previous attempts failed because:
- #128-129,131-132: DISABLE_LLVM_OPT env var doesn't affect AMD backend's specific flags
- #130: Disabling ALL LLVM opts works but is too aggressive (2x slower)
- #133: Monkey-patching C extension failed (wrong import path)
- #134: File patching failed (wrong glob pattern or string match)

New approach: Import the backend class and wrap make_llir to patch the source
file dynamically after locating it through the import system.
"""
import os, shutil, sys, importlib

def _apply_llvm_patch():
    """Find and patch the AMD backend compiler.py via Python import system."""
    try:
        # Import to find the module file
        mod = importlib.import_module('triton.backends.amd.compiler')
        fpath = mod.__file__
        if not fpath:
            print("[PATCH] No __file__ for module", file=sys.stderr)
            return False

        with open(fpath, 'r') as f:
            content = f.read()

        # Try multiple patterns for the optimize_module call
        patterns = [
            # Pattern with exact spacing from GitHub
            ("llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O3, options.arch, '', [], options.enable_fp_fusion)",
             "llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O3, options.arch, '', ['-disable-lsr', '-disable-machine-licm'], options.enable_fp_fusion)"),
            # Pattern with different variable names
            ("optimize_module(llvm_mod, llvm.OPTIMIZE_O3",
             None),  # Just for detection
        ]

        patched = False
        for old, new in patterns:
            if old in content:
                if new is not None and '-disable-lsr' not in content:
                    content = content.replace(old, new)
                    with open(fpath, 'w') as f:
                        f.write(content)
                    patched = True
                    print(f"[PATCH] Patched {fpath} with -disable-lsr -disable-machine-licm", file=sys.stderr)
                else:
                    print(f"[PATCH] Found pattern '{old[:50]}...' in {fpath}", file=sys.stderr)
                    if '-disable-lsr' in content:
                        print(f"[PATCH] Already patched!", file=sys.stderr)
                        patched = True
                break
        else:
            # Dump first occurrence of optimize_module for debugging
            idx = content.find('optimize_module')
            if idx >= 0:
                snippet = content[max(0,idx-20):idx+200]
                print(f"[PATCH] Found optimize_module: ...{snippet[:150]}...", file=sys.stderr)
            else:
                print(f"[PATCH] optimize_module not found in {fpath}", file=sys.stderr)
