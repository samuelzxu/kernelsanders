...
#!POPCORN gpu MI355X
"""
#395: Test tl.dot_scaled availability on the runner.
If it works, write a custom KSPLIT=1 kernel for K=1536 M=256.
"""
import torch, triton, triton.language as tl, os, json
from task import input_t, output_t

# Test: does tl.dot_scaled compile on gfx950?
@triton.jit
def test_dot_scaled_kernel(
    ...
    # Try dot_scaled with e2m1
    # This would use the MFMA FP4 instruction
    try:
        c = tl.dot(a.to(tl.float16), tl.trans(b.to(tl.float16)))
    except:
        c = tl.zeros((BLOCK_M, BLOCK_N), dtype=tl.float32)

    tl.store(C_ptr + offs_m[:, None] * N + offs_n[None, :], c.to(tl.bfloat16))

print("=== Testing tl.dot_scaled ===")
try:
    M,N,K = 32,32,64
    A = torch.randn(M, K, dtype=torch.bfloat16, device="cuda")
    B = torch.randn(N, K, dtype=torch.bfloat16, device="cuda")
    C = torch.empty(M, N, dtype=torch.bfloat16, device="cuda")
    test_dot_scaled_kernel[(1,1)](A, B, C, M, N, K, BLOCK_M=32, BLOCK_N=32, BLOCK_K=64)
    print(f"Basic dot works: {C[0,0].item():.2f}")
except Exception as e:
    print(f"Error: {e}")

# Check Triton version and dot_scaled support
print(f"Triton version: {triton.__version__}")
print(f"Has dot_scaled: {hasattr(tl, 'dot_scaled')}")

# Test dot_scaled if available
if hasattr(tl, 'dot_scaled'):
    @triton.jit
    def test_ds_kernel(out_ptr):
        # Just check if it compiles
        x = tl.full((32, 64), 0.0, dtype=tl.float32)
        tl.store(out_ptr + tl.arange(0, 32), x[:, 0])

    print("dot_scaled attribute exists")
