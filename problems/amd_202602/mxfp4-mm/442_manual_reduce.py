...
#!POPCORN gpu MI355X
"""
#442: skip_reduce=True + manual torch.sum reduce for KSPLIT>1 shapes.
The Triton reduce kernel has launch overhead. A PyTorch .sum() might be faster.
"""
...
for _m,_n,_k in [(4,2880,512),(16,2112,7168),(32,4096,512),(32,2880,512),(64,7168,2048),(256,3072,1536)]:
    try:
        ...
        # Also warmup skip_reduce path for KSPLIT>1 shapes
        if _k > 512:
            gemm_a16wfp4_preshuffle(_A,_Bw,_Bws,prequant=True,dtype=torch.bfloat16,skip_reduce=True)
    except:pass
torch.cuda.empty_cache()

# Shapes that use KSPLIT>1
_splitk_shapes = {(16,2112,7168), (64,7168,2048), (256,3072,1536)}
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if (m,n,k) in _splitk_shapes:
        # skip_reduce + manual sum for KSPLIT>1 shapes
        raw = gemm_a16wfp4_preshuffle(A,_cw,_cs,prequant=True,dtype=torch.bfloat16,skip_reduce=True)
        return raw.sum(dim=0).to(torch.bfloat16)
