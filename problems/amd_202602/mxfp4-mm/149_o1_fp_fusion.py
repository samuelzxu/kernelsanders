"""
MXFP4-MM: #149 - O1 + enable_fp_fusion=True.
#148 showed O1 produces better code than O3 (less register pressure).
Now combine with fp_fusion=True: FMA fusion at O1 might help since
O1 doesn't do loop transformations that interfere with FMA scheduling.
"""
import os, shutil, sys, importlib, importlib.util

def _patch_o1_fusion():
    try:
        ...
        new = "llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O1, options.arch, '', [], True)"
        if old in content:
            dst_dir = '/tmp/triton_amd_o1_fusion'
            ...
            print("[PATCH] O1 + fp_fusion=True", file=sys.stderr)
