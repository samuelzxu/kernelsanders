"""
MXFP4-MM: Full caching - B_scale + pre-allocated A quant buffers.
Eliminates per-call tensor allocation and B recomputation.
"""
import json
import os
import triton
...
def _quant_a_preallocated(x):
    ...
    x_fp4, blockscale_e8m0 = _a_buf_cache[key]

    # Same config logic as dynamic_mxfp4_quant
    if N <= 1024:
        NUM_ITER = 1
        NUM_STAGES = 1
        NUM_WARPS = 4
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Quantize A with pre-allocated buffers (avoids allocation per call)
    A_q, A_scale = _quant_a_preallocated(A)

    # B is constant - cache B_scale
    cache_key = (n, k)
    if cache_key not in _b_scale_cache:
        if k <= 512:
            _, B_scale = dynamic_mxfp4_quant(B)
        else:
            B_scale = e8m0_unshuffle(B_scale_sh.view(torch.uint8), n, k // 32)
        _b_scale_cache[cache_key] = B_scale

    B_scale = _b_scale_cache[cache_key]
