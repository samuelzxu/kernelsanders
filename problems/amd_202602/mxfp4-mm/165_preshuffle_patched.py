"""
MXFP4-MM: #165 - Preshuffle Triton kernel with b-undefined bug fixed.

The aiter preshuffle kernel (_gemm_afp4wfp4_preshuffle_kernel) has a JIT bug:
`b = tl.load(...)` is inside `if EVEN_K:` but `b.reshape(...)` is outside.
If EVEN_K is False (never for our shapes, but Triton JIT may still complain),
`b` is undefined.

Fix: add an `else` branch with masked loads.

Use preshuffle path for ALL non-fused shapes. This uses B_shuffle (preshuffled
weights from task) and B_scale_sh (already shuffled scales) directly, avoiding:
  1. e8m0_unshuffle on B_scale (saves ~0.5µs per call)
  2. Better memory coalescing from preshuffled weight layout

For M<=16, KSPLIT=1: still use fused kernel (saves quant dispatch).
For other shapes: use patched preshuffle Triton GEMM.
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_preshuffle_o1'
...
# Inject configs for BOTH regular and preshuffle paths
_CONFIGS = {
    "N=2880-K=512": {
...
from aiter.ops.triton.gemm.basic.gemm_afp4wfp4 import gemm_afp4wfp4, gemm_afp4wfp4_preshuffle
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # M<=16, KSPLIT=1: fused kernel (saves quant dispatch)
    ...
    # For other shapes: try preshuffle path (uses B_shuffle directly)
    A_q, A_scale = dynamic_mxfp4_quant(A)

    # Try preshuffle: B_shuffle has shape (N, K//2) in fp4x2, need (N//16, K*8) as uint8
    # B_scale_sh is already shuffled, view as (N//32, K) for preshuffle kernel
    try:
        B_sh_u8 = B_shuffle.view(torch.uint8)
        # Reshape: (N, K//2) -> (N//16, K//2 * 16) = (N//16, K*8)
        B_w = B_sh_u8.reshape(n // 16, k * 8)
        B_scale_ps = B_scale_sh.view(torch.uint8).reshape(n // 32, -1)

        # A_scale for preshuffle: M>=32 needs shuffled, M<32 keeps standard
        if m >= 32:
            A_scale_ps = e8m0_shuffle(A_scale)
        else:
            A_scale_ps = A_scale

        return gemm_afp4wfp4_preshuffle(
            A_q.view(torch.uint8), B_w, A_scale_ps, B_scale_ps,
            dtype=torch.bfloat16,
        )
    except Exception as e:
        print(f"[PRESHUFFLE] Error: {e}, falling back to regular", file=__import__('sys').stderr)
