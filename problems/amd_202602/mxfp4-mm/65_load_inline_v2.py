"""
MXFP4-MM: load_inline C++ extension that calls aiter's gemm_a4w4_asm.
No direct stream references — uses default stream like vectoradd example.
"""
import json
import os
import sys
...
# Try to build a minimal C++ extension (like vectoradd example)
# This just provides a dummy function - the real work is done by aiter
_HIP_EXT = None
try:
    ...
    """
    _HIP_EXT = load_inline(
        name='mxfp4_helper',
        cpp_sources=_dummy_cpp,
        cuda_sources=_dummy_source,
        functions=['noop'],
        verbose=False,
    )
    print("[INFO] load_inline succeeded", file=sys.stderr)
