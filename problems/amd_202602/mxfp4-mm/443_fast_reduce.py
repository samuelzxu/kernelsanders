...
#!POPCORN gpu MI355X
"""
#443: skip_reduce + fast manual reduce using in-place add for KSPLIT=2.
raw[0].add_(raw[1]) is a single fused kernel. Then .to(bf16) casts.
For KSPLIT=2: only 1 add + 1 cast vs Triton reduce kernel.
For KSPLIT=7 (K=7168): raw[0].add_(raw[1]).add_(raw[2])...add_(raw[6])
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if (m,n,k) in _splitk_shapes:
        ...
        # Fast reduce: in-place add then cast
        acc = raw[0]
