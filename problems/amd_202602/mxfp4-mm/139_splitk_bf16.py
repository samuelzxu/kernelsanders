"""
MXFP4-MM: #139 - Enable bf16 split-K partials.
Inspired by leaderboard 'globals_patch' approach.
Setting _USE_GEMM_SPLITK_BF16=True in gemm_afp4wfp4:
- Uses bf16 partials instead of fp32 for KSPLIT>1 reduction
- Halves memory for partial results (bf16 vs fp32)
- Reduction kernel uses REDUCE_BLOCK_SIZE_N=128 (vs 64)
- Faster reduction but minimal precision trade-off
Affects: M=16/K=7168 (KSPLIT=7), M=256/K=1536 (KSPLIT→1 after get_splitk)
"""
import json
import os

# Patch BEFORE importing gemm_afp4wfp4
import aiter.ops.triton.gemm.basic.gemm_afp4wfp4 as _gemm_mod
_gemm_mod._USE_GEMM_SPLITK_BF16 = True
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m <= 16 and config.get("NUM_KSPLIT", 1) == 1:
        ...
        # Pre-allocate y to work around _USE_GEMM_SPLITK_BF16 bug (y.dtype when y=None)
        out_key = (m, n)
        if out_key not in _out_cache:
            _out_cache[out_key] = torch.empty((m, n), dtype=torch.bfloat16, device=A.device)
        y = _out_cache[out_key]
