...
#!POPCORN gpu MI355X
"""
#318: Try BSN=192 KSPLIT=1 for K=1536 M=256.
BSN=256 KSPLIT=1: 96 blocks = 44.3µs (too few blocks)
BSN=64 KSPLIT=1: 384 blocks = 22.4µs (poor quant amortization)
BSN=128 KSPLIT=1: 192 blocks (0.63 waves) — untested middle ground
BSN=192 KSPLIT=1: 128 blocks — better quant amort than 128
Also try: BSN=256 KSPLIT=2 BSK=256 — but K_packed/2=384, 384/256=1.5 NOT clean

Actually let me check: does the preshuffle kernel support KSPLIT=2 with non-even split?
The get_splitk function adjusts KSPLIT to ensure divisibility.
Let me try KSPLIT=2 and let get_splitk handle the adjustment.
"""
...
_cfgs = {
    ...
    # K=1536: try KSPLIT=2 BSN=256 — let get_splitk adjust
    # K_packed=768, KSPLIT=2: 384, BSK=256: 1.5 iters — get_splitk will adjust
    "N=3072-K=1536": {
        ...
        "M_LEQ_256": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 2, "num_warps": 8, "num_stages": 2, "waves_per_eu": 4, "matrix_instr_nonkdim": 16, "cache_modifier": None},
        "any": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 1, "num_warps": 8, "num_stages": 2, "waves_per_eu": 4, "matrix_instr_nonkdim": 16, "cache_modifier": None}
    },
}
