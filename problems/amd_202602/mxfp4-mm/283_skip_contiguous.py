...
#!POPCORN gpu MI355X
"""
#283: Skip the B_scale_sh[:n,:].contiguous() call.
The preshuffle kernel does bounds checking internally.
Pass the PADDED B_scale_sh directly, reshaped without contiguous copy.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if dp != _ck:
        ...
        # Skip [:n,:].contiguous() - use the padded tensor directly
        # B_scale_sh shape is (N_padded, K//32) where N_padded >= N and is aligned to 32
        # Reshape to (N_padded//32, K) - this includes padding rows but the kernel bounds-checks
        bss = B_scale_sh.view(torch.uint8)
        N_padded = bss.shape[0]
        _cs = bss.reshape(N_padded // 32, k)
