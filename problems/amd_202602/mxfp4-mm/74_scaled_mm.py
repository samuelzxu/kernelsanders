"""
MXFP4-MM: Use torch._scaled_mm for FP4 GEMM.
This routes to hipBLASLt on ROCm — auto-tuned, zero Triton overhead.
"""
import json
import os
import sys
...
# Test if torch._scaled_mm supports FP4
_scaled_mm_ok = False
try:
    _test_a = torch.zeros(32, 16, dtype=torch.float4_e2m1fn_x2, device="cuda")
    _test_b = torch.zeros(32, 16, dtype=torch.float4_e2m1fn_x2, device="cuda")
    _test_sa = torch.ones(32, 1, dtype=torch.float8_e8m0fnu, device="cuda")
    _test_sb = torch.ones(32, 1, dtype=torch.float8_e8m0fnu, device="cuda")
    _test_c = torch._scaled_mm(_test_a, _test_b.T, scale_a=_test_sa, scale_b=_test_sb,
                                out_dtype=torch.bfloat16)
    _scaled_mm_ok = True
    print("[INFO] torch._scaled_mm FP4 works!", file=sys.stderr)
    del _test_a, _test_b, _test_sa, _test_sb, _test_c
except Exception as e:
    print(f"[INFO] torch._scaled_mm FP4 failed: {e}", file=sys.stderr)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Try torch._scaled_mm path for all shapes
    if _scaled_mm_ok:
        A_q, A_scale = dynamic_mxfp4_quant(A)
        # _scaled_mm expects: (M, K) @ (K, N) with per-row or per-block scales
        # A_q is (M, K//2) fp4x2, B_q is (N, K//2) fp4x2
        # We need A_q @ B_q.T with scales
        try:
            result = torch._scaled_mm(
                A_q.view(torch.float4_e2m1fn_x2),
                B_q.view(torch.float4_e2m1fn_x2).T,
                scale_a=A_scale.view(torch.float8_e8m0fnu),
                scale_b=e8m0_unshuffle(B_scale_sh, n, k // 32).view(torch.float8_e8m0fnu) if k > 512 else dynamic_mxfp4_quant(B)[1].view(torch.float8_e8m0fnu),
                out_dtype=torch.bfloat16,
            )
            return result
        except Exception as e:
            print(f"[WARN] _scaled_mm failed: {e}", file=sys.stderr)

    # Fallback: Triton path (same as #53)
    A_q, A_scale = dynamic_mxfp4_quant(A)
