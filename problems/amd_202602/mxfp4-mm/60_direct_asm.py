"""
MXFP4-MM: Call gemm_a4w4_asm DIRECTLY with explicit kernel name.
Bypasses the CK config lookup (which fails due to cu_num mismatch).
Uses the 32x128 ASM kernel for all shapes.
"""
import json
import os
import triton
...
# Still inject Triton configs as fallback
_CONFIGS = {
    "N=2880-K=512": {
...
# Import the low-level ASM kernel directly
from aiter.jit.core import compile_ops
compile_ops("module_gemm_a4w4_asm")  # Ensure the module is compiled
...
# ASM kernel name (32x128 tile, available on the runner)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Quantize A with shuffle for CK/ASM path
    A_q, A_scale = dynamic_mxfp4_quant(A)
    A_scale_sh = e8m0_shuffle(A_scale)

    # Pre-allocate padded output (CK requires M padded to 32)
    padded_m = ((m + 31) // 32) * 32
    out = torch.empty((padded_m, n), dtype=torch.bfloat16, device=A.device)

    # Call ASM kernel directly with explicit kernel name
    try:
        gemm_a4w4_asm(
            A_q.view(dtypes.fp4x2).view(m, k // 2),
            B_shuffle,
