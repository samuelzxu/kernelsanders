"""
MXFP4-MM: #97 - Eliminate double-quant for K=512.
Currently for K<=512 we call dynamic_mxfp4_quant(B) just to get B_scale,
which re-quantizes B unnecessarily (we already have B_q from input).
Instead, use e8m0_unshuffle(B_scale_sh) for ALL K values.
This saves the quant kernel launch (~2-3µs) for K=512 ranked benchmark.
Based on #92 configs (BSN=64 for K=2048, etc).
"""
import json
import os
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Get B_scale using e8m0_unshuffle for ALL K values (no double-quant!)
    # This avoids the expensive dynamic_mxfp4_quant(B) call for K=512
