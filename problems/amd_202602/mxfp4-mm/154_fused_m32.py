"""
MXFP4-MM: #154 - Extend fused quant+GEMM kernel to M<=32.

The fused quant+GEMM kernel avoids:
1. A separate dynamic_mxfp4_quant(A) kernel dispatch (~1-2µs overhead)
2. Writing A_q to HBM and reading it back in the GEMM kernel

Currently limited to M<=16. Extending to M<=32 captures additional savings
for M=32 with K=512 shapes (N=2880, N=4096), which have:
  - BLOCK_K=512 = full K in one loop iteration (no extra quant overhead)
  - NUM_KSPLIT=1 (no split-K, no reduction step)

For M=32 with K=2048/K=1536/K=7168 (which have KSPLIT>=1), the existing
path (separate quant + GEMM) is kept since the multi-iteration K loop
adds register pressure that outweighs the launch saving.

The M_LEQ_32 tile configs already exist in the injected JSON for N=2880 and
N=4096 shapes, so no new tile config is needed.
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_fused_m32_o1'
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Extended from m<=16 to m<=32: saves a dynamic_mxfp4_quant kernel dispatch
    # for M=32 shapes. Guard: NUM_KSPLIT==1 ensures no split-K reduction step.
    if m <= 32 and config.get("NUM_KSPLIT", 1) == 1:
        out_key = (m, n)
