"""
MXFP4-MM: Compute B_scale using PyTorch ops instead of dynamic_mxfp4_quant.
Avoids Triton quant kernel launch for B (we only need the scale, not fp4 data).
"""
import json
import os
import torch
...
def compute_b_scale_torch(B):
    """Compute E8M0 block scale using PyTorch ops (no Triton kernel needed)."""
    M, N = B.shape
    SG = 32
    # Reshape to blocks of 32
    x = B.float().view(M, N // SG, SG)
    # Compute amax per block
    amax = x.abs().amax(dim=-1)
    # Round amax up to nearest power of 2 (via bit manipulation)
    amax_bits = amax.view(torch.int32)
    amax_bits = ((amax_bits + 0x200000).to(torch.int32) & 0x7F800000).view(torch.float32)
    # Compute E8M0 scale: log2(amax) - 2 + 127
    scale = amax_bits.log2().floor() - 2
    scale = scale.clamp(-127, 127).to(torch.uint8) + 127
    return scale
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if key == _cache_key:
        ...
        if k <= 512:
            # PyTorch-based scale computation (no Triton kernel launch)
            B_scale = compute_b_scale_torch(B)
        else:
