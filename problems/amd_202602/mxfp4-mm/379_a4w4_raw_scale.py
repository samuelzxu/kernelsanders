...
#!POPCORN gpu MI355X
"""
#379: Test gemm_a4w4 float4 with RAW A_scale (not shuffled).
If it works, we skip e8m0_shuffle entirely!
Also: time gemm_a4w4 with raw vs shuffled to see GEMM-only time.
"""
...
A_s_raw = A_s.contiguous()  # raw, not shuffled
A_s_sh = e8m0_sh(A_s_raw)  # shuffled
...
B_s = torch.zeros(N,K//32,dtype=torch.uint8,device="cuda")

# Reference: preshuffle result for comparison
...
# Test with shuffled A_scale (known to work)
r_sh = gemm_a4w4(A_q_f4, B_q_f4, A_s_sh, B_s, bpreshuffle=True)
print(f"Shuffled result: {r_sh.shape}, sum={r_sh.sum().item():.2f}")

# Test with RAW A_scale
r_raw = gemm_a4w4(A_q_f4, B_q_f4, A_s_raw, B_s, bpreshuffle=True)
print(f"Raw result: {r_raw.shape}, sum={r_raw.sum().item():.2f}")

# Compare
diff = (r_sh - r_raw).abs().max().item()
print(f"Max diff shuffled vs raw: {diff}")

# Time GEMM with raw scale (avoid shuffle overhead)
...
# Warmup
for _ in range(5):
    _ = gemm_a4w4(A_q_f4, B_q_f4, A_s_raw, B_s, bpreshuffle=True)
torch.cuda.synchronize()
...
torch.cuda.synchronize()
print(f"GEMM with raw A_scale: {start.elapsed_time(end)/100*1000:.1f}µs")

# Cleanup
del A, A_q, A_s, A_q_f4, B_q, B_q_f4, B_s, A_s_raw, A_s_sh
torch.cuda.empty_cache()

# All shapes use preshuffle for now
