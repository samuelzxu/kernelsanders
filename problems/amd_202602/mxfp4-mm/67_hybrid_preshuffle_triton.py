"""
MXFP4-MM: Hybrid preshuffle (M<=16) + Triton (M>=32).
- M<=16: gemm_afp4wfp4_preshuffle — no double quant, no unshuffle
- M>=32, K<=512: Triton with double-quant + injected BSK=512 configs
- M>=32, K>512: Triton with e8m0_unshuffle
"""
import json
import os
import sys
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m <= 16:
        # PRESHUFFLE PATH: no double quant, no unshuffle
        # Format A_scale for preshuffle kernel
        ...
        # B_shuffle is already in preshuffle format
