"""
MXFP4-MM: Hybrid a16wfp4 (K<=512) + Triton (K>512).
gemm_a16wfp4 was slower overall but might be faster for K=512 shapes
where the on-the-fly quant loop is short (1-2 iterations).
For large K, use our proven Triton path.
"""
import json
import os
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if k <= 512:
        # K=512: use gemm_a16wfp4 (fused bf16->fp4 quant + GEMM)
        # No need for dynamic_mxfp4_quant(A) or double quant of B!
        ...
        return gemm_a16wfp4(A, B_q_uint8, B_scale, True, torch.bfloat16)
    else:
        # K>512: use proven Triton path
        key = (B.data_ptr(), B_q.data_ptr(), B_scale_sh.data_ptr())
        if key == _cache_key:
            B_scale = _cache_val
        else:
