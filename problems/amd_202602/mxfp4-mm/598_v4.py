...
#!POPCORN gpu MI355X
"""#598b: v3 kernel — A bf16 to LDS (cooperative), quant in register, no A write-back."""
        ...
        // Each thread loads 4 × uint4 (64 bytes)
        {
            ...
            for (int r = 0; r < bytes_per_thread; r += 16) {
                ...
                if (abs_row < M) {
                    ...
                    // Write each 16-byte chunk with independent swizzle
                    for (int chunk = 0; chunk < bytes_per_thread / 16; chunk++) {
                        int lds_idx = swz_a(a_r, a_c + chunk * 16);
                        *(uint4*)&lds_a[lds_idx] = *(const uint4*)(src + chunk * 16);
                    }
                } else {
                    for (int chunk = 0; chunk < bytes_per_thread / 16; chunk++) {
                        int lds_idx = swz_a(a_r, a_c + chunk * 16);
        ...
        for (int sub = 0; sub < 4; sub++) {
            ...
            float vals[32];
            {
                ...
                for (int v = 0; v < 4; v++) {
                    ...
                    uint4 c = *(uint4*)&lds_a[swz_a(l32, byte_off + v*16)];
...
_v3 = None
try:
    _v3 = load_inline(name='v4_598b',cpp_sources=_hip_cpp,cuda_sources=_hip_src,
