...
#!POPCORN gpu MI355X
"""
MXFP4-MM #236: Try BSN=512 and waves_per_eu=8 for K=1536 M=256.
Both are UNTESTED configs. BSN=512 gives larger tiles (more data reuse).
Grid: 8 x 6 x 3 = 144 blocks (smaller but bigger tiles).
"""
...
_CONFIGS = {
    ...
    # K=1536 M=256: NEW - try BSN=512 + waves_per_eu=8
    # BSN=512: 3072/512=6 N-tiles. Grid: 8 x 6 x 3 = 144 blocks
    # Also try KSPLIT=1 with BSN=512: 8 x 6 = 48 blocks (under-subscribed but no atomics)
    "N=3072-K=1536": {
        ...
        # Key experiment: BSN=512 + waves_per_eu=8 + KSPLIT=3
        "M_LEQ_256": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 512, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 3, "num_warps": 8, "num_stages": 2, "waves_per_eu": 8, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
