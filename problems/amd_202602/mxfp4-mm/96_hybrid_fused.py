"""
MXFP4-MM: #96 - Hybrid: old fused kernel for KSPLIT=1, new fused+splitK+xcd for KSPLIT>1.
#95 showed M=16 K=7168 improved 1.5µs with fused+splitK+xcd but M=4 regressed
(extra JIT overhead from new kernel specializations).
Solution: use SEPARATE kernels to avoid extra compilations on the fast path.
"""
import json
import os
import triton
...
# Kernel A: Original fused kernel for KSPLIT=1 (no split-K, no remap_xcd overhead)
@triton.jit
...
# Kernel B: Fused kernel with split-K + remap_xcd (for KSPLIT>1)
@triton.jit
def _fused_quant_gemm_splitk(
    a_bf16_ptr, b_ptr, c_ptr, b_scales_ptr,
    M, N, K,
    stride_am, stride_ak, stride_bk, stride_bn,
...
def _reduce_splitk_kernel(
    ...
    for k in range(NUM_KSPLIT):
        partial = tl.load(partial_ptr + k * M * N + offs_m[:, None] * N + offs_n[None, :], mask=mask, other=0.0)
        acc += partial
    tl.store(out_ptr + offs_m[:, None] * N + offs_n[None, :], acc.to(tl.bfloat16), mask=mask)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # For M<=16 with KSPLIT=1: use simple fused kernel (no remap_xcd overhead)
    ...
    # For M<=16 with KSPLIT>1: fused kernel with split-K + remap_xcd
    if m <= 16 and ksplit > 1:
        B_q_t = B_q_uint8.T
        ...
        _reduce_splitk_kernel[reduce_grid](partial, y, m, n, NUM_KSPLIT=ksplit, BLOCK_SIZE_M=16, BLOCK_SIZE_N=64, num_warps=4)
        return y
