"""
MXFP4-MM: #142 - Toggle enable_fp_fusion in LLVM optimizer.
The copy+redirect mechanism works (#136, #141 confirmed). Try:
1. Force enable_fp_fusion=True (fuse multiply-add → better MFMA scheduling)
2. Also try the features string parameter for pass control
"""
import os, shutil, sys, importlib, importlib.util

def _patch_fp_fusion():
    try:
        ...
        dst_dir = '/tmp/triton_amd_fp_fusion'
        os.makedirs(dst_dir, exist_ok=True)
        dst = os.path.join(dst_dir, 'compiler.py')

        # Force fp_fusion=True and try passing features for pass control
        old = "llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O3, options.arch, '', [], options.enable_fp_fusion)"
        new = "llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O3, options.arch, '', [], True)"
        if old in content:
            ...
            print("[PATCH] Force fp_fusion=True", file=sys.stderr)
            ...
            if idx >= 0:
                print(f"[PATCH] Found: {content[max(0,idx-10):idx+200]}", file=sys.stderr)
        return False
    except Exception as e:
        print(f"[PATCH] Error: {e}", file=sys.stderr)
        return False

_patched = _patch_fp_fusion()
