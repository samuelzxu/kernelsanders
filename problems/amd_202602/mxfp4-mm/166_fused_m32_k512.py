"""
MXFP4-MM: #166 - Fuse A quant into GEMM for M<=32 K<=512 with JIT prewarm.

#159 showed fused kernel is faster in raw benchmarks for K=512 M=32:
  Raw: 9.68µs (vs 9.99µs baseline) — 0.31µs faster
  Ranked: 15.6µs (vs 12.6µs baseline) — 3µs SLOWER

The ranked regression was JIT compilation overhead (~3-6µs per new kernel variant).
Fix: prewarm the fused kernel during module initialization so JIT compile happens
before the benchmark starts.

Guard: only fuse when KSPLIT==1 AND K<=512 (1 K-iteration). K=2048 with 4
iterations of inline quant was catastrophic in #159 (3.3x slower).
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_fused_m32_k512_o1'
...
def _prewarm_fused_kernel():
    """Trigger JIT compilation for all fused kernel variants before benchmark."""
    global _warmed
    if _warmed:
        return
    _warmed = True
    # Prewarm with each unique (BLOCK_SIZE_M, BLOCK_SIZE_N, BLOCK_SIZE_K) combo
    # that the fused path will use: M<=4 (BSM=16) and M<=32 (BSM=32)
    configs_to_warm = [
        # M<=4 shapes: BSM=16, BSN=64, BSK=512 (N=2880)
        (4, 2880, 512, {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512,
                        "GROUP_SIZE_M": 1, "num_warps": 4, "num_stages": 2,
                        "waves_per_eu": 3, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"}),
        # M<=32 shapes: BSM=32, BSN=64, BSK=512 (N=2880, N=4096)
        (32, 2880, 512, {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512,
                         "GROUP_SIZE_M": 1, "num_warps": 4, "num_stages": 2,
                         "waves_per_eu": 3, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"}),
    ]
    for m_w, n_w, k_w, cfg in configs_to_warm:
        try:
            A_w = torch.randn(m_w, k_w, dtype=torch.bfloat16, device="cuda")
            # Need B_q and B_scale for warmup — create dummy
            _, B_scale_w = dynamic_mxfp4_quant(torch.randn(n_w, k_w, dtype=torch.bfloat16, device="cuda"))
            B_q_w = torch.zeros(n_w, k_w // 2, dtype=torch.uint8, device="cuda")
            B_q_t_w = B_q_w.T
            y_w = torch.empty(m_w, n_w, dtype=torch.bfloat16, device="cuda")
            bsm = cfg["BLOCK_SIZE_M"]
            bsn = cfg["BLOCK_SIZE_N"]
            grid_mn = triton.cdiv(m_w, bsm) * triton.cdiv(n_w, bsn)
            _fused_quant_gemm_small_m[(grid_mn,)](
                A_w, B_q_t_w, y_w, B_scale_w, m_w, n_w, k_w,
                A_w.stride(0), A_w.stride(1), B_q_t_w.stride(0), B_q_t_w.stride(1),
                y_w.stride(0), y_w.stride(1), B_scale_w.stride(0), B_scale_w.stride(1),
                grid_mn, **cfg,
            )
            torch.cuda.synchronize()
        except Exception as e:
            print(f"[PREWARM] Error warming {m_w}x{n_w}x{k_w}: {e}", file=sys.stderr)
    print("[PREWARM] Fused kernel variants compiled", file=sys.stderr)


@torch.inference_mode()
def custom_kernel(data: input_t) -> output_t:
    global _cache_key, _cache_val
    _prewarm_fused_kernel()
    ...
    # Fuse for M<=32, K<=512, KSPLIT=1 only.
    # K<=512 guard ensures only 1 K-iteration (inline quant overhead minimal).
    # K=2048 with 4 iterations was catastrophic in #159 (3.3x slower).
    if m <= 32 and k <= 512 and config.get("NUM_KSPLIT", 1) == 1:
        out_key = (m, n)
        ...
        bsm = fused_config["BLOCK_SIZE_M"]
        bsn = fused_config["BLOCK_SIZE_N"]
