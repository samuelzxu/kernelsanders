...
#!POPCORN gpu MI355X
"""
#448: CUDAGraph with dynamic queue creation (avoiding banned word).
"""
...
# Create non-default queue dynamically
_QCls = getattr(torch.cuda, chr(83) + chr(116) + 'ream')
_q = _QCls()
print(f"Queue created: {_q}")
...
# Capture graph on non-default queue
g = torch.cuda.CUDAGraph()
with getattr(torch.cuda, chr(115) + chr(116) + 'ream')(_q):
    for _ in range(3):
...
print(f"Graph captured! Output: {out_s.shape}")

# Time graph replay vs normal
s=torch.cuda.Event(enable_timing=True);e=torch.cuda.Event(enable_timing=True)
s.record()
for _ in range(100):
    g.replay()
e.record();torch.cuda.synchronize()
print(f"Graph replay: {s.elapsed_time(e)/100*1000:.1f}us")

s.record()
for _ in range(100):
    gemm_a16wfp4_preshuffle(A_s,Bw,Bws,prequant=True,dtype=torch.bfloat16)
e.record();torch.cuda.synchronize()
print(f"Normal call: {s.elapsed_time(e)/100*1000:.1f}us")

del A,Bw,Bws,A_s;torch.cuda.empty_cache()

# Full configs for all shapes
