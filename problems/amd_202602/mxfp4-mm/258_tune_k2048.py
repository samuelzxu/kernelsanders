...
#!POPCORN gpu MI355X
"""
#258: Tune K=2048 M=64 shape. Current: 13.5µs with mfma32 BSM=16 BSN=256 KSPLIT=2.
Try: mfma16 with different tiles. Also try KSPLIT=4 for more parallelism.
K_packed=1024, KSPLIT=4: 256 per split, BSK=256 → 1 iter per block.
Grid: ceil(64/16)×ceil(7168/128)×4 = 4×56×4 = 896 blocks. Good occupancy.
"""
...
_CONFIGS = {
    ...
    # K=2048: NEW configs to try
    # Current best: BSM=16, BSN=256, BSK=512, KSPLIT=2, mfma32 → 13.5µs
    # Try: KSPLIT=4 with BSN=128 mfma16 for more parallelism
    # K_packed=1024, KSPLIT=4: 256 per split
    "N=7168-K=2048": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 4, "num_warps": 8, "num_stages": 2, "waves_per_eu": 4, "matrix_instr_nonkdim": 32, "cache_modifier": ".cg"},
