...
#!POPCORN gpu MI355X
"""
#436: Patch kernel to add cache_modifier to B scale loads.
Line 140: b_scales = tl.load(b_scale_ptrs)
...
# First restore tl.assume if missing
# The assume was in the preshuffle kernel. Let me just reload the module.
# Actually, the kernel file was already modified by #435. Let me check.

# Patch: add cache_modifier to b_scales load
# Find the exact line in the preshuffle kernel's inner loop
patched = kernel.replace(
...
if patched != kernel:
    print("Patched: added cache_modifier to b_scales load")
else:
    # Check what the actual line looks like
    for i, line in enumerate(kernel.split('\n')):
        if 'b_scale' in line and 'load' in line:
            print(f"  Line {i+1}: {line.strip()}")
    print("Trying alternate patch...")
    # Maybe the load is multi-line
    patched = kernel.replace(
        'tl.load(b_scale_ptrs, cache_modifier=cache_modifier)',
        'tl.load(b_scale_ptrs, cache_modifier=cache_modifier)'
    )
