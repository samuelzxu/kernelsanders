"""
MXFP4-MM: #93 - Use gemm_a16wfp4 for shapes with KSPLIT > 1.
For M=16/K=7168 (KSPLIT=8), the current path is: quant(A) + gemm_afp4wfp4.
Using gemm_a16wfp4 fuses the quant into the GEMM kernel, saving a kernel launch.
Same for M=64/K=2048 if KSPLIT>1, and M=256/K=1536 with KSPLIT=2.

Combined with #92's best config (BSN=64 for K=2048).
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # K=7168: Keep KSPLIT=8 (264 tiles)
    "N=2112-K=7168": {
    ...
    # K=1536: Original BSM=64 KSPLIT=2
    "N=3072-K=1536": {
...
# Also inject A16WFP4 configs for the fused quant+GEMM path
# These use same K values as AFP4WFP4 (the 2*K in code converts packed-K to actual-K)
_CONFIGS_A16 = {
    "N=2112-K=7168": {
        "M_LEQ_16": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 8, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 16, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # For M<=16 with no split-K: our custom fused quant+GEMM kernel
    ...
    # For shapes with KSPLIT > 1: use gemm_a16wfp4 (fused quant+GEMM)
    # This saves a separate dynamic_mxfp4_quant(A) kernel launch
    if ksplit > 1:
        return gemm_a16wfp4(A, B_q_uint8, B_scale, dtype=torch.bfloat16)

    # For KSPLIT=1 with M>16: separate quant + GEMM
