...
#!POPCORN gpu MI355X
"""
#441: Test skip_reduce=True for KSPLIT>1 shapes.
If the kernel uses atomic_add for split-K, skipping the reduce kernel
saves 1 kernel launch + f32 intermediate buffer.
"""
...
# Test: call with skip_reduce=True for K=1536 M=256
M,N,K=256,3072,1536
...
print(f"ref: {ref.shape}, dtype={ref.dtype}")

# Now try skip_reduce
try:
    out_sr=gemm_a16wfp4_preshuffle(A,Bw,Bws,prequant=True,dtype=torch.bfloat16,skip_reduce=True)
    print(f"skip_reduce: {out_sr.shape}, dtype={out_sr.dtype}")
    diff=(ref-out_sr[:M,:N] if out_sr.shape[0]>=M else ref).abs().max()
    print(f"diff: {diff.item()}")
except Exception as e:
    print(f"skip_reduce error: {e}")
