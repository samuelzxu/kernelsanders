"""
MXFP4-MM: #143 - Hybrid CK ASM + Triton.
CK ASM CSV has tuned configs for M=64/K=2048 (6.8µs) and M=256/K=1536 (6.2µs).
Use CK ASM for these shapes, Triton for K=512 and K=7168 shapes.
#88 failed because ALL shapes used CK ASM (K=512 had no tuned configs → terrible).
...
_cache_key = None
_cache_val = None  # standard B_scale for Triton path
_cache_val_sh = None  # shuffled B_scale for CK path
_out_cache = {}

# Shapes where CK ASM has tuned configs in CSV (confirmed exist)
_CK_SHAPES = {(64, 7168, 2048), (256, 3072, 1536)}


@torch.inference_mode()
def custom_kernel(data: input_t) -> output_t:
    global _cache_key, _cache_val, _cache_val_sh
    ...
    # For M=64/K=2048 and M=256/K=1536: use CK ASM (tuned, faster than Triton)
    if (m, n, k) in _CK_SHAPES:
        # CK ASM path: needs shuffled A and B
    ...
    # For other shapes: use Triton path
    if key != _cache_key:
        if k <= 512:
