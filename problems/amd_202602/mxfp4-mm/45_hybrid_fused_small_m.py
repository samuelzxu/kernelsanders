"""
MXFP4-MM: Fused quant+GEMM for M<=16, regular Triton for M>=32.
The fused kernel saves a kernel launch for tiny M but has too much
register pressure for larger M.
"""
import json
import os
import triton
...
def _fused_quant_gemm_small_m(
    ...
    """Fused quant_A + GEMM for small M (M<=16)."""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # For M<=16 with no split-K: use fused kernel
    ...
    if m <= 16 and config.get("NUM_KSPLIT", 1) == 1:
        ...
        # Regular path for M>=32 or split-K
