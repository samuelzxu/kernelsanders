...
#!POPCORN gpu MI355X
"""#599: v3 kernel — A bf16 to LDS (cooperative), quant in register, no A write-back."""
            ...
            for (int r = 0; r < bytes_per_thread; r += 16) {
                ...
                if (abs_row < M) {
                    ...
                    for (int chunk = 0; chunk < bytes_per_thread / 16; chunk++) {
                        int lds_idx = a_r * A_ROW_BYTES + a_c + chunk * 16;
...
_v3 = None
try:
    _v3 = load_inline(name='v4_599',cpp_sources=_hip_cpp,cuda_sources=_hip_src,
