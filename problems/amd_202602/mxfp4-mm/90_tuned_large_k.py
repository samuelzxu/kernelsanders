"""
MXFP4-MM: #90 - Aggressive config tuning for large-K shapes.
Current bottlenecks (ranked):
  M=16, N=2112, K=7168: 21.3µs (KSPLIT=8)
  M=64, N=7168, K=2048: 21.7µs (no KSPLIT)
  M=256, N=3072, K=1536: 20.6µs (KSPLIT=2)

Changes:
  K=7168/M=16: Try KSPLIT=4 with BSK=512 (fewer splits = less reduction overhead)
  K=2048/M=64: Try BSN=64 or BSN=128 (more N parallelism)
  K=1536/M=256: Try KSPLIT=1 with BSM=128 BSN=64 (no split-K overhead)
  Also: add torch.inference_mode() and pre-allocate output
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # CHANGED: Try KSPLIT=4 instead of 8 for M=16 (less reduction overhead)
    # Also try BSN=128 for more N parallelism
    "N=2112-K=7168": {
        "M_LEQ_16": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 4, "num_warps": 4, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
    ...
    # CHANGED: Try BSN=64 for more N parallelism (was BSN=32)
    "N=7168-K=2048": {
    ...
    # CHANGED: Try KSPLIT=1 for M=256 (eliminate reduction overhead)
    "N=3072-K=1536": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 3, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "M_LEQ_256": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 16, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
}
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # For M<=16 with no split-K: fused quant+GEMM
