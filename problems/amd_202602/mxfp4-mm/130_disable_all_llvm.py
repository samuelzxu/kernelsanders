"""
MXFP4-MM: #130 - Disable ALL LLVM optimizations.
#128/129 used comma-separated flags which may not be parsed correctly.
Try DISABLE_LLVM_OPT=1 to disable ALL LLVM opts and verify the env var works.
If this improves performance, then fine-tune which passes to disable.
"""
import os, shutil
# Disable ALL LLVM optimizations
os.environ['DISABLE_LLVM_OPT'] = '1'
# Also try the LLVM-style flag format as backup
os.environ.setdefault('LLVM_ARGS', '-disable-lsr -disable-machine-licm')

# Clear Triton cache
for cache_dir in [os.path.expanduser('~/.triton/cache'), os.path.expanduser('~/.triton')]:
