"""
MXFP4-MM: Custom minimal Triton FP4 GEMM kernel.
Stripped of all generic overhead: no XCD remapping, no split-K branching,
no config deserialization. Just the core dot_scaled loop.
"""
import json
import os
import triton
...
# Minimal FP4 GEMM kernel — no XCD remap, no split-K, no EVEN_K branching
@triton.jit
def _minimal_fp4_gemm(
    ...
    SG: tl.constexpr = 32  # scale group size

    pid = tl.program_id(0)
    num_m = tl.cdiv(M, BLOCK_M)
    num_n = tl.cdiv(N, BLOCK_N)
    pid_m, pid_n = pid_grid(pid, num_m, num_n, GROUP_SIZE_M=GROUP_SIZE_M)
    ...
    for _ in range(tl.cdiv(K, BLOCK_K // 2)):
        a_sc = tl.load(a_sc_ptrs)
        b_sc = tl.load(b_sc_ptrs, cache_modifier=cache_modifier)
        a = tl.load(a_ptrs)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m <= 16 and config.get("NUM_KSPLIT", 1) == 1:
    ...
    elif config.get("NUM_KSPLIT", 1) == 1:
        # Custom minimal kernel for M>=32, no split-K
        ...
        custom_config = {
            ...
            "BLOCK_K": max(config["BLOCK_SIZE_K"], 128),
            ...
            "cache_modifier": config.get("cache_modifier", ".cg"),
        }
        if custom_config["BLOCK_K"] >= 2 * K_packed:
            custom_config["BLOCK_K"] = triton.next_power_of_2(2 * K_packed)
        grid = lambda META: (triton.cdiv(m, META["BLOCK_M"]) * triton.cdiv(n, META["BLOCK_N"]),)
        _minimal_fp4_gemm[grid](
        ...
        # Split-K: use aiter's kernel (handles reduce)
        A_q, A_scale = dynamic_mxfp4_quant(A)
