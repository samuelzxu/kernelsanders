"""
MXFP4-MM: #136 - Copy+patch+redirect AMD compiler.py.
#135 showed PermissionError on the original file. New approach:
1. Copy compiler.py to writable /tmp/
2. Patch the copy to inject -disable-lsr -disable-machine-licm
3. Replace the module in sys.modules before Triton JIT compiles
"""
import os, shutil, sys, importlib, importlib.util

def _copy_patch_redirect():
    """Copy AMD backend compiler.py, patch it, redirect import."""
    try:
        src = '/usr/local/lib/python3.12/dist-packages/triton/backends/amd/compiler.py'
        if not os.path.exists(src):
            print(f"[PATCH] Source not found: {src}", file=sys.stderr)
            return False

        dst_dir = '/tmp/triton_amd_patched'
        ...
        # Patch: inject LLVM flags into optimize_module call
        old = "llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O3, options.arch, '', [], options.enable_fp_fusion)"
        new = "llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O3, options.arch, '', ['-disable-lsr', '-disable-machine-licm'], options.enable_fp_fusion)"
        ...
        if old in content:
            ...
            # Load patched module and replace in sys.modules
            ...
            print(f"[PATCH] Successfully patched and redirected AMD compiler", file=sys.stderr)
            ...
            print(f"[PATCH] Pattern not found in source. Dumping context...", file=sys.stderr)
            idx = content.find('optimize_module')
            if idx >= 0:
                print(f"[PATCH] Context: {content[max(0,idx-10):idx+150]}", file=sys.stderr)
            return False
