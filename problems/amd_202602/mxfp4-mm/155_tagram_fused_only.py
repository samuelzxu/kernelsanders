"""
MXFP4-MM: #155 - TagRAM stride padding, fused-path only (correctness fix for #152).

#152 applied TagRAM padding globally and passed non-contiguous B_q to
gemm_afp4wfp4, which assumes contiguous layout → correctness failure.

This version restricts the TagRAM padding to the fused quant+GEMM kernel path
only. In that path we pass explicit stride parameters to the Triton kernel, so
non-contiguous B_q_t (transposed padded view) is handled correctly.

The benefit: for M=1-16 with K=2048 (N=7168, NUM_KSPLIT=1) in the fused path,
B_q row stride changes from 1024 bytes (all rows hash to the same L2 cache set)
to 1088 bytes (rows spread across 8 distinct cache sets). This reduces L2 TagRAM
bank serialization for those shapes.

For all other shapes (large M, K=7168, K=1536, K=512), no change from #148.
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_tagram_fused_o1'
...
# Cache padded B_q views for the fused kernel path.
# Maps (data_ptr, n, k) -> padded view with stride(0) = K//2 + 64.
_TAGRAM_PAD = 64
_bq_pad_cache = {}


def _get_bq_padded_for_fused(B_q_uint8, n, k):
    """For fused kernel path: return a B_q view with TagRAM-dehotspotted stride.

    The fused kernel accepts strides explicitly, so non-contiguous B_q_t is safe.
    Only apply for K//2 that is a multiple of 512 bytes (actual hotspot shapes).
    """
    half_k = k // 2
    if (half_k % 512) != 0:
        return B_q_uint8
    ck = (B_q_uint8.data_ptr(), n, k)
    if ck in _bq_pad_cache:
        return _bq_pad_cache[ck]
    padded = torch.empty((n, half_k + _TAGRAM_PAD), dtype=torch.uint8, device=B_q_uint8.device)
    padded[:, :half_k] = B_q_uint8
    view = padded[:, :half_k]   # stride(0) = half_k + 64, stride(1) = 1
    _bq_pad_cache[ck] = view
    return view
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m <= 16 and config.get("NUM_KSPLIT", 1) == 1:
        ...
        # TagRAM padding only for fused path: Triton kernel uses explicit strides
        # so non-contiguous B_q_t is handled correctly.
        B_q_padded = _get_bq_padded_for_fused(B_q_uint8, n, k)
        B_q_t = B_q_padded.T
        ...
        # Large-M path: gemm_afp4wfp4 requires contiguous B_q
