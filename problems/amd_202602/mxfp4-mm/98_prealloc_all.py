"""
MXFP4-MM: #98 - Pre-allocate ALL outputs + pass config to gemm_afp4wfp4.
Reduces per-call overhead:
1. Pre-allocate output tensors for all shapes (no torch.empty per call)
2. Pass config dict to gemm_afp4wfp4 (skip internal _get_config lookup)
3. Cache fused_config dict per shape (avoid dict comprehension per call)
Based on #92 configs.
"""
import json
import os
...
# Cache per-shape config and output tensors
_shape_cache = {}
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if shape_key not in _shape_cache:
        ...
        _shape_cache[shape_key] = (use_fused, config, fused_config, y)

    use_fused, config, fused_config, y = _shape_cache[shape_key]
    ...
    if use_fused:
        ...
        return gemm_afp4wfp4(A_q, B_q_uint8, A_scale, B_scale, dtype=torch.bfloat16, y=y, config=config)
