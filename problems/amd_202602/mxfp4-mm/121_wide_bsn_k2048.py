"""
MXFP4-MM: #121 - BSM=16 BSN=128 for K=2048 M=64.
Current BSM=32 BSN=64: 2*112=224 tiles, each tile 32x64 accumulators.
New BSM=16 BSN=128: 4*56=224 tiles (same count), each tile 16x128 accumulators.
Same register pressure (2048 fp32 accumulators) but wider BSN means each tile
processes more N columns with the same A data → better A data reuse.
B loads are 256*128=32KB vs 256*64=16KB → better coalescing.
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # KEY CHANGE: BSM=16 BSN=128 with num_stages=4 for K=2048
    "N=7168-K=2048": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 4, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
