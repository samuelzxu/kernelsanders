"""
MXFP4-MM: #91 - Optimal tile configs based on CU occupancy analysis.
Key changes:
1. Shape 6 (M=256,K=1536): BSM=32 KSPLIT=1 → eliminates 768-block reduction kernel (~3-4µs)
2. Shapes 3/4 (M=32,K=512): Extend fused kernel to M<=32 → saves quant call (~1µs)
3. Shape 5 (M=64,K=2048): Try BSM=16 BSN=32 BSK=1024 → 896 tiles vs 448
4. Shape 2 (M=16,K=7168): Try num_stages=3 waves_per_eu=2 → better pipelining
...
_CONFIGS = {
    ...
    # Shape 2: Keep KSPLIT=8 (264 tiles, near-optimal). Try num_stages=3
    "N=2112-K=7168": {
    ...
    # Shape 5: BSN=64 proved better in #90 (-1.3µs). Keep BSK=512 since K=2048/512=4 iters
    "N=7168-K=2048": {
    ...
    # Shape 6: BSM=32 KSPLIT=2 → 768 tiles (3 clean waves) + small reduction
    # #90 showed KSPLIT=1 with BSM=128 was worse (only 96 tiles). Keep KSPLIT=2 but use BSM=32
    "N=3072-K=1536": {
        ...
        "M_LEQ_256": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 2, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 16, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
}
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # For M<=32 with no split-K: fused quant+GEMM (extended from M<=16)
