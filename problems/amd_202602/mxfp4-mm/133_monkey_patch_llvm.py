"""
MXFP4-MM: #133 - Monkey-patch Triton AMD backend to disable LSR+LICM.
Found the exact code: llvm.optimize_module(mod, O3, arch, '', [], fp_fusion)
The 5th arg [] is the pass list. Patch make_llir to pass disable flags.
"""
...
# Monkey-patch the AMD backend's make_llir to inject LLVM flags
def _patch_amd_backend():
    try:
        # Find and import the AMD backend
        from triton.backends.amd import compiler as amd_compiler
        from triton._C import libtriton as _triton

        # Get the original optimize_module function
        orig_optimize = _triton.llvm.optimize_module

        # Create patched version that injects disable flags
        def patched_optimize(mod, opt_level, arch, features, flags, fp_fusion):
            # Add flags to disable LSR and Machine LICM
            extra_flags = ['-disable-lsr', '-disable-machine-licm']
            all_flags = list(flags) + extra_flags
            return orig_optimize(mod, opt_level, arch, features, all_flags, fp_fusion)

        # Replace
        _triton.llvm.optimize_module = patched_optimize
        print("[PATCH] Successfully patched llvm.optimize_module with -disable-lsr -disable-machine-licm", file=sys.stderr)
        return True
        ...
        try:
            ...
            orig_make_llir = HIPBackend.make_llir.__func__

            def patched_make_llir(src, metadata, options):
                # Call original to get through MLIR passes
                result = orig_make_llir(src, metadata, options)
                return result

            # Can't easily patch make_llir since optimize_module is called inside
            print(f"[PATCH] Alternative path not viable", file=sys.stderr)
        except Exception as e2:
            print(f"[PATCH] Alt failed: {e2}", file=sys.stderr)
        return False

_patched = _patch_amd_backend()
