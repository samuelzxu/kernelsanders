"""
MXFP4-MM: #105 - Hardcoded dispatch for known shapes.
Eliminates per-call Python overhead: _get_config lookup, dict comprehension,
lambda creation, config filtering. Pre-computes everything at import time.
Based on #102 configs with MFMA32 for M=256.
"""
import json
import os
import triton
...
# Pre-compute fused configs for known shapes (avoid per-call dict comprehension)
_FUSED_KEYS = ("BLOCK_SIZE_M", "BLOCK_SIZE_N", "BLOCK_SIZE_K", "GROUP_SIZE_M",
               "num_warps", "num_stages", "waves_per_eu", "matrix_instr_nonkdim",
               "cache_modifier")

_dispatch_cache = {}  # (m, n, k) -> (use_fused, ksplit, fused_config_or_none, output_tensor)
_cache_key = None
_cache_val = None


def _get_dispatch(m, n, k, device):
    """Pre-compute and cache dispatch info for a shape."""
    if (m, n, k) in _dispatch_cache:
        return _dispatch_cache[(m, n, k)]
    config, _ = _get_config(m, n, k)
    ksplit = config.get("NUM_KSPLIT", 1)
    use_fused = (m <= 16 and ksplit == 1)
    fused_config = None
    if use_fused:
        fused_config = {k_: config[k_] for k_ in _FUSED_KEYS if k_ in config}
    y = torch.empty((m, n), dtype=torch.bfloat16, device=device)
    _dispatch_cache[(m, n, k)] = (use_fused, fused_config, y)
    return use_fused, fused_config, y
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Fast dispatch using cached shape info
    use_fused, fused_config, y = _get_dispatch(m, n, k, A.device)
