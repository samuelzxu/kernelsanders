"""
MXFP4-MM: #182 - Use e8m0_unshuffle for ALL K values, not just K>512.

CRITICAL BUG FOUND: The leaderboard benchmark regenerates input data every
iteration (recheck=True, new seed each call). This means our B_scale cache
(keyed by data_ptr) NEVER HITS — we were running dynamic_mxfp4_quant(B) on
the full N×K matrix EVERY call for K<=512 shapes.

For M=4, N=2880, K=512: quantizing 1.47M bf16 values every call just to get
B_scale — massive overhead. Meanwhile B_scale_sh is already provided in the
input. e8m0_unshuffle on the 46K uint8 scale tensor is ~30x cheaper.

Fix: use e8m0_unshuffle(B_scale_sh) for ALL shapes, removing the K<=512
special case that re-quantizes B.
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_unshuffle_all'
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # ALWAYS use e8m0_unshuffle on provided B_scale_sh — never re-quantize B.
    # The leaderboard benchmark regenerates data every iteration (recheck=True),
    # so our old data_ptr cache never hit. Re-quantizing the full N×K B matrix
    # every call was the main overhead for K<=512 shapes.
