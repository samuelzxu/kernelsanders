...
#!POPCORN gpu MI355X
"""
#446: Roofline analysis. How far are we from theoretical limits?
"""
import torch, os, json
from task import input_t, output_t

# Compute theoretical limits for each shape
print("=== Roofline Analysis ===")
print(f"MI355X: ~3.3 TB/s HBM, ~10 PFLOPS FP4")
for m,n,k in [(4,2880,512),(16,2112,7168),(32,4096,512),(32,2880,512),(64,7168,2048),(256,3072,1536)]:
    # FP4 FLOPS: M * N * K * 2 (multiply + add)
    flops = m * n * k * 2
    # Memory: A (bf16) + B_preshuffle (fp4 packed) + B_scale + output
    mem_a = m * k * 2  # bf16
    k_packed = k // 2
    mem_b = (n // 16) * (k_packed * 16)  # preshuffle format
    mem_bs = (n // 32) * k  # B scales
    mem_out = m * n * 2  # bf16 output
    mem_total = mem_a + mem_b + mem_bs + mem_out

    compute_time = flops / 10e15  # seconds at peak FP4 FLOPS
    bandwidth_time = mem_total / 3.3e12  # seconds at peak bandwidth

    print(f"  M={m:3d} N={n:4d} K={k:4d}: "
          f"FLOPS={flops/1e6:.1f}M "
          f"Mem={mem_total/1024:.0f}KB "
          f"compute={compute_time*1e6:.2f}µs "
          f"bw={bandwidth_time*1e6:.2f}µs "
          f"limit={max(compute_time,bandwidth_time)*1e6:.2f}µs")
