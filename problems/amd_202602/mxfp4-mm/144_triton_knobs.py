"""
MXFP4-MM: #144 - Explore Triton knobs system for compilation control.
Dump available knobs on the runner to find hidden compilation flags.
Also try setting knobs that might affect codegen quality.
"""
...
# Dump all available triton knobs/config for debugging
try:
    from triton.knobs import compilation as _comp_knobs
    print(f"[KNOBS] compilation: {dir(_comp_knobs)}", file=sys.stderr)
    for attr in dir(_comp_knobs):
        if not attr.startswith('_'):
            val = getattr(_comp_knobs, attr)
            print(f"[KNOBS] compilation.{attr} = {val}", file=sys.stderr)
except Exception as e:
    print(f"[KNOBS] compilation import failed: {e}", file=sys.stderr)

try:
    from triton.knobs import runtime as _rt_knobs
    print(f"[KNOBS] runtime: {dir(_rt_knobs)}", file=sys.stderr)
    for attr in dir(_rt_knobs):
        if not attr.startswith('_'):
            val = getattr(_rt_knobs, attr)
            print(f"[KNOBS] runtime.{attr} = {val}", file=sys.stderr)
except Exception as e:
    print(f"[KNOBS] runtime import failed: {e}", file=sys.stderr)

try:
    import triton.knobs as _knobs
    print(f"[KNOBS] top-level: {dir(_knobs)}", file=sys.stderr)
except Exception as e:
    print(f"[KNOBS] top-level failed: {e}", file=sys.stderr)

# Try setting knobs that might help
try:
    from triton import knobs
    if hasattr(knobs, 'compilation'):
        if hasattr(knobs.compilation, 'disable_line_info'):
            knobs.compilation.disable_line_info = True
            print("[KNOBS] Set disable_line_info=True", file=sys.stderr)
except Exception:
    pass
