"""
MXFP4-MM: #110 - torch.compile with reduce-overhead mode for M>16 path.
Unlike manual HIP graph (#109), torch.compile handles sync intelligently.
Only compile the M>16 path (quant+GEMM). Keep fused kernel for M<=16.
"""
import json
import os
import triton
...
def _quant_gemm(A_q, B_q_uint8, A_scale, B_scale):
    """Simple wrapper for torch.compile - no control flow."""
    return gemm_afp4wfp4(A_q, B_q_uint8, A_scale, B_scale, dtype=torch.bfloat16)


# Compile the GEMM with reduce-overhead (uses HIP graphs internally but managed properly)
try:
    _compiled_gemm = torch.compile(_quant_gemm, mode="reduce-overhead", fullgraph=True)
except Exception:
    _compiled_gemm = _quant_gemm
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m <= 16 and config.get("NUM_KSPLIT", 1) == 1:
        ...
        return _compiled_gemm(A_q, B_q_uint8, A_scale, B_scale)
