"""
MXFP4-MM: #162 - Balanced KSPLIT=3 for K=1536 M_LEQ_64.

K=1536, BLOCK_K=512: 1536/512 = 3 K-blocks total.
Current KSPLIT=2: 3/2 = 1.5 (NOT integer) — imbalanced splits.
  One split does 2 iterations, other does 1 — 33% MFMA waste in shorter split.

KSPLIT=3: 3/3 = 1.0 (exactly integer). Each split does 1 K-iteration.
  Zero wasted MFMA work. Perfectly balanced.

CTA counts for M_LEQ_64 with KSPLIT=3:
  m=64: 3 * ceil(64/32) * ceil(3072/64) = 3 * 2 * 48 = 288 CTAs (1.13 waves)
  m=32: 3 * 1 * 48 = 144 CTAs (0.56 waves)
  m=16: 3 * 1 * 48 = 144 CTAs (0.56 waves)
vs KSPLIT=2:
  m=64: 2 * 2 * 48 = 192 CTAs (0.75 waves)
  m=32: 2 * 1 * 48 = 96 CTAs (0.375 waves)

KSPLIT=3 gives better utilization at all M sizes.
Tradeoff: 3 atomic reductions vs 2. But perfectly balanced work + higher
utilization should outweigh the small extra reduction cost.

Also try KSPLIT=3 for M_LEQ_256 (same reasoning).
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_ksplit3_o1'
...
_CONFIGS = {
    ...
    # K=1536: balanced KSPLIT=3 (was 2)
    # 1536/512 = 3 K-blocks. KSPLIT=3: 1 iter each (perfect balance)
    "N=3072-K=1536": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 3, "num_warps": 4, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "M_LEQ_256": {"BLOCK_SIZE_M": 64, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 3, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 16, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
}
