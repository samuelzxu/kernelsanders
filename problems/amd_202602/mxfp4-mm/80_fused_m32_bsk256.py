"""
MXFP4-MM: Extend fused quant+GEMM to M<=32 with BSK=256, BSN=32.
Smaller tiles reduce register pressure that killed M=32 with BSK=512.
BSK=256 means 2 loop iterations for K=512 but less register usage per iter.
"""
import json
import os
import triton
...
# Config injection — use BSK=256 for fused M=32 path
_CONFIGS = {
    "N=2880-K=512": {
        ...
        "M_LEQ_32": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 32, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 3, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Fused quant+GEMM for M<=32 with KSPLIT=1 (uses BSK=256 for M=32)
