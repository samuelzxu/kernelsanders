...
#!POPCORN gpu MI355X
"""
#565: Atomic split-K reduce via custom HIP kernel.
Instead of Triton reduce kernel, use a fused HIP reduce that:
1. Reads [KSPLIT, M, N] f32 intermediate
2. Sums along dim 0
3. Converts to bf16
4. Writes [M, N] output
All in a single HIP kernel with minimal dispatch overhead.
"""
...
# Build custom fast reduce kernel via load_inline
_reduce_hip = r"""
#include <hip/hip_runtime.h>
#include <torch/extension.h>

// Optimized reduce for KSPLIT=7 (shape 2: M=16 N=2112 K=7168)
// Each thread handles 4 output elements (vectorized store)
// Contiguous layout: src is [KSPLIT, M, N] with stride_k = M*N
...
// Pre-allocated output to avoid per-call allocation
static torch::Tensor g_reduce_out;
static int g_reduce_m = 0, g_reduce_n = 0;

torch::Tensor fused_reduce(torch::Tensor src, int M, int N, int KSPLIT) {
    // Reuse output tensor if same shape
    if (M != g_reduce_m || N != g_reduce_n) {
        g_reduce_out = torch::empty({M, N}, torch::dtype(torch::kBFloat16).device(src.device()));
        g_reduce_m = M; g_reduce_n = N;
    }
    int MN = M * N;
    int threads = 256;
    int blocks = (MN + threads - 1) / threads;
    int stride_k = (int)src.stride(0);

    if (KSPLIT == 7) {
        fast_reduce_k7<<<blocks, threads, 0, 0>>>(
            src.data_ptr<float>(),
            (unsigned short*)g_reduce_out.data_ptr(),
            MN, stride_k);
    } else {
        fast_reduce_gen<<<blocks, threads, 0, 0>>>(
            src.data_ptr<float>(),
            (unsigned short*)g_reduce_out.data_ptr(),
            MN, stride_k, KSPLIT);
    }
...
_reduce_mod = None
try:
    _reduce_mod = load_inline(
        name='fast_reduce_565',
        cpp_sources=_reduce_cpp,
        cuda_sources=_reduce_hip,
        functions=['fused_reduce'],
        verbose=False,
        extra_cuda_cflags=['-O3', '-w', '-mcumode', '--offload-arch=gfx950'],
    )
    print("Fast reduce kernel OK")
except Exception as e:
    print(f"Fast reduce FAILED: {e}")

# Also build fused quant+CK ASM for shape 6
_hip_ck = r"""
#include <hip/hip_runtime.h>
#include <torch/extension.h>
...
torch::Tensor quant_and_gemm(
    torch::Tensor A, torch::Tensor B_shuffle, torch::Tensor B_scale_sh, int N_val
) {
    ...
    ka.stride_A0 = K_elem; ka.stride_A1 = 1;
...
try:
    ...
    if _ck_mod.init_ck_kernel(_co, _fn):
        ...
        _ck_mod.quant_and_gemm(_wA, _wBq, _wBs, 3072)
        torch.cuda.synchronize()
        del _wA, _wBq, _wBs
        print("CK ASM OK")
    else:
        _ck_mod = None
        print("CK init failed")
except Exception as e:
    _ck_mod = None
    print(f"CK build FAILED: {e}")

# Warmup preshuffle for all shapes
...
# Now monkey-patch preshuffle to use skip_reduce + fast HIP reduce for KSPLIT shapes
_orig_preshuffle = gemm_a16wfp4_preshuffle
_reduce_y = {}  # cache output tensors

def _patched_preshuffle(x, w, w_scales, prequant=True, dtype=torch.bfloat16, y=None, config=None):
    if _reduce_mod is None:
        # No fast reduce available, use original
        return _orig_preshuffle(x, w, w_scales, prequant=prequant, dtype=dtype, y=y, config=config)

    # Call original with skip_reduce=True for KSPLIT shapes
    result = _orig_preshuffle(x, w, w_scales, prequant=prequant, dtype=dtype, y=y, config=config, skip_reduce=True)

    if result.dim() == 3:
        # Got [KSPLIT, M, N] intermediate — use fast HIP reduce
        ks, m, n = result.shape
        return _reduce_mod.fused_reduce(result, m, n, ks)
    else:
        # KSPLIT=1, result is already [M, N]
        return result

# Check if skip_reduce parameter is supported
import inspect
_sig = inspect.signature(_orig_preshuffle)
if 'skip_reduce' in _sig.parameters:
    print("skip_reduce supported — patching preshuffle")
    # We'll use this in the kernel function
    _use_fast_reduce = True
else:
    print("skip_reduce NOT supported — using original")
    _use_fast_reduce = False

_ps_ck=None;_ps_cw=None;_ps_cs=None
_y_cache={}  # Pre-allocated output tensors per (m,n) shape
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Pre-allocate output tensor (reuse across calls with same shape)
    key=(m,n)
    if key not in _y_cache:
        _y_cache[key]=torch.empty((m,n),dtype=torch.bfloat16,device=A.device)
    _y_out=_y_cache[key]

    # Fast reduce for K=7168 (KSPLIT=8→7, high split count benefits from HIP reduce)
    if _use_fast_reduce and _reduce_mod is not None and k >= 7168:
        result = _orig_preshuffle(A,_ps_cw,_ps_cs,prequant=True,dtype=torch.bfloat16,skip_reduce=True)
        if result.dim() == 3:
            ks, rm, rn = result.shape
            return _reduce_mod.fused_reduce(result, rm, rn, ks)
        return result

    return _orig_preshuffle(A,_ps_cw,_ps_cs,prequant=True,dtype=torch.bfloat16,y=_y_out)
