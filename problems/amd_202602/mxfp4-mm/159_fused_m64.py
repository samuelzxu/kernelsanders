"""
MXFP4-MM: #159 - Extend fused quant+GEMM kernel to M<=64.

Extends the fused quant+GEMM threshold from M<=16 to M<=64 (guarded by
NUM_KSPLIT==1 to exclude K=7168 and K=1536 shapes).

New shapes captured (previously using separate quant + GEMM):
  m=32, k=512, n=2880  → 1 K-iteration, BLOCK_M=32 (clean, low register cost)
  m=32, k=512, n=4096  → same
  m=64, k=2048, n=7168 → 4 K-iterations, BLOCK_M=32 (more inline quant work)
  m=32, k=2048, n=7168 → 4 K-iterations, BLOCK_M=32

By fusing, we eliminate:
  1. One dynamic_mxfp4_quant(A) kernel dispatch (~1-2µs overhead)
  2. Writing A_q + A_scale to HBM and reading them back in the GEMM kernel

The k=512 shapes are the cleanest gain (1 K-iteration = minimal quant overhead).
The k=2048 shapes have 4 inline quant iterations per K-loop but the register
footprint stays manageable (BLOCK_M=32 * BLOCK_K=512 / 64 threads = 256 bf16
per thread in the A tile = ~32 VGPRs, comfortably within the 256-VGPR limit).
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_fused_m64_o1'
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Extend fused threshold to M<=64. Guard: NUM_KSPLIT==1 excludes K=7168
    # (KSPLIT=8) and K=1536 (KSPLIT=2), keeping only K=512 and K=2048 shapes.
    if m <= 64 and config.get("NUM_KSPLIT", 1) == 1:
        out_key = (m, n)
