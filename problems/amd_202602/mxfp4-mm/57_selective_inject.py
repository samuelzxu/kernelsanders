"""
MXFP4-MM: Only inject K=512 and K=7168 configs. Let runner's own configs handle K=2048 and K=1536.
The runner may have better tuned configs for those shapes that we've been overwriting.
"""
import json
import os
import triton
...
# Only inject configs we KNOW are better than defaults
_CONFIGS_TO_INJECT = {
    "N=2880-K=512": {
    ...
    # K=7168: our BSN=64/KSPLIT=8 is proven better than repo's BSN=32/KSPLIT=14
    "N=2112-K=7168": {
...
# K=2048 and K=1536: DON'T overwrite - let runner's tuned configs be used
# (We only write if not exists, preserving runner's originals)
_CONFIGS_IF_MISSING = {
    "N=3072-K=1536": {
...
def _inject_configs():
    ...
    # Force-write proven better configs
    for shape_key, config in _CONFIGS_TO_INJECT.items():
        fpath = f"{config_dir}/{dev}-GEMM-AFP4WFP4-{shape_key}.json"
        with open(fpath, "w") as f:
            json.dump(config, f)
    # Only write if missing (don't overwrite runner's tuned configs)
    for shape_key, config in _CONFIGS_IF_MISSING.items():
        fpath = f"{config_dir}/{dev}-GEMM-AFP4WFP4-{shape_key}.json"
