"""
MXFP4-MM: #114 - KSPLIT=3 for K=1536 (get_splitk compatible!).
Discovery: get_splitk(768, 512, 3) → KSPLIT=3 preserved!
  SPLITK_BLOCK_SIZE=512, K%256=0 ✓, 512%512=0 ✓, K%256=0 ✓
  576 tiles (2.25 waves) vs KSPLIT=1's 192 tiles (0.75 waves).
  Each tile does 1 BSK iteration (vs 3 with KSPLIT=1).
  Trade: fewer iters per tile + small 3-way reduction vs more iters no reduction.
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # KEY CHANGE: KSPLIT=3 for M=256 (passes get_splitk validation)
    # 576 tiles (2.25 waves) vs 192 tiles (0.75 waves) with KSPLIT=1
    "N=3072-K=1536": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 3, "num_warps": 4, "num_stages": 3, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "M_LEQ_256": {"BLOCK_SIZE_M": 64, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 3, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 16, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
}
