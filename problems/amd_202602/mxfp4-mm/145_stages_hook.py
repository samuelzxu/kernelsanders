"""
MXFP4-MM: #145 - Use add_stages_inspection_hook to patch LLVM optimization.
#144 discovered the hook: knobs.runtime.add_stages_inspection_hook
This hook receives the stages dict and can replace the llir stage.
We replace the llir stage with one that calls optimize_module at O1 level
or skips specific passes.
"""
...
# Set the stages inspection hook to patch llir stage
def _stages_hook(backend, stages, options, language, extra):
    """Replace the llir stage with a patched version."""
    orig_llir = stages.get("llir")
    if orig_llir is None:
        return

    def patched_llir(src, metadata):
        # Call original make_llir but with DISABLE_LLVM_OPT=disable-lsr
        os.environ['DISABLE_LLVM_OPT'] = 'disable-lsr'
        result = orig_llir(src, metadata)
        # Restore
        if 'DISABLE_LLVM_OPT' in os.environ:
            del os.environ['DISABLE_LLVM_OPT']
        return result

    stages["llir"] = patched_llir
    print(f"[HOOK] Patched llir stage for {language}", file=sys.stderr)

_triton_knobs.runtime.add_stages_inspection_hook = _stages_hook
print("[HOOK] Installed stages inspection hook", file=sys.stderr)
