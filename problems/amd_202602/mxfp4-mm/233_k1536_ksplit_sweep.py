...
#!POPCORN gpu MI355X
"""
MXFP4-MM #233: Try KSPLIT=2 + larger tiles for K=1536 M=256.
Hypothesis: KSPLIT=3 with BSK=256 does only 1 K-iter per block (no data reuse).
KSPLIT=2 with BSK=128 does 3 K-iters per block (better LDS reuse, less atomic overhead).
Grid: 8×24×2 = 384 blocks vs 8×12×3 = 288 blocks (more blocks too).
"""
...
_CONFIGS = {
    ...
    # K=1536: Try KSPLIT=2 with BSN=128, BSK=128
    # K_packed=768, KSPLIT=2 -> 384 per split, BSK=128 -> 3 iterations
    # Grid: ceil(256/32) x ceil(3072/128) x 2 = 8 x 24 x 2 = 384 blocks
    "N=3072-K=1536": {
        ...
        "M_LEQ_256": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 2, "num_warps": 4, "num_stages": 2, "waves_per_eu": 4, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
