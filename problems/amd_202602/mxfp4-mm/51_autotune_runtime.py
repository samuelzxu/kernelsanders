"""
MXFP4-MM: Runtime autotuning for K=2048/M=64 shape.
On first call for each shape, try 2-3 config variants and pick fastest.
Uses torch.cuda.Event timing (no CPU-GPU sync in the hot path).
"""
import json
import os
import triton
...
# Runtime autotune: for K=2048/M=64, try alternate configs on first call
_autotune_done = {}
_alt_configs_7168_2048 = [
    # Current best from tuning repo
    {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 32, "BLOCK_SIZE_K": 1024, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
    # Alternative: larger tiles
    {"BLOCK_SIZE_M": 64, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 3, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
    # Alternative: split-K
    {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 2, "num_warps": 4, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
]


def _run_gemm(A_q, B_q_uint8, A_scale, B_scale, config=None):
    return gemm_afp4wfp4(A_q, B_q_uint8, A_scale, B_scale, dtype=torch.bfloat16, config=config)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m <= 16 and config.get("NUM_KSPLIT", 1) == 1:
        ...
        return _run_gemm(A_q, B_q_uint8, A_scale, B_scale)
