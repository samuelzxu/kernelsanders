"""
MXFP4-MM: Pipeline experiment.
Test: does the GPU pipeline quant_A + B_scale_compute + GEMM automatically?
If kernels overlap on the GPU, launching them separately costs nothing extra.
This version launches quant_A and quant_B "simultaneously" to test overlap.
"""
import json
import os
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Pipeline test: launch quant_A first, then get B_scale
    # On GPU, these should overlap if they use different resources
    A_q, A_scale = dynamic_mxfp4_quant(A)

    # For K<=512: launch quant_B (overlaps with quant_A on GPU pipeline)
    # For K>512: unshuffle B_scale (CPU work, very fast)
