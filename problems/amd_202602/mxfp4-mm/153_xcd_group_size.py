"""
MXFP4-MM: #153 - XCD-aligned GROUP_SIZE_M for K=2048 large-M shapes.

MI355X has 8 XCDs (compute dies), each with its own L2 cache slice.
The tile dispatch pattern determines which L2 slice caches which tiles of B.

With GROUP_SIZE_M=2 (current, K=2048 "any"):
  - N=7168, BLOCK_N=128: num_pid_n = 56
  - Group size = 2 * 56 = 112 tiles per wave → 14 groups per XCD
  - B matrix tiles are re-loaded many times across XCD boundaries

With GROUP_SIZE_M=8 (this experiment):
  - Group size = 8 * 56 = 448 tiles per wave → 56 groups per XCD
  - 8 consecutive M-tiles share the same N-column → maximises L2 reuse for B
  - GROUP_SIZE_M=8 exactly matches the 8-XCD count for round-robin scheduling

Also adds KSPLIT=2 for M<=64 K=2048 to improve GPU utilisation:
  - M=32: 1*112 CTAs → 224 CTAs (fills more of 256 CUs)
  - M=64: 2*112 CTAs → 448 CTAs (>100% occupancy for hiding latency)
"""
...
_CONFIGS = {
    ...
    # K=2048: GROUP_SIZE_M=8 for "any" (XCD alignment: 8 XCDs, 8*56=448 tiles per group)
    # KSPLIT=2 for M<=64 to fill more of 256 CUs (112→224 CTAs for M=32)
    "N=7168-K=2048": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 2, "num_warps": 4, "num_stages": 4, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 8, "NUM_KSPLIT": 1, "num_warps": 8, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
    "N=3072-K=1536": {
