"""
MXFP4-MM: Combined best - fused kernel for small M + aggressive K=7168 config.
- M<=16, KSPLIT==1: fused quant+GEMM (saves kernel launch, ~1µs)
- K=7168, M=16: BSN=64, KSPLIT=8 (22µs vs 30µs with BSN=32/KSPLIT=14)
- K=512: BSK=512 injected configs
- All: data_ptr caching for warm runs
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # KEY: BSN=64 + KSPLIT=8 for K=7168 (was BSN=32, KSPLIT=14)
    "N=2112-K=7168": {
    ...
    # Use original tuned configs for these (aggressive ones hurt K=1536/M=256)
    "N=7168-K=2048": {
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # For small M with no split-K: fused quant+GEMM (saves kernel launch)
    config, _ = _get_config(m, n, k)
    if m <= 32 and k <= 1024 and config.get("NUM_KSPLIT", 1) == 1:
        y = torch.empty((m, n), dtype=torch.bfloat16, device=A.device)
        # Use B_q transposed view (no copy!) - strides handle the layout
        B_q_t = B_q_uint8.T  # just a view, stride(0)=1, stride(1)=K//2
