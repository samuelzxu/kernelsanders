...
#!POPCORN gpu MI355X
"""
#331: Call gemm_a4w4_blockscale directly for K=1536 M=256.
gemm_a4w4_blockscale(A, B, A_scale, B_scale, out, splitK=0)
This might use hipBLASLt internally and be much faster.
"""
import os, json, torch, time
from task import input_t, output_t
from aiter.ops.triton.quant import dynamic_mxfp4_quant

# Import and trigger JIT build for blockscale
print("=== Init blockscale ===")
t0 = time.time()
...
for desc, args, kwargs in [
    ("5args+splitK", (_A_q, _B_q, _A_scale.contiguous(), _B_scale, _out), {"splitK": 0}),
    ("5args_nosplit", (_A_q, _B_q, _A_scale.contiguous(), _B_scale, _out), {}),
    ("4args", (_A_q, _B_q, _A_scale.contiguous(), _B_scale), {}),
]:
    try:
        r = gemm_a4w4_blockscale(*args, **kwargs)
        print(f"  {desc}: OK {r.shape if hasattr(r,'shape') else type(r)}")
    except Exception as e:
        print(f"  {desc}: {str(e)[:120]}")

print(f"Probe took {time.time()-t0:.1f}s")
del _A, _A_q, _A_scale, _B_q, _B_scale, _out
torch.cuda.empty_cache()

# Standard preshuffle for all shapes
