"""
MXFP4-MM: #108 - Fix split-K configs to be compatible with get_splitk().
Critical insight: get_splitk() silently adjusts configs:
  K=1536/BSK=512/KSPLIT=2 → KSPLIT=1 (only 192 tiles, 0.75 utilization!)
  K=7168/BSK=256/KSPLIT=8 → KSPLIT=7 (231 tiles, not 264)

Fix: choose BSK values where KSPLIT is preserved:
  K=1536: BSK=256 KSPLIT=2 → 384 tiles (2x improvement!)
  K=7168: BSK=128 KSPLIT=8 → 264 tiles (14% more)
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # KEY FIX: BSK=128 preserves KSPLIT=8 (get_splitk was reducing 256/8 to 7)
    "N=2112-K=7168": {
        "M_LEQ_16": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 8, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
    ...
    # K=2048: BSN=64 (already KSPLIT=1, no get_splitk issue)
    "N=7168-K=2048": {
    ...
    # KEY FIX: BSK=256 preserves KSPLIT=2 (get_splitk was reducing 512/2 to 1)
    # This gives 384 tiles instead of 192 - 2x better CU utilization!
    "N=3072-K=1536": {
        "M_LEQ_64": {"BLOCK_SIZE_M": 32, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 2, "num_warps": 4, "num_stages": 3, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "M_LEQ_256": {"BLOCK_SIZE_M": 64, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 2, "NUM_KSPLIT": 2, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 16, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
}
