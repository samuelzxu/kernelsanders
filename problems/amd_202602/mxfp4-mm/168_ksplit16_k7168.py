"""
MXFP4-MM: #168 - KSPLIT=16 for K=7168 M<=16.
...
KSPLIT=4, BLOCK_K=256:
  - 7168/256 = 28 K-blocks. 28/16 = 1.75 iterations (get_splitk adjusts) per split (perfectly balanced)
  - CTAs = 16 * ceil(16/16) * ceil(2112/64) = 4 * 1 * 33 = 528 (2.06 waves)
  - Only 16 atomic reductions per output element (vs 8 with KSPLIT=8)
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_ksplit16_o1'
...
_CONFIGS = {
    ...
    "N=2112-K=7168": {
        "M_LEQ_16": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 16, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
