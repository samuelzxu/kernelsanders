"""
MXFP4-MM: #180 - Keep O1 patch but STOP clearing Triton cache.

The _patch_to_o1() function was deleting ~/.triton/cache on every module load.
This means every kernel compiles cold in every benchmark run, adding ~3-6µs
per unique kernel variant. This is why warmup never helped — the cache gets
wiped between runs.

Fix: remove the shutil.rmtree calls. The O1 patch still takes effect because
we replace the compiler module in sys.modules before any kernels compile.
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_no_cache_clear'
            ...
            print("[PATCH] O3->O1 (cache preserved)", file=sys.stderr)
            # NO cache clearing — let compiled kernels persist across runs
            return True
        return False
