"""
MXFP4-MM: #115 - Pre-allocate A_q and A_scale buffers for quant kernel.
dynamic_mxfp4_quant allocates 2 new tensors per call (~1µs overhead).
By pre-allocating and calling the quant Triton kernel directly, save ~1µs
for M>16 shapes (M=32, M=64, M=256).
Based on #102 configs.
"""
import json
import os
import triton
...
_cache_val = None
_out_cache = {}
_quant_cache = {}  # (m, k) -> (x_fp4, blockscale, grid, quant_config)


def _fast_mxfp4_quant(x, m, k):
    """Quant with pre-allocated output buffers."""
    if (m, k) not in _quant_cache:
        ...
        # Block size selection (matches dynamic_mxfp4_quant logic)
        if m <= 32:
            NUM_ITER, BLOCK_SIZE_M = 1, triton.next_power_of_2(m)
            BLOCK_SIZE_N, NUM_WARPS, NUM_STAGES = 32, 1, 1
        else:
            NUM_ITER, BLOCK_SIZE_M = 4, 64
            BLOCK_SIZE_N, NUM_WARPS, NUM_STAGES = 64, 4, 2
            if k <= 16384:
        ...
        _quant_cache[(m, k)] = (x_fp4, blockscale_e8m0, grid, qconfig)

    x_fp4, blockscale_e8m0, grid, qconfig = _quant_cache[(m, k)]

    _dynamic_mxfp4_quant_kernel[grid](
        x, x_fp4, blockscale_e8m0,
        x.stride(0), x.stride(1),
        x_fp4.stride(0), x_fp4.stride(1),
        blockscale_e8m0.stride(0), blockscale_e8m0.stride(1),
        M=m, N=k, **qconfig,
    )
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m <= 16 and config.get("NUM_KSPLIT", 1) == 1:
        ...
        # Use pre-allocated quant buffers
        A_q, A_scale = _fast_mxfp4_quant(A, m, k)
