...
#!POPCORN gpu MI355X
"""
#439: K=1536 BSK=512 KSPLIT=1 via direct config= parameter.
K_packed=768, BSK=512: 1.5 iters → EVEN_K=False. But fewer iters = less overhead.
Grid: 8×12=96 blocks. Under-subscribed but BSK=512 has better data reuse.
"""
...
# Use config= to pass BSK=512 KSPLIT=1 directly
_k1536_cfg = {"BLOCK_SIZE_M":32,"BLOCK_SIZE_N":256,"BLOCK_SIZE_K":512,"GROUP_SIZE_M":4,"NUM_KSPLIT":1,"num_warps":8,"num_stages":2,"waves_per_eu":4,"matrix_instr_nonkdim":32,"cache_modifier":".cg"}
...
for _m,_n,_k in [(4,2880,512),(16,2112,7168),(32,4096,512),(32,2880,512),(64,7168,2048),(256,3072,1536)]:
    try:
        ...
        # Use config= for K=1536 M=256 with BSK=512
        if _m==256 and _k==1536:
            gemm_a16wfp4_preshuffle(_A,_Bw,_Bws,prequant=True,dtype=torch.bfloat16,config=_k1536_cfg)
        else:
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m==256 and k==1536:
        return gemm_a16wfp4_preshuffle(A,_cw,_cs,prequant=True,dtype=torch.bfloat16,config=_k1536_cfg)
