"""
MXFP4-MM: Patch preshuffle + use ONLY for M=4,K=512 (one JIT compile).
Fall back to fused/Triton for everything else.
"""
import os
import sys
import json
...
# STEP 1: Patch preshuffle kernel BEFORE imports
    ...
    for kernel_path in candidates:
        ...
        if bug in source:
            ...
            with open(kernel_path, "w") as f:
                f.write(source)
            print(f"[PATCH] Fixed: {kernel_path}", file=sys.stderr)
            return True
    return False
...
# Fused kernel for M<=16 (fallback when preshuffle unavailable)
@triton.jit
...
# Import preshuffle if patch succeeded
_preshuffle_fn = None
if _patch_ok:
    try:
        from aiter.ops.triton.gemm.basic.gemm_afp4wfp4 import gemm_afp4wfp4_preshuffle
        _preshuffle_fn = gemm_afp4wfp4_preshuffle
        print("[INFO] preshuffle available after patch", file=sys.stderr)
    except Exception:
        pass
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Preshuffle for M=4,K=512 ONLY (one JIT compile, known to be faster)
    if _preshuffle_fn is not None and m <= 4 and k <= 512:
    ...
    # Fused kernel for M<=16 with KSPLIT=1
