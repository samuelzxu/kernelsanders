"""
MXFP4-MM: #88 - Use CK ASM kernel (gemm_a4w4) directly.
The reference kernel uses this path. We already have B_shuffle and B_scale_sh
in the right format. Maybe CK ASM is faster than Triton for some shapes.

Hypothesis: CK ASM may be better tuned than our Triton configs for large-K shapes.
"""
import json
import os
import triton
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Get B_scale (with caching) for Triton path
    ...
    # For larger M shapes: try CK ASM kernel (like reference)
    # This uses B_shuffle and B_scale_sh directly (already in right format)
    if m >= 32:
