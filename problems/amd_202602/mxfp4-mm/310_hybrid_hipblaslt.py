...
#!POPCORN gpu MI355X
"""
#310: Hybrid: hipBLASLt for K=1536 M=256 only (where GEMM saving > quant overhead).
All other shapes use preshuffle.
"""
...
void init_gemm(int M, int N, int K) {
    ...
    int cnt=0;
    CK(hipblasLtMatmulAlgoGetHeuristic(g_h, g_md, g_lA, g_lB, g_lC, g_lC, p, 1, &g_heur, &cnt));
    hipblasLtMatmulPreferenceDestroy(p);
    g_ok = (cnt > 0);
    printf("INIT M=%d N=%d K=%d ok=%d\n", M, N, K, g_ok);
}

torch::Tensor gemm(torch::Tensor A_fp4, torch::Tensor B_fp4,
    ...
    CK(hipblasLtMatmulDescSetAttribute(g_md, HIPBLASLT_MATMUL_DESC_A_SCALE_POINTER, &bs, sizeof(bs)));
    CK(hipblasLtMatmulDescSetAttribute(g_md, HIPBLASLT_MATMUL_DESC_B_SCALE_POINTER, &as, sizeof(as)));
    float alpha=1.0f, beta=0.0f;
    CK(hipblasLtMatmul(g_h, g_md, &alpha,
        B_fp4.data_ptr(), g_lA, A_fp4.data_ptr(), g_lB,
        &beta, out.data_ptr(), g_lC, out.data_ptr(), g_lC,
        &g_heur.algo, nullptr, 0, 0));
    return out;
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("init_gemm", &init_gemm);
    m.def("gemm", &gemm);
}
"""
...
# Init for K=1536 M=256 N=3072
mod.init_gemm(256, 3072, 1536)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if m == 256 and k == 1536:
        # hipBLASLt for K=1536 M=256 only
        B_q = data[2]; B = data[1]
        A_q, A_scale = dynamic_mxfp4_quant(A)
        A_scale = A_scale.contiguous()
        _, B_scale = dynamic_mxfp4_quant(B)
        B_scale = B_scale.contiguous()
        return mod.gemm(A_q, B_q, A_scale, B_scale, m, n, k)
    else:
        # Preshuffle for all other shapes
