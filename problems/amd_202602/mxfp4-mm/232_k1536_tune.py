...
#!POPCORN gpu MI355X
"""
MXFP4-MM #232: Aggressive config tuning for K=1536 M=256.
Try mfma32, higher KSPLIT, num_stages=3, different tile shapes.
"""
...
# Keep good configs for shapes where we already beat targets
_CONFIGS = {
    "N=2880-K=512": {
    ...
    # K=1536: EXPERIMENTAL - try mfma32 + KSPLIT=4 + BSN=128 + stages=2
    # Previous best: BSM=32, BSN=256, BSK=256, KSPLIT=3, mfma16 → 19.3µs
    # New: mfma32 with aggressive occupancy
    "N=3072-K=1536": {
        ...
        # Key experiment: mfma32 + KSPLIT=4 for M=256
        # K_packed=768, KSPLIT=4 → 192 per split, BSK=128 → 1.5 iters (not ideal)
        # KSPLIT=3 → 256 per split, BSK=128 → 2 iters, BSK=256 → 1 iter
        # Try: mfma32, BSM=64, BSN=128, BSK=256, KSPLIT=3, warps=8
        # Grid: ceil(256/64)×ceil(3072/128)×3 = 4×24×3 = 288 blocks
        "M_LEQ_256": {"BLOCK_SIZE_M": 64, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 3, "num_warps": 8, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 64, "BLOCK_SIZE_N": 128, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 4, "NUM_KSPLIT": 1, "num_warps": 8, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
}
