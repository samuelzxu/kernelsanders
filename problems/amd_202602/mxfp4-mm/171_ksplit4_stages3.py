"""
MXFP4-MM: #171 - KSPLIT=4 + num_stages=3 for K=7168 M<=16.

Testing whether KSPLIT=8 (tuned for MI300X with 64KB LDS) is too aggressive
for MI355X (160KB LDS). With 2.5x more LDS, each CTA can do more work
before spilling, making fewer, fatter splits optimal.

KSPLIT=4, BLOCK_K=256:
  - 7168/256 = 28 K-blocks. 28/4 = 7 iterations per split (perfectly balanced)
  - CTAs = 4 * ceil(16/16) * ceil(2112/64) = 4 * 1 * 33 = 132 (0.52 waves)
  - Only 4 atomic reductions per output element (vs 8 with KSPLIT=8)
  - Each CTA does 7 iterations = 14 MFMAs (good pipeline fill)

Trade-off: lower GPU utilization (132 vs 264 CTAs) but half the reduction
overhead and more compute per CTA for better latency hiding.

Also testing: K=2048 shapes stay KSPLIT=1, K=1536 stays KSPLIT=2.
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_ksplit4_stg3_o1'
...
_CONFIGS = {
    ...
    # K=7168: KSPLIT=4 (down from 8) — fewer reductions, more compute per CTA
    # 28 K-blocks / 4 = 7 iterations each (perfectly balanced)
    # Leveraging MI355X 160KB LDS for deeper pipeline within each CTA
    "N=2112-K=7168": {
        "M_LEQ_16": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 4, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
        "any": {"BLOCK_SIZE_M": 128, "BLOCK_SIZE_N": 256, "BLOCK_SIZE_K": 128, "GROUP_SIZE_M": 16, "NUM_KSPLIT": 1, "num_warps": 4, "num_stages": 3, "waves_per_eu": 2, "matrix_instr_nonkdim": 32, "cache_modifier": None}
    },
    "N=7168-K=2048": {
