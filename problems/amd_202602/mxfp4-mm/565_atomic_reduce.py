...
// Fast fused reduce: sum [KSPLIT, M, N] f32 -> [M, N] bf16
// Each thread handles one (m, n) element, summing across KSPLIT
// This replaces the separate Triton reduce kernel
extern "C" __global__ void fast_reduce_f32_to_bf16(
    const float* __restrict__ src,   // [KSPLIT, M, N]
    unsigned short* __restrict__ dst, // [M, N] bf16
    int M, int N, int KSPLIT,
    int stride_k, int stride_m, int stride_n
) {
    ...
    float sum = 0.0f;
    int base = m * stride_m + n * stride_n;
    for (int k = 0; k < KSPLIT; k++) {
        sum += src[k * stride_k + base];
    }

    // f32 -> bf16 with round-to-nearest-even
    unsigned int u = __float_as_uint(sum);
    u = u + (((u >> 16) & 1) + 0x7FFF);
    dst[m * N + n] = (unsigned short)(u >> 16);
}

torch::Tensor fused_reduce(torch::Tensor src, int M, int N, int KSPLIT) {
    auto dst = torch::empty({M, N}, torch::dtype(torch::kBFloat16).device(src.device()));
    int total = M * N;
    int threads = 256;
    int blocks = (total + threads - 1) / threads;
    fast_reduce_f32_to_bf16<<<blocks, threads, 0, 0>>>(
        src.data_ptr<float>(),
        (unsigned short*)dst.data_ptr(),
        M, N, KSPLIT,
        (int)src.stride(0), (int)src.stride(1), (int)src.stride(2)
    );
    return dst;
}
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Fast reduce only for high-KSPLIT shapes (K=7168 has KSPLIT=8)
    # K=2048 with KSPLIT=2 is worse with fast reduce — keep Triton reduce
