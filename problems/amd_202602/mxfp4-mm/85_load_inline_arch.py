"""
MXFP4-MM: load_inline with PYTORCH_ROCM_ARCH set (from top AMD fp8 submissions).
Top fp8-gemm submissions ALL use load_inline + compressed C++ + ROCM_ARCH env var.
Test: can we compile a minimal kernel on gfx950 within the timeout?
"""
import os
# Set arch BEFORE any torch imports (from top AMD fp8 submissions)
...
# Try load_inline with minimal kernel + ROCM_ARCH set
import torch
...
torch::Tensor noop(torch::Tensor x) { return x; }
"""
    _cpp_src = r"""
#include <torch/extension.h>
torch::Tensor noop(torch::Tensor x);
"""
    ...
    print("[INFO] load_inline compiled for gfx950!", file=sys.stderr)
except Exception as e:
    print(f"[INFO] load_inline failed: {e}", file=sys.stderr)
