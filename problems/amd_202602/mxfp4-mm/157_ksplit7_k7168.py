"""
MXFP4-MM: #157 - Balanced KSPLIT=7 + BLOCK_K=512 for K=7168 small-M.

Key findings from analysis:

1) KSPLIT IMBALANCE (current KSPLIT=8 is broken for K=7168):
   K=7168, BLOCK_K=256: 28 total K-blocks.
   28 / KSPLIT=8 = 3.5 (NOT integer!) → some CTAs do 4 iterations,
   others do 3.5 (partial last block). Partial block wastes MFMA work:
     Wasted work = (8*4 - 28) blocks / (8*4) = 4/32 = 12.5%

   KSPLIT=7: 28/7 = 4.0 (exactly integer). Zero wasted work.
   CTA count: 7 * (2112/64) = 7*33 = 231 CTAs (90% of 256 CUs).

2) BLOCK_K TOO SMALL (IREE issue #22309 finding):
   BLOCK_K=256 with matrix_instr_nonkdim=16 (16x16 MFMA, K=128 per inst):
     256/128 = 2 MFMA instructions per K-iteration
   IREE found this insufficient for pipeline hiding on gfx950; larger K
   tiles (K=512) give 4 MFMA instructions per iteration, better overlap
   with async memory loads.

   BLOCK_K=512, KSPLIT=7: K_per_split = 7168/7 = 1024.
   K-iterations per CTA = 1024/512 = 2 (exactly integer).
   Each iteration: 512/128 = 4 MFMA instructions → 2x more compute
   per pipeline stage versus BLOCK_K=256.

3) num_stages=2 instead of 3:
   With only 2 K-iterations, num_stages=3 means 3 prefetch buffers for
   2 iterations — the 3rd buffer is never used, wasting LDS. num_stages=2
   is sufficient and saves LDS for higher occupancy.

Combined: KSPLIT=7 (balanced split) + BLOCK_K=512 (better MFMA density)
+ num_stages=2 for the K=7168 M_LEQ_16 config.
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_ksplit7_o1'
...
_CONFIGS = {
    ...
    # K=7168 M_LEQ_16: balanced KSPLIT=7, larger BLOCK_K=512
    # KSPLIT=8 was imbalanced (28 K-blocks / 8 = 3.5, partial last block).
    # KSPLIT=7: 28/7=4 K-blocks each, perfectly balanced, zero wasted MFMA.
    # BLOCK_K=512: 4 MFMAs/iter vs 2 with BLOCK_K=256 (better pipeline overlap).
    "N=2112-K=7168": {
        "M_LEQ_16": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 512, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 7, "num_warps": 4, "num_stages": 2, "waves_per_eu": 2, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
