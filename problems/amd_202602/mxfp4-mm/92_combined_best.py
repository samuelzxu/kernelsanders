"""
MXFP4-MM: #92 - Combined best configs from experiments 53, 90, 91.
Key changes vs #53:
  K=2048: BSN=64 BSK=512 (confirmed -1.3µs in #90 and #91)
  K=7168: num_stages=3 waves_per_eu=2 (small gain from #91)
  All: torch.inference_mode() + output pre-alloc for fused path
  Keep: fused kernel at M<=16 only (M<=32 was worse in #91)
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # K=7168: Keep KSPLIT=8 (264 tiles). Try num_stages=3 (-0.3µs in #91)
    "N=2112-K=7168": {
    ...
    # K=2048: BSN=64 BSK=512 (confirmed -1.3µs in #90/#91)
    "N=7168-K=2048": {
    ...
    # K=1536: Keep original BSM=64 KSPLIT=2 (BSM=32 was worse in #91)
    "N=3072-K=1536": {
