...
#!POPCORN gpu MI355X
"""
#433: Patch the preshuffle kernel to add cache_modifier to A loads.
Currently only B loads have cache_modifier. Adding .cg to A loads
might improve L2 efficiency for A data.
"""
...
# Patch: add cache_modifier to A loads in the preshuffle kernel
# Original line 144: a_bf16 = tl.load(a_ptrs)
# Patched:           a_bf16 = tl.load(a_ptrs, cache_modifier=cache_modifier)
patched = original.replace(
    'a_bf16 = tl.load(a_ptrs)',
    'a_bf16 = tl.load(a_ptrs, cache_modifier=cache_modifier)',
    1  # Only replace the first occurrence (in the EVEN_K branch)
)

# Verify patch was applied
if patched != original:
    print("Patch applied: added cache_modifier to A load (EVEN_K branch)")
    # Also patch the non-EVEN_K branch
    # The non-EVEN_K load already has other args but no cache_modifier
    # Leave it as-is for now (it has mask+other args)
else:
    print("WARNING: Patch NOT applied!")
...
for _m,_n,_k in [(4,2880,512),(16,2112,7168),(32,4096,512),(32,2880,512),(64,7168,2048),(256,3072,1536)]:
    try:
        ...
        print(f"Warmup {_m}x{_n}x{_k} error: {e}")
