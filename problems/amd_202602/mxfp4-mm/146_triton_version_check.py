"""
MXFP4-MM: #146 - Check Triton version and try pip install newer version.
Also dump the actual optimize_module C function signature to understand
what parameters it really accepts.
"""
import os, sys

# Version check and C binding inspection
import triton
print(f"[INFO] Triton version: {triton.__version__}", file=sys.stderr)
print(f"[INFO] Triton path: {triton.__file__}", file=sys.stderr)

try:
    from triton._C import libtriton
    llvm = libtriton.llvm
    # Check available optimization levels
    for attr in dir(llvm):
        if 'OPTIMIZE' in attr or 'optimize' in attr:
            print(f"[INFO] llvm.{attr} = {getattr(llvm, attr)}", file=sys.stderr)
    # Check optimize_module signature
    print(f"[INFO] optimize_module: {llvm.optimize_module}", file=sys.stderr)
    if hasattr(llvm.optimize_module, '__doc__'):
        print(f"[INFO] optimize_module doc: {llvm.optimize_module.__doc__}", file=sys.stderr)
except Exception as e:
    print(f"[INFO] C binding inspection failed: {e}", file=sys.stderr)

# Check if pip install --user is possible
try:
    import subprocess
    result = subprocess.run(['pip', 'install', '--user', '--dry-run', 'triton==3.3.0'],
                          capture_output=True, text=True, timeout=30)
    print(f"[INFO] pip dry-run: {result.stdout[:200]}", file=sys.stderr)
    print(f"[INFO] pip dry-run stderr: {result.stderr[:200]}", file=sys.stderr)
except Exception as e:
    print(f"[INFO] pip check failed: {e}", file=sys.stderr)
