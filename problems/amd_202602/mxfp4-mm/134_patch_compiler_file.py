"""
MXFP4-MM: #134 - Directly patch AMD compiler.py to disable LSR+LICM.
Patch the actual source file to change:
  llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O3, options.arch, '', [], ...)
to:
  llvm.optimize_module(llvm_mod, llvm.OPTIMIZE_O3, options.arch, '', ['-disable-lsr', '-disable-machine-licm'], ...)
"""
import os, shutil, glob, sys

# Find and patch the AMD backend compiler.py
def _patch_compiler():
    # Common paths for Triton AMD backend
    search_paths = [
        '/usr/local/lib/python*/dist-packages/triton/backends/amd/compiler.py',
        '/usr/local/lib/python*/dist-packages/triton/third_party/amd/backend/compiler.py',
        '/home/runner/.local/lib/python*/dist-packages/triton/backends/amd/compiler.py',
    ]
        ...
        for fpath in glob.glob(pattern):
            try:
                with open(fpath, 'r') as f:
                    content = f.read()
                # Find the optimize_module call and inject flags
                ...
                if old in content:
                    ...
                    with open(fpath, 'w') as f:
                        f.write(content)
                    print(f"[PATCH] Patched {fpath}", file=sys.stderr)
                    # Clear cache
                    ...
                    print(f"[PATCH] Pattern not found in {fpath}", file=sys.stderr)
            except Exception as e:
                print(f"[PATCH] Error with {fpath}: {e}", file=sys.stderr)
    print("[PATCH] No compiler.py found to patch", file=sys.stderr)
    return False
