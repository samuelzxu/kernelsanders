...
#!POPCORN gpu MI355X
"""
Debug: verify graph replay is working. Count normal vs replay calls.
"""
...
_captured={};_cap_outs={};_ck=None;_cw=None;_cs=None
_normal_count=0;_replay_count=0

@torch.inference_mode()
def custom_kernel(data: input_t) -> output_t:
    global _ck,_cw,_cs,_normal_count,_replay_count
    ...
    if dp!=_ck:
        ...
        if key in _captured:del _captured[key]

    if key in _captured:
        _replay_count+=1
        if _replay_count<=3 or _replay_count%100==0:
            print(f"REPLAY {key} #{_replay_count}")
        _captured[key].replay()
        return _cap_outs[key]

    _normal_count+=1
    print(f"NORMAL {key} #{_normal_count} A@{A.data_ptr()}")
    result=gemm_a16wfp4_preshuffle(A,_cw,_cs,prequant=True,dtype=torch.bfloat16)

    # Capture after first normal call
    if key not in _captured:
        torch.cuda.synchronize()
        with _ctx(_q):
            for _ in range(3):gemm_a16wfp4_preshuffle(A,_cw,_cs,prequant=True,dtype=torch.bfloat16)
        torch.cuda.synchronize()
        g=torch.cuda.CUDAGraph()
        with _ctx(_q):
            g.capture_begin()
            co=gemm_a16wfp4_preshuffle(A,_cw,_cs,prequant=True,dtype=torch.bfloat16)
            g.capture_end()
        _captured[key]=g;_cap_outs[key]=co
