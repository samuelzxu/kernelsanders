"""
MXFP4-MM: #137 - BSN=64 KSPLIT=14 for K=7168 (inspired by leaderboard).
Competitor 'josusanmartin' at 9.35µs uses m16_bn64_ksplit14.
#126 tried BSN=32 KSPLIT=14 → 29µs (terrible). But BSN=64 is different:
- BSN=64 has better N-dimension locality (32KB vs 8KB B loads)
- 462 tiles (1.8 waves) vs our BSN=64 KSPLIT=7 (231 tiles, 0.9 waves)
- Each tile: 2 K-iters (vs 4 with KSPLIT=7)
- Trade: 14-way reduction vs 7-way reduction
"""
import json
import os
import triton
...
_CONFIGS = {
    ...
    # KEY: BSN=64 KSPLIT=14 (leaderboard-inspired)
    "N=2112-K=7168": {
        "M_LEQ_16": {"BLOCK_SIZE_M": 16, "BLOCK_SIZE_N": 64, "BLOCK_SIZE_K": 256, "GROUP_SIZE_M": 1, "NUM_KSPLIT": 14, "num_warps": 4, "num_stages": 2, "waves_per_eu": 1, "matrix_instr_nonkdim": 16, "cache_modifier": ".cg"},
