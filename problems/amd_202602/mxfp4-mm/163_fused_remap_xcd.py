"""
MXFP4-MM: #163 - Add remap_xcd to fused quant+GEMM kernel.

The aiter gemm_afp4wfp4 kernel already applies remap_xcd() before pid_grid()
for XCD-aware workgroup scheduling. Our custom fused quant+GEMM kernel was
missing this — workgroups were distributed round-robin across 8 XCDs, causing
B-matrix data to be loaded redundantly into each XCD's separate 4MB L2 cache.

With remap_xcd, consecutive pids are grouped onto the same XCD, so CTAs
computing adjacent N-tiles share B columns via L2 reuse within the chiplet.

For K=512, M=4, N=2880: grid = 45 CTAs → 5-6 per XCD (contiguous N-tiles).
For K=512, M=4, N=4096: grid = 64 CTAs → 8 per XCD.
Each XCD only needs to load its subset of B columns from HBM.
"""
...
def _patch_to_o1():
    try:
        ...
        if 'llvm.OPTIMIZE_O3' in content:
            dst_dir = '/tmp/triton_amd_remap_xcd_o1'
...
def _fused_quant_gemm_small_m(
    ...
    # XCD-aware workgroup remapping — match aiter's gemm_afp4wfp4 kernel
