"""
MXFP4-MM: preshuffled_scales with CORRECT scale format (ATOM pattern).
For M>=32: scales reshaped via .view(X//32, -1) — no unshuffle, no double quant!
For M<32: fused quant+GEMM kernel (same as #53).
"""
import json
import os
import sys
...
# Inject FP4 configs for M<=16 fused + large K
_CONFIGS = {
    "N=2880-K=512": {
...
_cache_key = None
_cache_val = None
_cache_bscale_fmt = None


def custom_kernel(data: input_t) -> output_t:
    global _cache_key, _cache_val, _cache_bscale_fmt
    ...
    # M < 32: fused quant+GEMM (same as #53)
    if m < 32:
    ...
    # M >= 32: try preshuffled_scales (ATOM pattern — no unshuffle, no double quant)
    A_q, A_scale = dynamic_mxfp4_quant(A)
    A_scale_sh = e8m0_shuffle(A_scale)

    # Reshape shuffled scales to (X//32, K) format via simple .view()
    ...
    try:
        return gemm_afp4wfp4_preshuffled_scales(
            A_q, B_q_uint8, A_scale_fmt, B_scale_fmt,
            dtype=torch.bfloat16,
        )
    except Exception as e:
        print(f"[WARN] preshuffled_scales failed: {e}", file=sys.stderr)
