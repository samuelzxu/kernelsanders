"""
MXFP4-MM: #99 - Use gemm_afp4wfp4_preshuffle for M>16 shapes.
The preshuffle variant uses B_shuffle (N//16, K*16) directly with optimized
memory access patterns. Requires converting scales to preshuffle format.
- B_scale: e8m0_unshuffle(B_scale_sh) → shuffle_scales → (N//32, K)
- A_scale: For M>=32: shuffle_scales(A_scale) → (M//32, K)
            For M<32: keep standard (M, K//32)
"""
import json
import os
import triton
...
# AFP4WFP4 configs for fused kernel path (M<=16 KSPLIT=1)
_CONFIGS = {
    "N=2880-K=512": {
...
_bscale_cache_std = None  # standard (N, K//32) for fused kernel
_bscale_cache_ps = None   # preshuffle (N//32, K) for preshuffle kernel
_out_cache = {}


@torch.inference_mode()
def custom_kernel(data: input_t) -> output_t:
    global _bscale_cache_key, _bscale_cache_std, _bscale_cache_ps
    ...
    # For M<=16 with KSPLIT=1: use fused kernel (needs standard B_scale)
    config, _ = _get_config(m, n, k)
    if m <= 16 and config.get("NUM_KSPLIT", 1) == 1:
        # Get standard B_scale
        if key == _bscale_cache_key:
            B_scale = _bscale_cache_std
        else:
            if k <= 512:
            ...
            _bscale_cache_std = B_scale
            _bscale_cache_ps = None  # invalidate preshuffle cache
    ...
    # For other shapes: use preshuffle variant with B_shuffle
    # Get preshuffle B_scale (N//32, K)
    if key == _bscale_cache_key and _bscale_cache_ps is not None:
        B_scale_ps = _bscale_cache_ps
    else:
        # First get standard scale
        if k <= 512:
            _, B_scale_std = dynamic_mxfp4_quant(B)
        else:
            B_scale_std = e8m0_unshuffle(B_scale_sh.view(torch.uint8), n, k // 32)
        # Then shuffle to preshuffle format
        B_scale_ps = shuffle_scales(B_scale_std)
        _bscale_cache_key = key
        _bscale_cache_std = B_scale_std
        _bscale_cache_ps = B_scale_ps

    # Quant A
    A_q, A_scale = dynamic_mxfp4_quant(A)

    # Shuffle A_scale if M>=32 (preshuffle kernel expects shuffled A_scale for BSM>=32)
    if m >= 32:
        A_scale_ps = shuffle_scales(A_scale.view(torch.uint8))
    else:
        A_scale_ps = A_scale.view(torch.uint8)

    # B_shuffle is in fp4x2 dtype - must view as uint8 for Triton
