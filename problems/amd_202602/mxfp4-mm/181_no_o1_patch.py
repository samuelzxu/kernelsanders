"""
MXFP4-MM: #181 - No O1 patch, no cache clearing. Pure O3 compilation.

Tests whether the O3→O1 patch is still needed. The patch was added early
to fix "slower or incorrect code" from O3, but subsequent kernel changes
may have resolved the underlying issue. O3 produces faster code than O1
for Triton kernels in general.

If this passes correctness AND is faster, we get both:
  1. Warm cache (no shutil.rmtree)
  2. Better-optimized compiled code (O3 vs O1)
"""
import json
import os
import triton
