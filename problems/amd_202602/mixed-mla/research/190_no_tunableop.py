"""
MLA decode - torch.compile only, NO TunableOp.

Hypothesis: TunableOp may be finding suboptimal kernels on recent runs,
or the tuning warmup itself is polluting the GPU state for subsequent
assembly calls. Test compile-only to isolate TunableOp's effect.

torch.compile still provides kernel fusion for GEMM path.
"""
