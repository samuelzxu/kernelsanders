"""
MLA decode - kvg=32 for ALL assembly paths.

Attempt 160 showed kvg=32 beats kvg=64 for kv>1024 by 2-3µs.
Now try kvg=32 for kv<=1024 as well (was kvg=16).

kv=1024/kvg=32: 32 groups per batch vs 64 (kvg=16)
Each CTA with nks=8: 128/32=4 groups per CTA (vs 8 with kvg=16)
Fewer groups but still reasonable load balancing.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        # kvg=32: fewer groups than kvg=16, less metadata overhead
        kvg, nks = 32, 8
    else:
        q_input, q_scale = quantize_fp8_triton(q)
        qd = FP8_DTYPE
        # kvg=32: proven better than kvg=64 in attempt 160
        kvg, nks = 32, 32
