"""
MLA decode - honest, optimized operation order.
All allocations first, then GPU kernels back-to-back.
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Phase 1: ALL CPU allocations upfront (CUDA pool, no GPU kernels)
    ...
    klp = torch.empty(bs, dtype=torch.int32, device="cuda")
    kvi = torch.empty(n, dtype=torch.int32, device="cuda")
    ...
    # Phase 2: Q quantization (GPU kernels, only for kv>1024)
    if kv_seq_len <= 1024:
        q_input, q_scale = q, None
    else:
        q_input, q_scale = quantize_fp8_triton(q)

    # Phase 3: GPU kernels back-to-back (no CPU work between them)
    klp.fill_(1)
    torch.arange(n, dtype=torch.int32, device="cuda", out=kvi)
