"""
MLA decode - compiled Q quantization + higher tuning budget.

New: compile the Q quantization function for the a8w8 path.
The quantize_fp8_triton function calls a Triton kernel — torch.compile
can optimize the surrounding tensor operations (view, empty_like, zeros).

Also: even higher TunableOp iterations (200) and warmup (100ms).
"""
...
os.environ.setdefault('PYTORCH_TUNABLEOP_MAX_TUNING_ITERATIONS', '200')
os.environ.setdefault('PYTORCH_TUNABLEOP_MAX_WARMUP_DURATION_MS', '100')
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        # Inline Q quantization (avoid function call overhead)
        t2d = q.view(-1, DQ)
