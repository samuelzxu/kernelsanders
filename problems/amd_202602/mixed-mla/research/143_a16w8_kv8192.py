"""
MLA decode - GEMM for small cases, a16w8 assembly for ALL assembly paths.

Key change: use a16w8 (bf16 Q + fp8 KV) for kv>1024 too, eliminating Q quantization.
Q quantization (Triton kernel) adds ~2-3µs overhead per call. With a16w8:
- No Triton quantization kernel launch
- Q data is tiny vs KV (4.5MB bf16 vs 1.2GB fp8 for bs=256)
- a16w8 handles any kv length (same underlying computation)

Also: nks=16 for kv<=1024 (was 8)
- bs=64: 64*16=1024 CTAs = 4 waves (vs 2 waves with nks=8)
- bs=256: 256*16=4096 CTAs = 16 waves (vs 8 waves)
- More waves = better HBM latency hiding (empirically validated)
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Always use a16w8: bf16 Q + fp8 KV, no Q quantization
    # Q is tiny vs KV — no meaningful bandwidth penalty
    q_input, q_scale, qd = q, None, BF16

    if kv_seq_len <= 1024:
        # nks=16: more waves for better HBM latency hiding
        # bs=64: 64*16=1024 CTAs = 4 waves (vs 2 with nks=8)
        # bs=256: 256*16=4096 CTAs = 16 waves (vs 8 with nks=8)
        kvg, nks = 16, 16
    else:
        # kvg=64, nks=32: empirically optimal for kv>1024
        kvg, nks = 64, 32
