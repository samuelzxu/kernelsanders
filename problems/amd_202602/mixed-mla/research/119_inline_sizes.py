"""
MLA decode - GEMM for tiny cases, assembly for the rest.
torch.bmm for bs<=4/kv<=1024 (no metadata overhead).
a16w8/a8w8 assembly for larger cases.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 32 and kv_seq_len <= 1024:
        # GEMM path: 3 kernels (QK^T + softmax + attn@V), no metadata
        kv = kv_data["bf16"].view(bs, kv_seq_len, DQ)
        q_3d = q.view(bs, NH, DQ)

        # QK^T with fused scale via baddbmm
        scores = torch.baddbmm(
            torch.empty((bs, NH, kv_seq_len), dtype=BF16, device=q.device),
            q_3d, kv.transpose(-2, -1), beta=0, alpha=SM_SCALE)
        attn = F.softmax(scores, dim=-1, dtype=FP32).to(BF16)
        return torch.bmm(attn, kv[:, :, :DV])
    ...
    kvd = kv_buffer.dtype
    # Inline metadata buffer sizes (intra_batch_mode=True, NH=16, q_seq=1)
    cu = 256  # MI355X CU count
    wm = torch.empty(2, dtype=torch.uint64, device="cuda")
    wi = torch.empty(cu + 1, dtype=torch.int32, device="cuda")
    wis = torch.empty(bs * nks, 8, dtype=torch.int32, device="cuda")
    ri = torch.empty(bs + 1, dtype=torch.int32, device="cuda")
    rfm = torch.empty(bs, 2, dtype=torch.int32, device="cuda")
    rpm = torch.empty(bs * nks, dtype=torch.int32, device="cuda")
