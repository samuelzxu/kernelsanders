"""
MLA decode - TunableOp for GEMM + torch.compile for kv<=1024 GEMM.

PYTORCH_TUNABLEOP_ENABLED=1 auto-tunes all GEMM calls (baddbmm, bmm)
through rocBLAS and hipBLASLt solution search. First call benchmarks
all available algorithms, cached for subsequent calls.

Combined with torch.compile for kv<=1024 GEMM from attempt 153.
TunableOp may also improve the uncompiled bs=4/kv=8192 GEMM path.
"""
...
# Compile for kv<=1024 GEMM (fused kernel launches)
