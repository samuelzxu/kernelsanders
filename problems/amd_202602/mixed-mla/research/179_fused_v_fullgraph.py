"""
MLA decode - V slice inside compiled fn + fullgraph=True.

Two changes:
1. Move kv[:,:,:DV] slice INSIDE compiled function (pass kv not v).
   Inductor can potentially fuse the V slice with the bmm operation,
   avoiding a separate non-contiguous tensor.

2. fullgraph=True: forces single graph without breaks, giving the
   compiler maximum optimization freedom. Never tried before.
"""
...
def _gemm_attn_fused_v(q_3d, kv_t, kv):
    # V slice inside compiled fn: Inductor can fuse with bmm
    v = kv[:, :, :DV]
    bs = q_3d.shape[0]
    kv_len = kv_t.shape[2]
...
# fullgraph=True: single graph, max optimization
_compiled_gemm_short = torch.compile(_gemm_attn_fused_v, fullgraph=True)
_compiled_gemm_long = torch.compile(_gemm_attn_fused_v, fullgraph=True)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 4 or (bs <= 32 and kv_seq_len <= 1024):
        ...
        if kv_seq_len <= 1024:
            return _compiled_gemm_short(q_3d, kv_t, kv)
        else:
            return _compiled_gemm_long(q_3d, kv_t, kv)
