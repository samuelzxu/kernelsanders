"""
MLA decode - test kvg=32 for kv>1024 (never tested before).

kvg=16: too many groups (confirmed worse in attempt 145, +9µs)
kvg=64: current best
kvg=128: slightly worse (attempt 146, +1µs)
kvg=32: untested middle ground — might offer better load balancing than 64
while less metadata overhead than 16.

With kvg=32/kv=8192: 256 groups per batch item (vs 128 with kvg=64)
Each CTA with nks=32: 256 tokens / 32 = 8 groups per CTA
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        qd = FP8_DTYPE
        # Try kvg=32: 8 groups per CTA (between kvg=16's 16 and kvg=64's 4)
        kvg, nks = 32, 32
