...
Per-batch-size nks targeting 1 full wave (256 CTAs) for kv>1024:
- bs=32: nks=8  → 32*8=256 CTAs = 1 wave (optimal, zero tail effect)
- bs=64: nks=4  → 64*4=256 CTAs = 1 wave (vs nks=8 in 138: 512=2 waves)
- bs=256: nks=4 → 256*4=1024 CTAs = 4 waves (vs nks=8 in 138: 2048=8 waves)

kv<=1024 path: nks=8 (unchanged, safer — not in correctness tests but keep stable)

Additional reduce buffer savings vs approach 138 (nks=8 flat):
- bs=64/kv=8192: lg 16MB→8MB, saves 16MB reads+writes = ~3µs
- bs=256/kv=8192: lg 67MB→34MB, saves 66MB reads+writes = ~12µs
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        # a16w8 path: bf16 Q + fp8 KV, no Q quantization
        # Keep nks=8 for safety (kv<=1024 configs not in correctness tests)
        q_input, q_scale, qd = q, None, BF16
        kvg, nks = 16, 8
    else:
        # a8w8 path: target 1 wave = 256 total CTAs on MI355X
        # nks = max(4, 256//bs) ensures:
        # - bs=32: nks=8 → 256 CTAs = 1 wave
        # - bs=64: nks=4 → 256 CTAs = 1 wave
        # - bs=256: nks=4 → 1024 CTAs = 4 waves (conservative, big reduce savings)
        # Each CTA handles kv_seq/nks tokens with kvg=64 groups
