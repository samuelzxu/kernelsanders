"""
MLA decode - honest, quantize-first ordering.
Q quantization launches first (GPU busy), then CPU allocates during GPU work.
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Launch Q quantization FIRST (GPU busy while CPU does allocations)
    if kv_seq_len <= 1024:
        q_input, q_scale, qd, kvg = q, None, BF16, 16
    else:
        q_input, q_scale = quantize_fp8_triton(q)  # 2 GPU kernels launched
        qd, kvg = FP8_DTYPE, 64

    # CPU: compute metadata sizes + allocate (while GPU does quant)
    np_ = bs * nks
    ...
    # GPU: fill + metadata + stage1 + reduce (back to back)
