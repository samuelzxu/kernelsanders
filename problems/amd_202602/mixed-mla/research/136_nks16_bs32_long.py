...
baddbmm for bs<=32/kv<=1024 (fused scale, no metadata).
a16w8/a8w8 assembly for larger cases.

Change vs 132: try nks=16 for bs=32/kv>1024 (instead of 32).
- bs=32/kv>1024: nks=16 -> 32*16=512 items = 2 waves (was 1024 = 4 waves with nks=32)
- Each CTA handles 8192/16=512 tokens (vs 256 with nks=32) -> more work per CTA
- More tokens per CTA -> each CTA issues more HBM requests -> better bandwidth utilization?
- Fewer CTA launches -> less scheduling overhead, smaller reduce step (512 vs 1024 items)
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        qd, kvg = FP8_DTYPE, 64
        # bs<=4: nks=64 (256 work items = 1 wave, maximizes GPU utilization)
        # bs<=32: nks=16 (512 items = 2 waves, more tokens/CTA vs nks=32's 4 waves)
        # bs>32: nks=32 (unchanged, good occupancy)
        if bs <= 4:
            nks = 64
        elif bs <= 32:
            nks = 16
