"""
MLA decode - HIP quantization instead of Triton for Q fp8 quantization.

aiter has two per-tensor quant implementations:
1. Triton: dynamic_per_tensor_quant_fp8_i8 (current) — uses torch.zeros for scale
2. HIP: aiter.dynamic_per_tensor_quant (pre-compiled) — uses torch.empty for scale

HIP version potentially faster:
- No Triton JIT overhead (pre-compiled HIP/C++ kernel)
- Uses torch.empty for scale (saves zero-fill kernel ~0.5µs)
- Single fused HIP kernel vs Triton dispatch
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        # HIP quant: pre-compiled kernel + torch.empty for scale (no zero-fill)
        t2d = q.view(-1, DQ)
