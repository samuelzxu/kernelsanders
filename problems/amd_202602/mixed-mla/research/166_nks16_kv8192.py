"""
MLA decode - test nks=16 for kv>1024 (never tested before!).

Previous tests for kv>1024:
- nks=8: WORSE (+10-15µs, too few waves for HBM bandwidth)
- nks=32: current best
- nks=16: UNTESTED — might find a sweet spot

nks=16 halves the reduce buffer vs nks=32:
- lg = bs*16*16*512*4 (vs bs*32*16*512*4) — 50% less data movement in reduce
- But 50% fewer waves — the question is whether bandwidth saturates earlier

For bs=256/kv=8192: nks=16 → 4096 CTAs = 16 waves (vs 32 waves with nks=32)
16 waves should still saturate HBM bandwidth (~90%+ of peak).
Reduce savings: 134MB → 67MB = 25µs saved.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        qd = FP8_DTYPE
        # TEST: nks=16 for kv>1024 (halves reduce buffer, 50% fewer waves)
        kvg, nks = 32, 16
