"""
MLA decode - torch.compile for kv<=1024 GEMM only, original for kv>1024.

Discovery from attempt 152: torch.compile helps kv=1024 GEMM configs
(-4.7µs for bs=4, -4.4µs for bs=32) but hurts kv=8192 GEMM (+5.4µs).
Dynamic shape compilation is suboptimal for large N=8192 GEMM.

Fix: only use compiled path for kv<=1024 GEMM, keep original baddbmm for kv>1024.
...
# Compile ONLY for kv<=1024 GEMM (kv=8192 GEMM is worse when compiled)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 4 or (bs <= 32 and kv_seq_len <= 1024):
        ...
        if kv_seq_len <= 1024:
            # Compiled path: fused kernel launches for short kv
