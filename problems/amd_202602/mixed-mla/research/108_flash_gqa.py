"""
MLA decode - flash attention with GQA support.
Uses F.scaled_dot_product_attention with enable_gqa=True.
No expand/broadcast needed - kernel handles GQA internally.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        # Flash attention path: single fused kernel, no metadata
        kv = kv_data["bf16"]  # (total_kv, 1, 576) bf16
        kv_3d = kv.view(bs, kv_seq_len, DQ)  # (bs, kv_len, 576)

        # Q: (bs, 1, NH, DQ) -> need (bs, NH, 1, DQ)
        q_4d = q.view(bs, 1, NH, DQ).transpose(1, 2)  # (bs, NH, 1, DQ)

        # K: (bs, kv_len, DQ) -> (bs, NKV, kv_len, DQ)
        k_4d = kv_3d.unsqueeze(1)  # (bs, 1, kv_len, DQ)

        # V: (bs, kv_len, DV) -> (bs, NKV, kv_len, DV)
        v_4d = kv_3d[:, :, :DV].unsqueeze(1)  # (bs, 1, kv_len, DV)

        # GQA-aware SDPA: handles 16:1 head ratio without expand
        try:
            ...
            # Fallback if enable_gqa not supported
            k_4d = k_4d.expand(-1, NH, -1, -1)
            v_4d = v_4d.expand(-1, NH, -1, -1)
            out = F.scaled_dot_product_attention(
                q_4d, k_4d, v_4d,
                scale=SM_SCALE, is_causal=False
            )
        ...
        # Persistent a8w8 for long sequences
