...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 64 and kv_seq_len <= 1024:
        # GEMM path: 3 kernels (QK^T + softmax + attn@V), no metadata
        kv = kv_data["bf16"].view(bs, kv_seq_len, DQ)
        q_3d = q.view(bs, NH, DQ)

        # QK^T via matmul (bf16, no conversion)
        scores = torch.matmul(q_3d, kv.transpose(-2, -1)) * SM_SCALE  # (bs, NH, kv_len)
        attn = F.softmax(scores, dim=-1, dtype=FP32).to(BF16)
        out = torch.matmul(attn, kv[:, :, :DV])  # (bs, NH, DV)
        return out
