...
Uniform kvg=16 and nks=32 for all assembly paths (matching reference config).
Plus a16w8 (no Q quantization) and GEMM for small cases.

Discovery: the reference uses kvg=16 and nks=32 for ALL configs.
Our kvg=64 for kv>1024 may have been suboptimal — the assembly kernel
was designed/tuned with kvg=16.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # a16w8 for all assembly: bf16 Q + fp8 KV (skip Q quantization)
    q_input, q_scale, qd = q, None, BF16

    # Uniform config matching reference: kvg=16, nks=32 for all assembly paths
    kvg, nks = 16, 32
