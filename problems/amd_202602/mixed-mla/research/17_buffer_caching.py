...
# ---------------------------------------------------------------------------
# Buffer cache — reuse allocations across calls with same (batch_size, num_kv_splits)
# ---------------------------------------------------------------------------
_buf_cache: dict[tuple[int, int], list[torch.Tensor]] = {}
_kv_indices_cache: dict[int, torch.Tensor] = {}
_output_cache: dict[int, torch.Tensor] = {}


def get_num_kv_splits(batch_size: int, kv_seq_len: int) -> int:
    """Tune num_kv_splits based on workload size."""
    _ = kv_seq_len
    if batch_size <= 4:
        return 16
    else:
...
# ---------------------------------------------------------------------------
# Persistent mode metadata helper with buffer caching
# ---------------------------------------------------------------------------

def _get_or_alloc_buffers(batch_size: int, num_kv_splits: int, q_dtype, kv_dtype):
    """Get cached work buffers or allocate new ones."""
    ...
    if key not in _buf_cache:
        ...
        _buf_cache[key] = [torch.empty(s, dtype=t, device="cuda") for s, t in info]
...
def _make_mla_decode_metadata(
    ...
    """Populate work buffers for persistent mla_decode_fwd (cached allocation)."""
    bufs = _get_or_alloc_buffers(batch_size, num_kv_splits, q_dtype, kv_dtype)
...
def _get_kv_indices(total_kv_len: int) -> torch.Tensor:
    """Get cached kv_indices or create new ones."""
...
def _get_output_buf(total_q: int) -> torch.Tensor:
    """Get cached output buffer or allocate new one."""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Reshape kv_buffer to 4D for aiter
    kv_buffer_4d = kv_buffer_fp8.view(total_kv_len, PAGE_SIZE, NUM_KV_HEADS, QK_HEAD_DIM)

    kv_last_page_len = (kv_indptr[1:] - kv_indptr[:-1]).to(torch.int32)

    # Tune num_kv_splits based on workload
    num_kv_splits = get_num_kv_splits(batch_size, kv_seq_len)

    # Build persistent-mode metadata (cached buffer allocation)
