...
a16w8 for short seq, a8w8 for long seq.
Uses aiter's Triton quantization (2 kernel launches vs 3-4 for PyTorch).
"""
...
def quantize_fp8_fast(tensor):
    """Use aiter's Triton quant (2 kernel launches vs 3-4 for PyTorch)."""
    shape = tensor.shape
    ...
    dynamic_per_tensor_quant_fp8_i8(qx, tensor_2d, scale)
    return qx.view(shape), scale


def quantize_fp8_torch(tensor):
    """Fallback PyTorch quantization."""
...
# Try fast quant, fall back to torch
_use_fast_quant = True


@torch.inference_mode()
def custom_kernel(data: input_t) -> output_t:
    global _use_fast_quant
    q, kv_data, qo_indptr, kv_indptr, cfg = data
    bs = cfg["batch_size"]

    kv_seq_len = cfg.get("kv_seq_len", 1024)
    if kv_seq_len <= 1024:
        # a16w8: bf16 Q + fp8 KV (skip Q quantization)
        ...
        if _use_fast_quant:
            try:
                ...
                _use_fast_quant = False
                q_input, q_scale = quantize_fp8_torch(q)
        else:
            q_input, q_scale = quantize_fp8_torch(q)
