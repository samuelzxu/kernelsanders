"""
MLA decode - hybrid persistent/non-persistent.
Non-persistent bf16 for kv<=1024 (no metadata, no quant, ~25us).
Persistent a8w8+Triton for kv>1024 (correct for long sequences).
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024 and bs <= 4:
        # NON-PERSISTENT bf16: no metadata, no quant, ~15us faster
        # Uses qSeqLen=1 optimized kernel (mla_dec_stage1_bf16_a16w16_subQ16_mqa16)
        kv_buffer = kv_data["bf16"]
        n = kv_buffer.shape[0]
        kvi = torch.arange(n, dtype=torch.int32, device="cuda")
        klp = torch.ones(bs, dtype=torch.int32, device="cuda")
        o = torch.empty((bs, NH, DV), dtype=BF16, device="cuda")

        mla_decode_fwd(
            q.view(-1, NH, DQ),
            kv_buffer.view(n, 1, NKV, DQ),
            o, qo_indptr, kv_indptr, kvi, klp,
            1, page_size=1, nhead_kv=NKV, sm_scale=SM_SCALE,
            logit_cap=0.0, num_kv_splits=16 if bs <= 4 else 32,
            intra_batch_mode=True)
        return o

    else:
        # PERSISTENT: a16w8 for kv<=1024, a8w8 for kv>1024
        kv_buffer, kv_scale = kv_data["fp8"]
        n = kv_buffer.shape[0]
        nks = 16 if bs <= 4 else 32

        if kv_seq_len <= 1024:
            q_input, q_scale, qd, kvg = q, None, BF16, 16
        else:
            q_input, q_scale = quantize_fp8_triton(q)
            qd, kvg = FP8_DTYPE, 64

        info = get_mla_metadata_info_v1(
            bs, 1, NH, qd, FP8_DTYPE, is_sparse=False, fast_mode=False,
        ...
        get_mla_metadata_v1(
            ...
            intra_batch_mode=True, dtype_q=qd, dtype_kv=FP8_DTYPE)
