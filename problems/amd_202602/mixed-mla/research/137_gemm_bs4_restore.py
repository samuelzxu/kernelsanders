...
Restore GEMM for bs<=4 (all kv lengths):
- bs<=4: GEMM (baddbmm) regardless of kv_seq_len
- bs<=32, kv<=1024: GEMM
- Otherwise: assembly (a16w8 for kv<=1024, a8w8 for kv>1024)

Key insight: for bs=4/kv=8192, GEMM wins over assembly because:
- 37.7MB bf16 KV fits in L2 cache (64MB on MI355X)
- GEMM has no fixed overhead: no Q quant, no metadata, no reduce step
- Assembly overhead: ~15µs (Q quant + metadata + reduce) even for small batches
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # For bs<=4: even kv=8192 (37.7MB) fits in L2 cache (64MB MI355X)
    # GEMM avoids all assembly overhead: no Q quant, metadata, or reduce step
