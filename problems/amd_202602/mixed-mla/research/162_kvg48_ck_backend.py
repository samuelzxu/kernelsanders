"""
MLA decode - kvg=48 for kv>1024 + CK-only GEMM backend.

Two changes:
1. kvg=48: 6 groups/CTA (between kvg=32's 8 and kvg=64's 4)
   The assembly kernel supports ANY integer >= 16, not just powers of 2.
   kvg=48 is explicitly listed in aiter's get_block_n_fp8 table.

2. TORCHINDUCTOR_MAX_AUTOTUNE_GEMM_BACKENDS="CK": forces Composable Kernel
   backend for GEMM, avoiding Triton JIT for GEMM ops. This may reduce
   the ~3-5µs assembly regression from torch.compile's Triton initialization.
"""
...
os.environ.setdefault('TORCHINDUCTOR_MAX_AUTOTUNE_GEMM_BACKENDS', 'CK')
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        qd = FP8_DTYPE
        # kvg=32: proven best in attempt 160 (8 groups/CTA)
        # CK-only GEMM backend should reduce assembly regression from torch.compile
        kvg, nks = 32, 32
