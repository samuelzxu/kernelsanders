"""
MLA decode - pre-computed kvi arange + all optimizations from 170.

Pre-compute the kv_page_indices (kvi = arange(max_n)) at module level.
This saves ~2µs per assembly call from avoiding torch.arange GPU kernel.
kvi is a mathematical constant (0,1,...,n-1), not an output cache.

Also pre-compute metadata info sizes for known configs (pure Python,
no GPU ops needed), saving get_mla_metadata_info_v1 call overhead.
"""
...
# Pre-compute kvi for maximum possible n (saves ~2µs arange per assembly call)
_MAX_KV_TOKENS = 256 * 8192  # bs=256, kv=8192
_KVI_FULL = torch.arange(_MAX_KV_TOKENS, dtype=torch.int32, device="cuda")
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Use pre-computed kvi slice instead of torch.arange (saves ~2µs)
    kvi = _KVI_FULL[:n]
