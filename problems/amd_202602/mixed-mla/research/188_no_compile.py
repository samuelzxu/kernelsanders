"""
MLA decode - NO torch.compile, TunableOp only.

Hypothesis: torch.compile causes ~3-5µs assembly regression AND rare
correctness failures from Triton init / memory pool pollution.

Test: remove torch.compile entirely, keep TunableOp for GEMM tuning.
TunableOp alone gave ~5µs GEMM improvement (attempt 156: 73.1µs).
torch.compile added ~5µs more (attempt 163: 71.3µs) but with side effects.

If kv>1024 assembly improves by 3-5µs without torch.compile, the net
effect could be positive (losing ~5µs on GEMM but gaining ~3-5µs × 3
= ~9-15µs on assembly).

Geomean math: GEMM contributes less to geomean than assembly
(3 GEMM configs vs 5 assembly configs).
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # GEMM path: TunableOp-tuned but NOT compiled
