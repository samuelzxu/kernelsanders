"""
MLA decode - pre-computed kvi for kv<=1024 ONLY.

Attempt 172 showed pre-computed kvi dramatically helps kv<=1024 assembly:
- bs=64/kv=1024: 50.6 → 46.7µs (-3.9µs!)
- bs=256/kv=1024: 113 → 108µs (-5µs!)

But the 8MB _KVI_FULL tensor caused kv>1024 regression (+4-8µs) from
memory pressure. Fix: only pre-compute for kv<=1024 (1MB tensor).
kv>1024 uses fresh torch.arange (no memory pressure).
"""
...
# Pre-compute kvi for kv<=1024 configs only (1MB, no memory pressure)
# max n for kv<=1024: bs=256 * kv=1024 = 262144
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        kvg, nks = 32, 8
        kvi = _KVI_SHORT[:n]  # Pre-computed slice (no GPU kernel launch)
    else:
        t2d = q.view(-1, DQ)
        ...
        kvg, nks = 32, 32
        kvi = torch.arange(n, dtype=torch.int32, device="cuda")  # Fresh alloc
