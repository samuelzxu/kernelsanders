...
Changes vs 132:
- nks=64 for bs<=4/kv>1024 (same)
- Merge lg+ls into single FP32 allocation (saves 1 GPU alloc call)
  lg: (np_, 1, NH, DV) = np_*NH*DV fp32 elements
  ls: (np_, 1, NH, 1)  = np_*NH fp32 elements
  Combined: buf of size np_*NH*(DV+1), lg at front, ls at back
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Merge lg and ls into a single FP32 allocation (saves 1 GPU alloc)
    # lg: (np_, 1, NH, DV) -> np_*NH*DV elements at offset 0
    # ls: (np_, 1, NH, 1)  -> np_*NH elements after lg
    lgls_buf = torch.empty(np_ * NH * (DV + 1), dtype=FP32, device="cuda")
    lg = lgls_buf[:np_ * NH * DV].view(np_, 1, NH, DV)
    ls = lgls_buf[np_ * NH * DV:].view(np_, 1, NH, 1)
