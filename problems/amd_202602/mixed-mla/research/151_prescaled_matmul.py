"""
MLA decode - Optimized GEMM path with pre-scaled Q.

Instead of baddbmm(empty, Q, K^T, beta=0, alpha=SM_SCALE),
use matmul(Q*SM_SCALE, K^T) which:
1. Pre-scales Q (tiny: 16*576 per batch = negligible)
2. Uses torch.matmul instead of baddbmm (potentially different, faster kernel)
3. Avoids the empty tensor allocation for baddbmm output

Also uses torch.bmm for Q*scale instead of element-wise multiply.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # GEMM path: pre-scaled Q + matmul instead of baddbmm
    if bs <= 4 or (bs <= 32 and kv_seq_len <= 1024):
        kv = kv_data["bf16"].view(bs, kv_seq_len, DQ)
        # Pre-scale Q: tiny operation (bs*NH*DQ elements)
        q_scaled = q.view(bs, NH, DQ) * SM_SCALE
        # matmul dispatches differently than baddbmm — might select a faster kernel
        s = torch.bmm(q_scaled, kv.transpose(-2, -1))
        return torch.bmm(F.softmax(s, dim=-1, dtype=FP32).to(BF16), kv[:, :, :DV])
