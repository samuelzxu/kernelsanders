...
Key change vs current best: nks=64 for bs<=4/kv>1024.
- bs=4/kv>1024: 4*64=256 work items = 1 full wave on MI355X (256 CUs)
- Previous: 4*16=64 work items = only 25% GPU utilization
- Expect significant improvement for bs=4/kv=8192 config
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        qd, kvg = FP8_DTYPE, 64
        # bs<=4: nks=64 gives 4*64=256 work items = exactly 1 wave on MI355X (256 CUs)
        # This maximizes GPU utilization for the small-batch long-sequence case.
        # bs>4: nks=32 is sufficient (32*32=1024+ items = multiple waves)
