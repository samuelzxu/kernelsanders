...
Changes vs 132:
- nks=64 for bs<=4/kv>1024 (fills 1 wave on MI355X)
- kvg=128 for kv>1024 (was 64): halves metadata entries, faster metadata computation
  With kvg=128 and nks=64 for bs=4: 4*64=256 CTAs, 4*64=256 groups -> 1:1 mapping
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        qd = FP8_DTYPE
        # kvg=128: 8192/128=64 groups per batch item (vs 128 groups with kvg=64)
        # Fewer metadata entries -> faster metadata computation
        kvg = 128
        # bs<=4: nks=64 -> 4*64=256 work items = exactly 1 wave on MI355X (256 CUs)
        # With kvg=128: 4*64=256 groups, 256 CTAs -> 1:1 mapping (optimal balance)
