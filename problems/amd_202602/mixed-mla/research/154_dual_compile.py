"""
MLA decode - dual torch.compile for GEMM, assembly for the rest.

Two compiled functions:
1. _compiled_short for kv<=1024 (bs=4,32 × kv=1024)
2. _compiled_long for kv>1024  (bs=4 × kv=8192)
Both use dynamic=True for handling different batch sizes.

From attempt 152: compiled kv=8192 was 47.9µs (vs uncompiled 42.5µs).
Hypothesis: separate compilation for kv=8192 with its own optimization
might avoid the dynamic shape penalty.
"""
...
_compiled_gemm = torch.compile(_gemm_attn, dynamic=True)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Compiled GEMM path for ALL GEMM cases
    if bs <= 4 or (bs <= 32 and kv_seq_len <= 1024):
        ...
        return _compiled_gemm(q_3d, kv_t, v, scores_buf)
