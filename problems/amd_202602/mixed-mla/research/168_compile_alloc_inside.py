"""
MLA decode - move scores_buf allocation INSIDE compiled function.

When torch.compile traces a function, internal allocations can be
optimized away (reusing cached memory pools). By moving the scores_buf
allocation inside the compiled function, the compiler can potentially:
1. Reuse the same buffer across calls (no re-allocation)
2. Fuse the allocation with the first GEMM kernel
3. Use the HIP memory pool more efficiently

Also: try kvg=32 for kv<=1024 a16w8 path in isolation (never done in
isolation — attempt 161 combined with kv>1024 changes).
"""
...
def _gemm_attn_self_alloc(q_3d, kv_t, v):
    # Allocation INSIDE compiled function — compiler can optimize this
    bs = q_3d.shape[0]
    kv_len = kv_t.shape[2]
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        # Test kvg=32 for kv<=1024 in isolation
        kvg, nks = 32, 8
    else:
