"""
MLA decode - reduce-overhead torch.compile + TunableOp for GEMM.

Change vs 156: use mode="reduce-overhead" for torch.compile.
This uses HIP graphs to batch kernel launches, potentially eliminating
the remaining ~10µs dispatch overhead in the compiled GEMM path.

Risk: HIP graphs require fixed tensor shapes. Using dynamic=False
means separate compilations for bs=4 and bs=32 (two graphs captured
during warmup, replayed in ranked).
"""
...
# reduce-overhead: uses HIP graphs for minimal dispatch overhead
_compiled_short = torch.compile(_gemm_attn_short, mode="reduce-overhead")
