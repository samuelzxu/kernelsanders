"""
MLA decode - nks=16 for ALL assembly paths + per-shape compile + TunableOp.

nks=16 validated for kv>1024 (attempt 166: tied with nks=32 at ~71µs).
Also test nks=16 for kv<=1024 (attempt 165 showed bs=64 improved -0.7µs).

nks=16 uses 50% smaller reduce buffers than nks=32, potentially helping
memory-bound configs without sacrificing bandwidth (16 waves saturates HBM).
"""
