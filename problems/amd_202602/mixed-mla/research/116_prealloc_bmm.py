...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 32 and kv_seq_len <= 1024:
        ...
        kv_t = kv.transpose(-2, -1)  # (bs, DQ, kv_len) - view, no copy
        v = kv[:, :, :DV]  # (bs, kv_len, DV) - view, no copy
        ...
        o = torch.empty((bs, NH, DV), dtype=BF16, device=q.device)
        torch.bmm(attn, v, out=o)
        return o
