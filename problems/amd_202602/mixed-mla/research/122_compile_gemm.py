...
def _gemm_attn_impl(q_3d, kv, bs, kv_seq_len):
    scores = torch.baddbmm(
        torch.empty((bs, NH, kv_seq_len), dtype=BF16, device=q_3d.device),
        q_3d, kv.transpose(-2, -1), beta=0, alpha=SM_SCALE)
    attn = F.softmax(scores, dim=-1, dtype=FP32).to(BF16)
    return torch.bmm(attn, kv[:, :, :DV])

_gemm_attn = torch.compile(_gemm_attn_impl, mode="max-autotune-no-cudagraphs")
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 32 and kv_seq_len <= 1024:
        # GEMM path: fused via torch.compile
        kv = kv_data["bf16"].view(bs, kv_seq_len, DQ)
        return _gemm_attn(q.view(bs, NH, DQ), kv, bs, kv_seq_len)
