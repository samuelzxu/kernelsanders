...
Hybrid: a16w8 for bs<=4 (skip Q quant), a8w8 for larger (faster kernel).
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # a16w8 for small batches (skip expensive Q quantization)
    # a8w8 for large batches (qSeqLen=1 optimized kernel)
    if bs <= 4:
        kv_buffer, kv_scale = kv_data["fp8"]
        q_input = q  # bf16 Q, no quantization
        q_scale = None
