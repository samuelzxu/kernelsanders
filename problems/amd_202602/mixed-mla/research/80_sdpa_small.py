"""
MLA decode - honest, SDPA for small cases.
SDPA fuses attention into 1 kernel (no metadata/reduce).
a8w8+Triton quant for large cases.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 32 and kv_seq_len <= 1024:
        # SDPA path: single fused kernel, no metadata needed
        kv_buffer = kv_data["bf16"]  # (total_kv, 1, 576) bf16
        kv_3d = kv_buffer.view(bs, kv_seq_len, DQ)  # (bs, kv_len, 576)

        # Q: (bs, 1, NH, DQ) -> (bs, NH, 1, DQ) for SDPA
        q_sdpa = q.view(bs, 1, NH, DQ).transpose(1, 2)  # (bs, NH, 1, DQ)

        # K: (bs, kv_len, DQ) -> (bs, 1, kv_len, DQ) -> broadcast to (bs, NH, kv_len, DQ)
        k_sdpa = kv_3d.unsqueeze(1)  # (bs, 1, kv_len, DQ)

        # V: first DV dims of KV, (bs, kv_len, DV) -> (bs, 1, kv_len, DV)
        v_sdpa = kv_3d[:, :, :DV].unsqueeze(1)  # (bs, 1, kv_len, DV)
        ...
        return out.transpose(1, 2).reshape(bs, NH, DV).to(BF16)

    else:
        # Persistent a8w8 path for large cases
