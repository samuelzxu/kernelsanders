"""
MLA decode - torch.compile for GEMM path, assembly for the rest.

Key idea: torch.compile(mode="reduce-overhead") fuses the GEMM attention
(bmm+softmax+bmm) into fewer kernel launches, reducing the ~15µs overhead
from 3+ separate kernel dispatches.

Inductor on MI355X uses CK backend (not Triton), avoiding JIT timeout.
Compilation happens during warmup iterations, cached for ranked runs.
"""
...
def _gemm_attention(q_3d, kv_t, v):
    s = torch.bmm(q_3d, kv_t)
    return torch.bmm(F.softmax(s, dim=-1, dtype=FP32).to(BF16), v)


# Compile the GEMM attention path for reduced kernel launch overhead
_compiled_gemm = torch.compile(_gemm_attention, mode="reduce-overhead", dynamic=True)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Compiled GEMM path: fused kernel launches
    if bs <= 4 or (bs <= 32 and kv_seq_len <= 1024):
        ...
        v = kv[:, :, :DV]
        return _compiled_gemm(q_scaled, kv_t, v)
