"""
MLA decode - honest, using get_mla_metadata_v1_no_redundant.
Simpler metadata: fewer params, returns tensors (no pre-alloc needed).
...
# Import the simpler metadata function
try:
    from aiter import get_mla_metadata_v1_no_redundant
    _HAS_SIMPLE_META = True
except ImportError:
    _HAS_SIMPLE_META = False
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if _HAS_SIMPLE_META:
        # Simpler: returns tensors directly, no pre-alloc needed
        wm, wi, wis, ri, rfm, rpm = get_mla_metadata_v1_no_redundant(
