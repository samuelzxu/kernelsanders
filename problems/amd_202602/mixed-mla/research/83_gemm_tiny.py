"""
MLA decode - honest. GEMM-based for bs=4 with torch.bmm.
Assembly for larger cases.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 4 and kv_seq_len <= 1024:
        # GEMM-based: Q @ K^T -> softmax -> attn @ V
        # Only 3 kernel launches (bmm + softmax + bmm), no metadata needed
        kv = kv_data["bf16"].view(bs, kv_seq_len, DQ)  # (bs, kv_len, 576)
        q_4d = q.view(bs, NH, 1, DQ)  # (bs, NH, 1, 576)
        k_4d = kv.unsqueeze(1).expand(-1, NH, -1, -1)  # (bs, NH, kv_len, 576)
        scores = torch.matmul(q_4d, k_4d.transpose(-2, -1)) * SM_SCALE  # (bs, NH, 1, kv_len)
        attn = F.softmax(scores, dim=-1)
        v_4d = kv[:, :, :DV].unsqueeze(1).expand(-1, NH, -1, -1)  # (bs, NH, kv_len, 512)
        out = torch.matmul(attn, v_4d)  # (bs, NH, 1, 512)
