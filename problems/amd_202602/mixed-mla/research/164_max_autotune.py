"""
MLA decode - max-autotune compiled GEMM + TunableOp + kvg=32.

Change vs 163: mode="max-autotune" for torch.compile.
max-autotune enables:
- More aggressive kernel search (CK + Triton + hipBLAS candidates)
- HIP graph trees for reduced dispatch overhead
- Better pointwise fusion

In attempt 157, reduce-overhead helped GEMM but hurt assembly.
With per-shape compilation (163), assembly regression was eliminated.
max-autotune may give even better GEMM kernels without assembly impact.
"""
...
# max-autotune: deeper kernel search + HIP graph trees
_compiled_gemm_short = torch.compile(_gemm_attn, mode="max-autotune")
_compiled_gemm_long = torch.compile(_gemm_attn, mode="max-autotune")
