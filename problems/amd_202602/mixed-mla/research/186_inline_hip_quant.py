"""
MLA decode - inline HIP quant via aiter.dynamic_per_tensor_quant.

Fix: avoid importing aiter.ops.quant (heavy module with ALL quant backends).
Instead, call aiter.dynamic_per_tensor_quant directly (already available
from `import aiter`). Also avoid importing Triton quant entirely.

Saves: 1 kernel launch (HIP fused quant) + lighter imports.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        # Inline HIP quant: single fused kernel, no extra module imports
        t2d = q.view(-1, DQ)
