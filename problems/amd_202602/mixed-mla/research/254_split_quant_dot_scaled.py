...
def _stage1(
    ...
    # Offsets for K loading (nope and rope parts)
    offs_k_nope = tl.arange(0, 256)   # NOPE_HALF = 256
    offs_k_rope = tl.arange(0, 32)    # ROPE_HALF = 32
    offs_sc_nope = tl.arange(0, 16)   # NOPE_SCALES = 16
    offs_sc_rope = tl.arange(0, 4)    # ROPE_SCALES = 2 → padded to 4
    ...
    if s1 > s0:
        for start_n in range(s0, s1, BLOCK_N):
            ...
            # --- Load K nope from MXFP4: (BLOCK_N, 256) + (BLOCK_N, 16) ---
            k_nope = tl.load(
            ...
            # --- Load K rope from MXFP4: (BLOCK_N, 32) + (BLOCK_N, 4 padded) ---
            k_rope = tl.load(
            ...
            k_rope_sc = tl.load(
                ...
                mask=valid[:, None] & (offs_sc_rope[None, :] < 2), other=0,
            )

            # --- Q@K^T = nope_score + rope_score ---
            qk = tl.zeros([PADDED_H, BLOCK_N], dtype=tl.float32)

            # Nope: (32, 256) e2m1 @ (32, 256) e2m1 → (32, 32) f32
            qk = tl.dot_scaled(
            ...
            # Rope: (32, 32) e2m1 @ (32, 32) e2m1 → (32, 32) f32
            qk = tl.dot_scaled(
