...
Change vs 132: also try nks=4 for the a16w8 path (kv<=1024, bs>32).
- bs=64/kv=1024: nks=4 -> 64*4=256 work items = exactly 1 wave on MI355X (256 CUs)
  Previously nks=8 -> 512 items = 2 waves
- 1 wave: no tail effect, faster scheduling, smaller reduce (256 vs 512 partial results)
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        kvg = 16
        # nks=4: bs=64->256 work items (1 wave), bs=256->1024 items (4 waves)
        # Previously nks=8: bs=64->512 (2 waves), bs=256->2048 (8 waves)
        # 1 wave is optimal: all 256 CUs busy with no tail overhead
        nks = 4
    else:
