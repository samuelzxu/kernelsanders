"""
MLA decode - dual compiled GEMM functions + TunableOp.

Two separate compiled functions to avoid dynamic shape penalty:
1. _compiled_short: kv<=1024 GEMM (dynamic=True for bs=4,32)
2. _compiled_long: kv>1024 GEMM (only bs=4/kv=8192)
Each specializes for its shape range.

In attempt 152: single dynamic compile helped kv=1024 (-4.5µs)
but hurt kv=8192 (+5µs). Split compilation avoids the conflict.
"""
...
# Separate compiled functions for different kv ranges
_compiled_short = torch.compile(_gemm_attn, dynamic=True)  # kv=1024: bs=4,32
_compiled_long = torch.compile(_gemm_attn)  # kv=8192: bs=4 only (fixed shape)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 4 or (bs <= 32 and kv_seq_len <= 1024):
        ...
        if kv_seq_len <= 1024:
            ...
            return _compiled_long(q_3d, kv_t, v, scores_buf)
