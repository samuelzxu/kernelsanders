"""
MLA decode - pre-computed kvi AFTER torch.compile (allocation order fix).

Hypothesis: the intermittent correctness failure in 173 was from
_KVI_SHORT being allocated BEFORE torch.compile's lazy compilation.
When compile captures a graph, its private memory pool may overlap
with _KVI_SHORT's memory.

Fix: allocate _KVI_SHORT AFTER the compile wrappers are created.
This ensures torch.compile's memory pool is established first.
Still uses view (no clone) for maximum speed.
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        kvg, nks = 32, 8
        kvi = _KVI_SHORT[:n]  # View of pre-computed (no GPU kernel)
    else:
        t2d = q.view(-1, DQ)
