"""
MLA decode - try a16w16 assembly kernel (bf16 Q + bf16 KV).

Discovery: aiter has specialized kernels we haven't tried:
- mla_dec_stage1_bf16_a16w16_subQ16_mqa16.co (specialized for NH=16 MQA)
- MLA_A16W16_1TG_4W_32mx1_16nx1_Coex0_Msk1_QH16.co (1 tile group, 4 warps)

These might be MORE efficient per byte despite reading 2x more data (bf16 vs fp8):
- No fp8 dequantization overhead inside the kernel
- Better memory alignment (2-byte bf16 vs 1-byte fp8)
- Specialized tiling for NH=16 decode config

Trade-off: bf16 KV = 2x more bandwidth, but potentially faster kernel.
Try for kv<=1024 where KV data is small (75.5MB vs 37.7MB fp8).
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Try a16w16 (bf16 Q + bf16 KV) for kv<=1024 assembly path
    if kv_seq_len <= 1024:
        # Use bf16 KV with a16w16 kernel (specialized for NH=16 MQA)
        kv_buffer_bf16 = kv_data["bf16"]
        n = kv_buffer_bf16.shape[0]
        q_input, q_scale, qd = q, None, BF16
        kvd = BF16  # bf16 KV → triggers a16w16 kernel
        kvg, nks = 32, 8
        kv_scale = None
        ...
        aiter.mla_decode_stage1_asm_fwd(
            ...
            kv_buffer_bf16.view(n, 1, NKV, DQ),
    ...
    # Standard a8w8 for kv>1024 (fp8 KV, proven optimal)
