...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 32 and kv_seq_len <= 1024:
        ...
        # QK^T with fused scale via baddbmm (1 kernel instead of matmul+scale)
        scores = torch.empty((bs, NH, kv_seq_len), dtype=BF16, device=q.device)
        torch.baddbmm(scores, q_3d, kv.transpose(-2, -1), beta=0, alpha=SM_SCALE, out=scores)
        attn = F.softmax(scores, dim=-1, dtype=FP32).to(BF16)  # FP32 softmax for precision
        return torch.bmm(attn, kv[:, :, :DV])  # (bs, NH, DV)
