"""
MLA decode - TunableOp with rotating buffer + per-shape compile + kvg=32.

Change vs 163: add PYTORCH_TUNABLEOP_ROTATING_BUFFER_SIZE=256
This rotates input tensors during tuning to simulate cold-cache conditions,
potentially selecting kernels that perform better in real workloads
(vs default tight-loop tuning that over-optimizes for cache-hot data).

Also try per-config nks for kv<=1024:
- bs=64/kv=1024: nks=16 → 1024 CTAs = 4 waves (was nks=8 → 2 waves)
- bs=256/kv=1024: nks=8 (unchanged, already 8 waves)
More waves showed improvement for kv>1024 (kvg=32 helped because of
better work distribution). Test if same applies to kv<=1024.
"""
...
os.environ.setdefault('PYTORCH_TUNABLEOP_ROTATING_BUFFER_SIZE', '256')
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        # Per-config nks: bs=64→nks=16 (4 waves), bs=256→nks=8 (8 waves)
        kvg = 16
        nks = 16 if bs <= 64 else 8
    else:
