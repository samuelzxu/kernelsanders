"""
MLA decode — extend GEMM to bs<=64/kv<=1024, a16w8 assembly for rest.

Key insight: bs=64/kv=1024 GEMM should beat assembly because:
- Per-XCD: 8 batch items × 1024×576×2 = 9.4MB KV fits in L2 (16-32MB/XCD)
- baddbmm reads K from HBM into L2; bmm reads V from L2 (cache hit!)
- Effective HBM: ~75.5MB (single pass) vs assembly reading 37.7MB fp8 + 16MB lg
- Assembly overhead (metadata, reduce, kernel dispatch) adds ~25µs

Assembly: a8w8 for kv>1024 (Q quantization required but kernel is MUCH faster).
a16w8 for kv<=1024 (assembly path only used for bs=256/kv=1024 now).
Uniform kvg=16, nks=32 matching reference implementation.
"""

import torch
import torch.nn.functional as F
from task import input_t, output_t

import aiter
from aiter import dtypes as aiter_dtypes
from aiter import get_mla_metadata_info_v1, get_mla_metadata_v1
from aiter.ops.triton.quant.quant import dynamic_per_tensor_quant_fp8_i8

FP8_DTYPE = aiter_dtypes.fp8
BF16 = torch.bfloat16
FP32 = torch.float32
NH = 16
NKV = 1
DQ = 576
DV = 512
SM_SCALE = 1.0 / (DQ ** 0.5)


@torch.inference_mode()
def custom_kernel(data: input_t) -> output_t:
    q, kv_data, qo_indptr, kv_indptr, cfg = data
    bs = cfg["batch_size"]
    kv_seq_len = cfg.get("kv_seq_len", 1024)

    # GEMM path: extended to bs<=64/kv<=1024 + bs<=4 for all kv
    # For bs<=64/kv<=1024: per-XCD KV (9.4MB) fits in L2 cache
    # → V reads hit L2 on second pass (bmm), halving effective HBM reads
    if bs <= 4 or (bs <= 64 and kv_seq_len <= 1024):
        kv = kv_data["bf16"].view(bs, kv_seq_len, DQ)
        q_3d = q.view(bs, NH, DQ)
        v = kv[:, :, :DV]
        kv_t = kv.transpose(-2, -1)
        s = torch.baddbmm(
            torch.empty(bs, NH, kv_seq_len, dtype=BF16, device=q.device),
            q_3d, kv_t, beta=0, alpha=SM_SCALE)
        return torch.bmm(F.softmax(s, dim=-1, dtype=FP32).to(BF16), v)

    kv_buffer, kv_scale = kv_data["fp8"]
    n = kv_buffer.shape[0]

    # Assembly path: only reached for bs=256/kv=1024 or any kv>1024
    if kv_seq_len <= 1024:
        # a16w8: bf16 Q + fp8 KV (no Q quantization needed)
        q_input, q_scale, qd = q, None, BF16
    else:
        # a8w8: fp8 Q + fp8 KV (a8w8 kernel is MUCH faster for kv>1024)
        t2d = q.view(-1, q.shape[-1])
        qx = torch.empty_like(t2d, dtype=FP8_DTYPE)
        scale = torch.zeros(1, dtype=FP32, device=q.device)
        dynamic_per_tensor_quant_fp8_i8(qx, t2d, scale)
        q_input, q_scale, qd = qx.view(q.shape), scale, FP8_DTYPE

    # Uniform kvg=16, nks=32 matching reference implementation
    kvg, nks = 16, 32

    kvd = kv_buffer.dtype
    info = get_mla_metadata_info_v1(
        bs, 1, NH, qd, kvd, is_sparse=False, fast_mode=False,
        num_kv_splits=nks, intra_batch_mode=True)
    wm, wi, wis, ri, rfm, rpm = [torch.empty(s, dtype=t, device="cuda") for s, t in info]
    klp = qo_indptr[1:bs+1]
    kvi = torch.arange(n, dtype=torch.int32, device="cuda")

    get_mla_metadata_v1(
        qo_indptr, kv_indptr, klp, NH // NKV, NKV, True,
        wm, wis, wi, ri, rfm, rpm,
        page_size=1, kv_granularity=kvg,
        max_seqlen_qo=1, uni_seqlen_qo=1,
        fast_mode=False, max_split_per_batch=nks,
        intra_batch_mode=True, dtype_q=qd, dtype_kv=kvd)

    np_ = bs * nks
    lg = torch.empty((np_, 1, NH, DV), dtype=FP32, device="cuda")
    ls = torch.empty((np_, 1, NH, 1), dtype=FP32, device="cuda")
    o = torch.empty((bs, NH, DV), dtype=BF16, device="cuda")

    aiter.mla_decode_stage1_asm_fwd(
        q_input.view(-1, NH, DQ),
        kv_buffer.view(n, 1, NKV, DQ),
        qo_indptr, kv_indptr, kvi, klp,
        None, wm, wi, wis, 1, 1, NKV, SM_SCALE,
        lg, ls, o, q_scale, kv_scale)
    aiter.mla_reduce_v1(lg, ls, ri, rfm, rpm, 1, o, None)
    return o
