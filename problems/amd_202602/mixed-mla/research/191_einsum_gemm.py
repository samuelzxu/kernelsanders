"""
MLA decode - try torch.einsum for GEMM attention (different Inductor lowering).

torch.einsum('bhd,bdk->bhk', q_scaled, kv_t) might trigger a different
compiled kernel path than torch.baddbmm. Inductor may lower einsum
differently, potentially finding a faster kernel for our M=16 shapes.

Also: pre-scale Q outside the compiled function (one less op inside graph).
"""
...
def _gemm_attn_einsum(q_scaled, kv_t, v):
    # einsum may trigger different Inductor lowering than baddbmm
    scores = torch.bmm(q_scaled, kv_t)
    return torch.bmm(F.softmax(scores, dim=-1, dtype=FP32).to(BF16), v)


_compiled_gemm_short = torch.compile(_gemm_attn_einsum)
_compiled_gemm_long = torch.compile(_gemm_attn_einsum)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if bs <= 4 or (bs <= 32 and kv_seq_len <= 1024):
        ...
        # Pre-scale Q OUTSIDE compiled fn (saves 1 op inside graph)
        q_scaled = q.view(bs, NH, DQ) * SM_SCALE
        v = kv[:, :, :DV]
        kv_t = kv.transpose(-2, -1)

        if kv_seq_len <= 1024:
            return _compiled_gemm_short(q_scaled, kv_t, v)
        else:
            return _compiled_gemm_long(q_scaled, kv_t, v)
