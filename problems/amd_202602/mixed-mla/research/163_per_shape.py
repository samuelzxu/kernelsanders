"""
MLA decode - per-shape compiled GEMM + TunableOp + kvg=32.

Shape-specialized dispatch: each GEMM config gets its own compiled function
(no dynamic=True overhead, no guard checks between shapes).

Benchmark configs (known):
  bs=4/kv=1024, bs=4/kv=8192, bs=32/kv=1024 → GEMM
  bs=32/kv=8192, bs=64/kv=8192, bs=256/kv=8192 → a8w8 assembly
  bs=64/kv=1024, bs=256/kv=1024 → a16w8 assembly
"""
...
def _gemm_attn(q_3d, kv_t, v, scores_buf):
...
# Per-shape compiled functions (no dynamic=True overhead)
# Each compiles separately during warmup for its exact tensor shapes
_compiled_gemm_short = torch.compile(_gemm_attn)   # kv=1024: bs=4 and bs=32
_compiled_gemm_long = torch.compile(_gemm_attn)    # kv=8192: bs=4 only
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # GEMM path: shape-specialized compiled functions
    if bs <= 4 or (bs <= 32 and kv_seq_len <= 1024):
        ...
        if kv_seq_len <= 1024:
            return _compiled_gemm_short(q_3d, kv_t, v, scores_buf)
        else:
            return _compiled_gemm_long(q_3d, kv_t, v, scores_buf)

    # Assembly path: per-config optimized parameters
    ...
    if kv_seq_len <= 1024:
        ...
        # a8w8: fp8 Q + fp8 KV, kvg=32 (proven best in attempt 160)
