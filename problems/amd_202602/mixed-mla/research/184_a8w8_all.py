"""
MLA decode - a8w8 for ALL assembly paths (use qSeqLen=1 optimized kernel).

CRITICAL DISCOVERY from mla_asm.csv:
- a16w8 persistent: only has qSeqLen=4 kernel! (m16x4, 4 queries/block)
- a8w8 persistent: has dedicated qSeqLen=1 kernel! (qseqlen1_gqaratio16)

We've been using the WRONG kernel for kv<=1024!
The a16w8 qSeqLen=4 kernel wastes ~75% compute processing 3 empty query slots.

Fix: use a8w8 (fp8 Q + fp8 KV) for ALL assembly paths.
Cost: +3µs Q quantization for kv<=1024.
Benefit: use the qSeqLen=1-optimized a8w8 kernel (potentially -10µs).
Net: potentially -7µs for kv<=1024 assembly configs!

bs=64/kv=1024: 50.6µs → potentially ~43µs (huge improvement!)
bs=256/kv=1024: 113µs → potentially ~106µs
"""
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # ALWAYS use a8w8 (fp8 Q + fp8 KV) for ALL assembly paths
    # This selects the qSeqLen=1-optimized kernel for decode!
    t2d = q.view(-1, DQ)
