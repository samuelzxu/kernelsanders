"""
MLA decode - honest, try CPU-based metadata (no_redundant).
Falls back to GPU metadata if not available.
...
# Try to import the CPU-based metadata function
try:
    from aiter.ops.attention import get_mla_metadata_v1_no_redundant as _nr_meta
    _HAS_NR = True
except Exception:
    _HAS_NR = False
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if _HAS_NR:
        # CPU-based metadata: no GPU kernel launch needed!
        wm, wi, wis, ri, rfm, rpm = _nr_meta(
            qo_indptr, kv_indptr, NH // NKV, NKV, True, kvg)
        klp = torch.ones(bs, dtype=torch.int32, device="cuda")
    else:
        # GPU-based metadata (fallback)
        kvd = kv_buffer.dtype
