"""
MLA decode - pre-computed kvi with .clone() to avoid memory aliasing.

Attempt 173 gave 69.5µs but had ~25% intermittent correctness failure.
Hypothesis: the _KVI_SHORT[:n] view shares memory with the module-level
tensor, and torch.compile's internal memory management occasionally
overwrites this shared memory.

Fix: use .clone() to create an independent copy. This adds one small
memcpy (~0.5µs for 1MB) but eliminates the aliasing risk.
"""
...
# Pre-compute kvi values at module level (1MB)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    if kv_seq_len <= 1024:
        ...
        kvg, nks = 32, 8
        # Clone to avoid memory aliasing with torch.compile internals
        kvi = _KVI_SHORT[:n].clone()
    else:
        t2d = q.view(-1, DQ)
