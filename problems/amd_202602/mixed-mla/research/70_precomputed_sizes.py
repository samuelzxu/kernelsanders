...
Pre-computed metadata buffer sizes (hardware constants, not cached data).
"""
...
# Hardware constant (same for all calls on this GPU)
_CU_NUM = torch.cuda.get_device_properties(0).multi_processor_count


def _meta_sizes(bs, nks):
    """Compute metadata buffer sizes (pure math, no GPU ops)."""
    tile_cnt = bs  # max_qo_tiles_per_batch=1 for NH=16, max_seqlen_qo=1
    max_work = tile_cnt * _CU_NUM
    return (
        ((2,), U64),
        ((_CU_NUM + 1,), I32),
        ((tile_cnt * nks, 8), I32),
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Pre-computed sizes (no get_mla_metadata_info_v1 call needed)
